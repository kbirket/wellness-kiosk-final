...
export default function WellnessHub() {
  ...
  const getBaseRate = (p, b) => {
    ...
    if (['MILITARY', 'HD6', 'HD6 FAMILY', 'FIRST DAY FREE'].includes(p)) return 0;
  ...
  <div className="bg-white p-4 md:p-6 rounded-xl shadow-sm border border-slate-200 print:shadow-none print:border-slate-300" style={{ borderLeft: `6px solid ${color}` }}>
    <p className="text-3xl md:text-5xl font-extrabold mb-1 print:text-3xl" style={{ color }}>{value}</p>
    <p className="text-[10px] md:text-xs font-bold text-[#001f3f] uppercase tracking-tight print:text-[10px]">{label}</p>
  </div>
);
  ...
  return (
    <div classname="flx min-h-screen bg- [#f02f5] font-sans text-slate-800">
...
<div className="md:hidden flex items-center justify-between mb-4 -mt-4">
  <button onClick={() => setSidebarOpen(!sidebarOpen)} className="bg-[#001f3f] text-white p-3 rounded-xl shadow-lg">
    <LayoutDashboard size={20} />
  </button>
  <img src={LOGO_URL} alt="Logo" className="h-6 invert" />
</div>
{sidebarOpen && <div className="fixed inset-0 bg-black/50 z-30 md:hidden" onClick={() => setSidebarOpen(false)} />}
  ...
  <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-x-auto pb-24">
  <table className="w-full text-left border-collapse min-w-[700px]">
    <thead className="bg-slate-50 text-[11px] font-black text-slate-400 uppercase tracking-widest border-b"><tr><th className="px-6 py-4">Visitor</th>
<th className="px-4 py-4">Pass Type</th><th className="px-4 py-4">Referring Provider</th><th className="px-4 py-4">Expires</th><th className="px-4 py-4">Status</th><th className="px-4 py-4">Visits</th><th className="px-4 py-4">Actions</th></tr></thead>
              ...
              {corporatePartners.map(corp => {
                 ...
                 return (
                   <div key={corp.id} className={`bg-white p-6 rounded-2xl shadow-sm border flex flex-col justify-between transition-all ${isPaid ? 'border-green-300' : 'border-slate-200'}`}>
                     <div>
                       <div className="flex justify-between items-start mb-1"><h3 className="font-black text-[#001f3f] text-xl">{corp.name}</h3><div className="flex items-center gap-2"><button onClick={() => setEditingCorp(corp)} className="bg-blue-50 text-[#1080ad] hover:bg-blue-100 px-3 py-1 rounded-full text-[10px] font-black tracking-widest uppercase transition-colors">Edit</button><span className={`px-3 py-1 rounded-full text-[10px] font-black tracking-widest ${isPaid ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>{isPaid ? `PAID ${corpPayMethod ? `(${corpPayMethod.toUpperCase()})` : ''}` : 'UNPAID'}</span></div></div>
                       ...
                       <div className="space-y-2 mb-6">
                         ...
                         <div className="flex justify-between items-center bg-blue-50 p-3 rounded-lg border border-blue-100"><span className="text-xs font-bold text-blue-600 uppercase">Visits ({displayPeriod.split(' ')[0]})</span><span className="text-lg font-black text-[#1080ad]">{totalPeriodVisits}</span></div>
                       ...
                       <label className="flex items-center gap-2 mb-6 cursor-pointer group w-fit"><input type="checkbox" checked={isUsageBased} onChange={(e) => setUsageBasedCorps(prev => ({...prev, [corp.id]: e.target.checked}))} className="w-4 h-4 text-[#1080ad] rounded border-slate-300 cursor-pointer" /><span className="text-xs font-bold text-slate-500 uppercase tracking-widest group-hover:text-[#1080ad] transition-colors">Usage-Based Billing</span></label>
                       <button onClick={() => setExpandedCorpId(expandedCorpId === corp.id ? null : corp.id)} className="w-full text-left py-2 border-t border-slate-100 flex justify-between items-center text-xs font-bold text-slate-500 hover:text-[#1080ad] transition-colors mb-4">{expandedCorpId === corp.id ? 'Hide Employee Roster ↑' : 'View Employee Roster ↓'}</button>
                       {expandedCorpId === corp.id && (<div className="mb-6 bg-slate-50 rounded-xl border border-slate-100 overflow-hidden"><div className="max-h-48 overflow-y-auto p-3 space-y-2">{enrichedMembers.length === 0 ? <p className="text-xs text-slate-400 italic">No members enrolled.</p> : enrichedMembers.map(em => (<div key={em.id} className="flex justify-between items-center bg-white p-2 rounded-lg border border-slate-100 shadow-sm"><div><p className="text-xs font-bold text-slate-800">{em.firstName} {em.lastName}</p><p className="text-[9px] text-slate-400 uppercase tracking-widest">{em.type} • {em.periodVisits} visit{em.periodVisits !== 1 ? 's' : ''}</p></div><span className={`text-xs font-black ${em.memberOwed > 0 ? 'text-[#16a34a]' : 'text-slate-400'}`}>${em.memberOwed.toFixed(2)}</span></div>))}</div></div>)}
                     </div>
        ...
        {activeTab === 'reports' && (() => {
          ...
          const revenueByPlan = {};
          scopedMembers.filter(m => m.status === 'ACTIVE').forEach(m => {
            const rate = parseFloat(String(m.monthlyRate).replace(/[^0-9.]/g, '')) || 0;
            const planLabel = m.type.includes('CORPORATE') ? 'Corporate' : m.type.includes('HD6') || m.type.includes('MILITARY') ? 'Comped' : m.type === 'SINGLE' ? 'Single' : m.type.includes('FAMILY') ? 'Family' : m.type.includes('SENIOR') ? 'Senior' : m.type.includes('STUDENT') ? 'Student' : 'Other';
          ...
          const revChartData = Object.entries(revenueByPlan).filter(([_, v]) => v > 0).sort((a, b) => b[1] - a[1]).map(([label, value]) => {
            const colorMap = { Single: '#1080ad', Family: '#f59e0b', Senior: '#16a34a', Student: '#8b5cf6', Corporate: '#ef4444', Comped: '#94a3b8', Other: '#64748b' };
            return { label, value: Math.round(value), color: colorMap[label] || '#64748b' };
          });
          const projectedRevenue = scopedMembers.filter(m => m.status === 'ACTIVE').reduce((sum, m) => sum + (parseFloat(String(m.monthlyRate).replace(/[^0-9.]/g, '')) || 0), 0);
          const collectedRevenue = scopedMembers.filter(m => m.status === 'ACTIVE' && m.paymentMethod).reduce((sum, m) => sum + (parseFloat(String(m.monthlyRate).replace(/[^0-9.]/g, '')) || 0), 0);
          const corpCollected = corporatePartners.reduce((sum, cp) => {
            ...
            const corpMems = scopedMembers.filter(m => m.sponsorName === cp.sponsorMatch && m.status === 'ACTIVE');
            return sum + corpMems.reduce((s, m) => s + (parseFloat(String(m.monthlyRate).replace(/[^0-9.]/g, '')) || 0), 0);
          }, 0);
          const totalCollected = collectedRevenue + corpCollected;
          const collectionRate = projectedRevenue > 0 ? Math.round((totalCollected / projectedRevenue) * 100) : 0;
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
                  ...
                  {revChartData.length > 0 ? (
                    <div className="space-y-4">
                      <DonutChart data={revChartData} totalLabel="Projected" />
                    </div>
                  ...
                  <div className="space-y-6">
                    <div>
                      <div className="flex justify-between items-end mb-2">
                        <div><p className="text-xs font-bold text-slate-400 uppercase tracking-widest">Projected Monthly Revenue</p><p className="text-3xl font-black text-[#001f3f]">${Math.round(projectedRevenue).toLocaleString()}</p></div>
                      </div>
                    </div>
                    <ProgressBar value={Math.round(totalCollected)} max={Math.round(projectedRevenue)} color="#16a34a" label="Collected vs Projected" />
                    <div className="grid grid-cols-2 gap-4 mt-4">
                      ...
                      <div className="bg-red-50 p-4 rounded-xl border border-red-100">
                        ...
                        <p className="text-2xl font-black text-red-500">${Math.round(Math.max(0, projectedRevenue - totalCollected)).toLocaleString()}</p>
                      </div>
                    </div>
        ...
        {activeTab === 'help' && (() => {
          const faqs = [
            ...
            { category: "Corporate & Usage-Based Billing", icon: <Briefcase size={20} className="text-[#8b5cf6]" />, items: [
                ...
                { id: 'corp_pay', q: "How do I log a corporate payment?", a: "When a business pays their invoice, click the 'Mark as Paid' button on their company card. A popup will ask you how they paid (e.g., Check, ACH). This payment method will be saved and automatically applied to all of their employees on the monthly Financial Report!" },
            ...
            { category: "Reporting & Exports", icon: <FileText size={20} className="text-[#dd6d22]" />, items: [
                ...
                { id: 'r2', q: "How does the CFO Financial Report work?", a: "Go to the 'Reports' tab, select your month/quarter, and click 'Print Financials' or 'CSV'.<br/><br/><b>Important Note:</b> This report <i>only</i> shows members who have a logged payment for that period. Anyone who hasn't paid will be automatically hidden so the report is perfectly accurate for the CFO. The report also automatically splits into two clean sections: Standard Members (paying individually) and Corporate Members (paid by their employer)." },
                { id: 'r3', q: "What is the Board Report?", a: "The 'Board Report' button generates a one-page printable leadership summary with all the KPIs: total visits, new sign-ups, retention rate, average visits per member, revenue breakdown, top visitors, and at-risk members. Perfect for monthly board meetings at Patterson HC." }
            ]}
          ];
          return (
