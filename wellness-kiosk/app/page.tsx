...
export default function WellnessHub() {
  ...
  const [activeClass, setActiveClass] = useState(null);   const [savedClassRosters, setSavedClassRosters] = useState(() => { try { return JSON.parse(localStorage.getItem('wellnessClassRosters')) || {}; } catch(e) { return {}; } });
  ...
  useEffect(() => { localStorage.setItem('wellnessUsagePrefs', JSON.stringify(usageBasedCorps)); }, [usageBasedCorps]);   useEffect(() => { localStorage.setItem('wellnessClassRosters', JSON.stringify(savedClassRosters)); }, [savedClassRosters]);
...
// --- QR SCANNER ENGINE FOR KIOSK ---
  ...
  var showToast = function(message, type, duration) { setToast({ message: message, type: type || 'success' }); setTimeout(function() { setToast(null); }, duration || 4000); };    if (!isMounted) return <div className="min-h-screen bg-[#001f3f]" />;
