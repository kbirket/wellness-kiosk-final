            ...
            <div className="flex-1 bg-[#f5f6f8] flex flex-col items-center justify-center px-10 py-8 relative">
              ...
              {kioskMessage.text && (
                <div className={`mt-8 px-8 py-4 rounded-xl text-lg font-black flex items-center gap-4 ${kioskMessage.type === 'success' ? 'bg-green-50 text-green-700 border border-green-200' : kioskMessage.type === 'warning' ? 'bg-amber-50 text-amber-700 border border-amber-200' : 'bg-red-50 text-red-700 border border-red-200'}`}>
                  ...
                  {kioskMessage.subtext && <span className="block text-xs font-bold mt-1 opacity-70">{tKiosk(kioskMessage.subtext)}</span>}
                </div></div>
              )}
            </div>
          </div>
        ...
        {isScanning && (
          <div className="fixed inset-0 bg-[#001226]/95 backdrop-blur-2xl z-[200] flex flex-col items-center justify-center p-6">
            ...
            <div className="mb-12 text-center">
              ...
              <p className="text-[#1080ad] font-bold tracking-widest animate-pulse uppercase text-sm">{kioskLang === 'es' ? 'Alinee el código QR' : 'Align QR Code within the markers'}</p>
            </div>
            ...
            <div className="space-y-8">
              ...
              {(() => {
                ...
                blockedCheckins.filter(b => b.outcome === 'Pending' && b.reason !== 'Cooldown (2-hr)').forEach(b => {
                  const mem = members.find(m => m.airtableId === b.memberRecId);
                  let stillRelevant = true;
                  if (mem) { var rr = b.reason || ''; if (rr.indexOf('Missing') === 0) stillRelevant = (!mem.basicOrientation || !mem.paperworkCompleted || (mem.isMinor && !mem.parentPermission)); else if (rr.indexOf('Anthony Orientation') !== -1) stillRelevant = !mem.orientationAnthony; else if (rr.indexOf('Harper Orientation') !== -1) stillRelevant = !mem.orientationHarper; else if (rr === 'Past Due Payment') stillRelevant = getStoplight(mem) === 'red'; else if (rr === 'Inactive Membership') stillRelevant = !!mem.inactive; }
