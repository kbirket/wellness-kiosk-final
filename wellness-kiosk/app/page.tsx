...
export default function WellnessHub() {
  ...
  if (view === 'kiosk') { return (<div className="min-h-screen bg-[#f0f2f5] flex flex-col items-center justify-center p-6 font-sans relative overflow-hidden"><button onClick={() => {
...
const exportTodaysLog = () => { const tv = filteredVisits.filter(v => new Date(v.time).toDateString() === todayStr); if (tv.length === 0) { alert('No check-ins today yet.'); return; } const csv = ["Name,Center,Time,Type,Check-In Method",...tv.map(v => `"${v.name}","${v.center}","${new Date(v.time).toLocaleTimeString()}","${v.type}","${v.method || 'General Workout'}"`)].join('\n'); const b = new Blob([csv],{type:'text/csv'}); const u = window.URL.createObjectURL(b); const a = document.createElement('a'); a.href=u; a.download=`Check_Ins_${new Date().toISOString().slice(0,10)}.csv`; a.click(); window.URL.revokeObjectURL(u); };          const greeting = today.getHours() < 12 ? 'Good morning' : today.getHours() < 17 ? 'Good afternoon' : 'Good evening';
        ...
        {activeTab === 'classes' && (() => {
          ...
          return (
            <div className="space-y-6">
                ...
                {displayedClasses.map((c, i) => (
                  <div key={i} className={`bg-white p-6 rounded-2xl shadow-sm border border-slate-100 border-t-4 ${c.color} flex flex-col justify-between`}>
                    ...
                    <div className="flex justify-between items-end mt-6">
                      <button onClick={() => setActiveClass(c)} className="w-full bg-blue-50 text-blue-600 hover:bg-blue-600 hover:text-white px-4 py-3 rounded-xl text-sm font-bold transition-colors shadow-sm flex items-center justify-center gap-2">
                        <QrCode size={16} /> Start Class Scanner
                      </button>
                    </div>
                  </div>
                ))}
              ...
              {activeClass && (() => {
                // BRILLIANT FIX: This filters the roster feed to ONLY show people specifically checked into THIS exact class today!
              ...
              <div className="flex justify-between items-center mb-6">
                ...
                <div className="flex gap-4">
                  ...
                  <button onClick={() => {
                    ...
                    const rows = scopedMembers.map(m => `<tr><td>${m.firstName} ${m.lastName}</td><td>${m.type}</td><td>${m.billingMethod || 'N/A'}</td><td>$${parseFloat(String(m.monthlyRate).replace(/[^0-9.]/g, '')) || '0'}</td><td>${m.nextPayment || 'N/A'}</td><td><span style="color: ${m.status === 'ACTIVE' ? '#16a34a' : '#dc2626'}; font-weight: bold;">${m.status}</span></td></tr>`).join('');
                    const html = `<!DOCTYPE html><html><head><title>Financial Roster - ${centerName}</title><style>@media print { body { margin: 0; -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; } } body { font-family: Arial, sans-serif; color: #1e293b; margin: 0; padding: 40px; } .hdr { background: #003d6b; padding: 20px; display: flex; align-items: center; gap: 15px; border-radius: 8px 8px 0 0; } .hdr img { height: 40px; } .hdr-text { color: white; } .hdr-title { font-size: 24px; font-weight: 900; margin: 0; } .hdr-sub { font-size: 12px; color: #8bb8d9; text-transform: uppercase; letter-spacing: 1px; } .accent { height: 4px; background: linear-gradient(to right, #dba51f, #dd6d22); margin-bottom: 30px; } .summary { display: flex; justify-content: space-between; margin-bottom: 20px; background: #f8fafc; padding: 15px; border-radius: 8px; border: 1px solid #e2e8f0; } .sum-box { text-align: center; } .sum-lbl { font-size: 10px; font-weight: bold; color: #64748b; text-transform: uppercase; letter-spacing: 1px; } .sum-val { font-size: 24px; font-weight: 900; color: #003d6b; margin-top: 5px; } table { width: 100%; border-collapse: collapse; margin-bottom: 30px; font-size: 12px; } th { background: #003d6b; color: white; text-align: left; padding: 12px; font-size: 10px; text-transform: uppercase; letter-spacing: 1px; } td { padding: 10px 12px; border-bottom: 1px solid #e2e8f0; } tr:nth-child(even) { background-color: #f8fafc; }</style></head><body><div class="hdr"><img src="${LOGO_URL}" /><div class="hdr-text"><h1 class="hdr-title">Financial Roster</h1><div class="hdr-sub">${centerName} • ${new Date().toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</div></div></div><div class="accent"></div><div class="summary"><div class="sum-box"><div class="sum-lbl">Total Members</div><div class="sum-val">${scopedMembers.length}</div></div><div class="sum-box"><div class="sum-lbl">Active Accounts</div><div class="sum-val" style="color: #16a34a;">${scopedMembers.filter(m=>m.status==='ACTIVE').length}</div></div><div class="sum-box"><div class="sum-lbl">Expected Monthly Revenue</div><div class="sum-val" style="color: #dd6d22;">$${totalRevenue.toFixed(2)}</div></div></div><table><thead><tr><th>Member Name</th><th>Plan Type</th><th>Billing Method</th><th>Monthly Rate</th><th>Next Payment</th><th>Status</th></tr></thead><tbody>${rows}</tbody></table></body></html>`;
