                      ...
                      return (
                        <tr key={p.airtableId} className="border-b hover:bg-slate-50/80">
                          ...
                          <td className="px-4 py-4">
                           {user && user.role !== 'business' && p.memberId !== 'VISITOR' && (<div className="flex gap-1 justify-end"><button onClick={function() { setEditPaymentModal(p); }} className="text-slate-300 hover:text-[#1080ad] transition-colors p-1" title="Edit this payment"><FileText size={14}/></button>{!p.isRefund && (<button onClick={function() { var mem = members.find(function(m) { return m.airtableId === p.memberRecId; }); if (!mem) { alert('Member not found.'); return; } setRefundModal({ member: mem, originalPayment: p }); setRefundAmount(String(parseFloat(p.amount) || 0)); setRefundReason(''); setRefundMethod(p.method || 'Cash'); setRefundCheckNumber(''); setRefundDate(new Date().toISOString().split('T')[0]); }} className="text-slate-300 hover:text-orange-500 transition-colors p-1" title="Issue a refund"><RotateCcw size={14}/></button>)}
