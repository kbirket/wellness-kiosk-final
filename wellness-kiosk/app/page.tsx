...
This uses 1 blank 3-up keytag CR80 card.')) return;                     const renderKeytag = (data) => {                       if (!data) { return '<div class="keytag empty"><div class="empty-msg">Blank</div></div>'; }                       const m = data.m;                       const isHarper = m.center && m.center.toLowerCase().includes('harper');                       const accentColor = isHarper ? '#dba51f' : '#003d6b';                       const centerLabel = isHarper ? 'HARPER' : 'ANTHONY';                       const qrColor = isHarper ? 'dba51f' : '003d6b';                       const qrUrl = 'https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=' + encodeURIComponent(m.id) + '&color=' + qrColor + '&bgcolor=ffffff';                       const photoHtml = m.photoUrl ? '<img class="photo" src="' + m.photoUrl + '" />' : '<div class="photo initials" style="background:' + accentColor + '">' + (m.firstName[0] || '').toUpperCase() + (m.lastName[0] || '').toUpperCase() + '</div>';                       return '<div class="keytag"><div class="accent-bar" style="background:' + accentColor + '"></div>' + photoHtml + '<div class="info"><div class="name">' + m.firstName + ' ' + m.lastName + '</div><div class="id">ID: ' + m.id + '</div><div class="center" style="color:' + accentColor + '">' + centerLabel + ' WELLNESS</div></div><img class="qr" src="' + qrUrl + '" /></div>';                     };                     const keytags = [0, 1, 2].map(i => renderKeytag(memberBatch[i])).join('');                     const html = '<!DOCTYPE html><html><head><title>3-Up Keytags Batch</title><link href="https://fonts.googleapis.com/css2?family=Montserrat:wght@400;700;900&display=swap" rel="stylesheet"><style>@page{size:3.375in 2.125in;margin:0}@media print{*{-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}}body{font-family:Montserrat,Arial,sans-serif;margin:0;padding:0;background:#fff}.card{width:3.375in;height:2.125in;display:flex;flex-direction:column;box-sizing:border-box}.keytag{flex:1;display:flex;flex-direction:row;align-items:center;padding:0.04in 0.06in 0.04in 0.14in;box-sizing:border-box;position:relative;border-bottom:1px dashed #cbd5e1;overflow:hidden}.keytag:last-child{border-bottom:none}.keytag.empty{background:#f8fafc;justify-content:center;align-items:center}.empty-msg{font-size:8px;color:#cbd5e1;text-transform:uppercase;letter-spacing:2px;font-weight:700}.accent-bar{position:absolute;left:0;top:0.05in;bottom:0.05in;width:0.06in;border-radius:2px}.photo{width:0.55in;height:0.55in;object-fit:cover;border-radius:4px;flex-shrink:0;margin-right:0.08in}.photo.initials{display:flex;align-items:center;justify-content:center;color:#fff;font-weight:900;font-size:16px;letter-spacing:1px}.info{flex:1;display:flex;flex-direction:column;justify-content:center;overflow:hidden;padding-right:0.05in}.name{font-size:11px;font-weight:900;color:#001f3f;line-height:1.1;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;letter-spacing:-0.2px}.id{font-family:monospace;font-size:8px;font-weight:700;color:#64748b;margin-top:2px;letter-spacing:0.5px}.center{font-size:7px;font-weight:900;letter-spacing:1.5px;margin-top:3px}.qr{width:0.55in;height:0.55in;flex-shrink:0}</style></head><body><div class="card">' + keytags + '</div></body></html>';                     const w = window.open('', '_blank');                     w.document.write(html);                     w.document.close();                     setTimeout(() => w.print(), 1000);                     if (!window.confirm('Mark these ' + memberBatch.length + ' fob request(s) as Printed?')) return;                     for (const item of memberBatch) {                       try {                         await fetch('/api/update-card-request', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ recordId: item.req.airtableId, status: 'Printed' }) });                       } catch (err) { console.error('Failed to mark printed:', err); }                     }                     setCardQueue(prev => prev.map(c => memberBatch.some(x => x.req.airtableId === c.airtableId) ? Object.assign({}, c, { status: 'Printed', printedDate: new Date().toISOString().split('T')[0] }) : c));                     showToast('Marked ' + memberBatch.length + ' as printed.', 'success', 3000);                   }} className="bg-[#003d6b] text-white px-4 py-2 rounded-xl text-sm font-bold shadow-sm flex items-center gap-2 hover:bg-[#001f3f] transition-colors"><Printer size={14}/> Print 3-Up Keytags</button><div class="back-header">If Found, Please Return</div><div class="addresses"><div class="back-address"><strong>Anthony Wellness</strong>309 W Main St<br/>Anthony, KS 67003<br/>(620) 842-5190</div><div class="back-address"><strong>Harper Wellness</strong>615 W 12th St<br/>Harper, KS 67058<br/>(620) 896-1202</div></div><div class="back-footer">This card is the property of Patterson Health Center. It is non-transferable and intended for the exclusive use of the member named on the reverse side. If found, please return to one of the addresses above. Thank you!</div></div>'; } const html = '<!DOCTYPE html><html><head><title>Card Backs Batch (' + count + ')</title><link href="https://fonts.googleapis.com/css2?family=Montserrat:wght@300;500;700;900&display=swap" rel="stylesheet"><style>@page{size:3.375in 2.125in;margin:0}@media print{*{-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}.card-page{page-break-after:always}}body{font-family:Montserrat,Arial,sans-serif;margin:0;padding:0;background:#fff}.card-page{width:3.375in;height:2.125in;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:0.2in;text-align:center;background:#fff;box-sizing:border-box}.back-header{font-size:11px;font-weight:900;color:#003d6b;text-transform:uppercase;letter-spacing:2px;margin-bottom:0.15in}.addresses{display:flex;gap:0.25in;justify-content:center;width:100%}.back-address{font-size:9px;color:#475569;line-height:1.5;text-align:center}.back-address strong{display:block;color:#003d6b;font-weight:900;font-size:10px;margin-bottom:3px;text-transform:uppercase;letter-spacing:0.5px}.back-footer{font-size:7px;color:#94a3b8;text-align:center;line-height:1.4;margin-top:0.18in;padding:0 0.1in}</style></head><body>' + pages + '</body></html>'; const w = window.open('', '_blank'); w.document.write(html); w.document.close(); setTimeout(() => w.print(), 800); }} className="bg-[#dba51f] text-white px-4 py-2 rounded-xl text-sm font-bold shadow-sm flex items-center gap-2 hover:bg-amber-600 transition-colors"><Printer size={14}/> Print Backs Batch</button><div className="bg-white px-4 py-2 border border-slate-200 rounded-xl text-sm font-bold text-slate-500 shadow-sm flex items-center gap-2"><Bell size={14} className="text-[#dba51f]"/> {pendingRequests.length} pending</div></div>               </div>                              {/* PENDING REQUESTS - SPLIT BY TYPE */}
              {(() => {
                ...
                const handlePrint3UpFob = () => {
                  const fobs = fobRequests.slice(0, 3);
                  if (fobs.length < 3) { alert('Need at least 3 pending fob requests. Currently have ' + fobs.length + '.'); return; }
                  const fobMembers = fobs.map(req => members.find(mem => mem.airtableId === req.memberRecId)).filter(Boolean);
                  if (fobMembers.length < 3) { alert('Could not load all 3 members. One may have been deleted.'); return; }
                  
                  let fobSections = '';
                  fobMembers.forEach(m => {
                    const isHarper = m.center && m.center.toLowerCase().includes('harper');
                    const accentColor = isHarper ? '#dd6d22' : '#1080ad';
                    ...
                    fobSections += '<div class="fob"><div class="fob-bar" style="background:' + accentColor + '"></div><div class="fob-content"><div class="hole"></div><img class="qr" src="' + qrUrl + '" /><div class="member-id">' + m.id + '</div><div class="center-label">' + (isHarper ? 'Harper' : 'Anthony') + '</div></div></div>';
                  });
                  
                  const html = '<!DOCTYPE html><html><head><title>3-Up Fob Card</title><link href="https://fonts.googleapis.com/css2?family=Montserrat:wght@300;500;700;900&display=swap" rel="stylesheet"><style>@page{size:3.375in 2.125in;margin:0}@media print{*{-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}}body{font-family:Montserrat,Arial,sans-serif;margin:0;padding:0;background:#fff}.card{width:3.375in;height:2.125in;display:flex;flex-direction:row;background:#fff;box-sizing:border-box}.fob{width:1.125in;height:2.125in;display:flex;flex-direction:column;position:relative;border-right:1px dashed #ccc}.fob:last-child{border-right:none}.fob-bar{height:0.15in;width:100%}.fob-content{flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:0.05in 0.05in 0.08in 0.05in;position:relative}.hole{position:absolute;top:0.18in;width:0.08in;height:0.08in;border:1.5px solid #999;border-radius:50%;background:#fff}.qr{width:0.75in;height:0.75in;display:block;margin-top:0.12in}.member-id{font-size:9px;font-family:monospace;font-weight:900;color:#003d6b;margin-top:0.06in;letter-spacing:0.5px}.center-label{font-size:6px;font-weight:700;color:#64748b;text-transform:uppercase;letter-spacing:1px;margin-top:0.02in}</style></head><body><div class="card">' + fobSections + '</div></body></html>';
                  const w = window.open('', '_blank');
                  w.document.write(html);
                  w.document.close();
