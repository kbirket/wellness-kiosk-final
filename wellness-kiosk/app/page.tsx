    ...
    <div className="flex min-h-screen bg-[#f0f2f5] font-sans text-slate-800">
      ...
      <main className="flex-1 p-10 h-screen overflow-y-auto relative print:m-0 print:p-0 print:h-auto print:overflow-visible">
        ...
        {activeTab === 'reports' && (() => {
          ...
          const paidMembers = scopedMembers.filter(m => m.status === 'ACTIVE' && !m.type.includes('HD6') && !m.type.includes('MILITARY') && m.type !== 'FIRST DAY FREE');
          const revenueByPlan = {};
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
                  ...
                  {revChartData.length > 0 ? (
                    <div className="space-y-4">
                      <DonutChart data={revChartData} totalLabel="Actual" />
                    </div>
                  ...
                  <div className="space-y-6">
                    <div>
                      <div className="flex justify-between items-end mb-2">
                        <div><p className="text-xs font-bold text-slate-400 uppercase tracking-widest">Actual Monthly Revenue</p><p className="text-3xl font-black text-[#001f3f]">${Math.round(expectedRevenue).toLocaleString()}</p></div>
                      </div>
                    </div>
