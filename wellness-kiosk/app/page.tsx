            ...
            <div className="space-y-6">
             ...
             {Object.keys(savedClassRosters).length > 0 && (
                <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200 mt-6">
                  <h3 className="text-lg font-bold text-[#001f3f] mb-2 flex items-center gap-2"><FileText size={18} className="text-[#f59e0b]"/> Historical Class Rosters</h3>
                  <div className="bg-blue-50 border border-blue-100 rounded-xl p-3 mb-4 flex items-start gap-2"><AlertCircle size={14} className="text-blue-500 shrink-0 mt-0.5"/><p className="text-[11px] text-blue-700 font-medium">These are older class rosters from before class attendance was unified with regular check-ins. New class attendance now appears in member visit history and counts toward total visits. This section is read-only and kept for reference.</p></div>
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-3">
              ...
              {corporatePartners.filter(c => c.name.toLowerCase().includes(corpSearch.toLowerCase())).map(corp => {
                 ...
                 const isPaid = isInternal || !!paidMatch;
                   ...
                   <div key={corp.id} className={`bg-white rounded-2xl shadow-sm border transition-all ${isPaid ? 'border-green-300' : 'border-slate-200'}`}>
                     ...
                     <button onClick={() => setExpandedCorpId(expandedCorpId === corp.id ? null : corp.id)} className="w-full p-5 flex items-center justify-between text-left hover:bg-slate-50/50 transition-colors rounded-2xl">
                       ...
                       <div className="flex items-center gap-3 shrink-0">
                         <span className={`px-3 py-1 rounded-full text-[10px] font-black tracking-widest ${isInternal ? 'bg-slate-100 text-slate-600' : isPaid ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>{isInternal ? 'INTERNAL · NO CHARGE' : isPaid ? `PAID${corpPayMethod ? ` · ${corpPayMethod.toUpperCase()}` : ''}` : 'UNPAID'}</span>
                       ...
                       <div className="px-6 pb-6 border-t border-slate-100">
                         <div className="pt-5 space-y-4">
                           ...
                           <div className="flex flex-col gap-2 pt-2">
                             ...
                             {!isInternal && (<button onClick={togglePayment} className={`w-full py-3 rounded-xl text-sm font-bold shadow-sm transition-colors border ${isPaid ? 'bg-white border-slate-200 text-slate-500 hover:bg-slate-50' : 'bg-blue-50 border-blue-200 text-blue-700 hover:bg-blue-100'}`}>{isPaid ? 'Mark as Unpaid' : 'Mark as Paid'}</button>)}
                           </div>
                         </div>
                       </div>
                     )}
