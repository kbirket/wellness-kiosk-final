...
const handleAddMemberSubmit = async (e) => {
 ...
 const handleLogVisitorVisit = async (v) => {
    ...
    const visitISO = new Date(iso + 'T12:00:00').toISOString();
    ...
    try {
      ...
      setVisits(prev => { const updated = [{ name: v.firstName + ' ' + v.lastName, center: centerLabel, time: visitISO, type: 'VISITOR - ' + (v.passType || 'Pass'), method: iso === todayISO ? 'Staff Entry' : 'Staff Entry - Backdated' }].concat(prev); updated.sort((a, b) => new Date(b.time) - new Date(a.time)); return updated; });
      showToast('Visit logged for ' + v.firstName + ' on ' + formatMDY(iso) + '.' + (counts ? ' ' + newRemaining + ' left.' : ''), 'success', 4000);
