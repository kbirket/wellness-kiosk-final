...
const handleAddMemberSubmit = async (e) => {
    ...
    if (isFamily && !familyFlow) {
      try {
        const res = await fetch('/api/add-family-member', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ firstName, lastName, email, phone, plan, center, corporateSponsor: sponsor, needsOrientation, basicOrientation: !needsOrientation, basicOrientationDate: !needsOrientation ? new Date().toISOString().split('T')[0] : '', paperworkCompleted: !needsOrientation, paperworkCompletedDate: !needsOrientation ? new Date().toISOString().split('T')[0] : '', orientationHarper: !needsOrientation && center.toLowerCase().includes('harper'), orientationAnthony: !needsOrientation && center.toLowerCase().includes('anthony'), startDate, address, city, state: mstate, zip: mzip, billingMethod: billing, access247, badgeNumber, discountCode, discountExpiration, monthlyRate: finalMonthlyRate }) });
        const result = await res.json();
      ...
      try {
        const res = await fetch('/api/add-member', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ firstName, lastName, email, phone, plan, center, corporateSponsor: sponsor, needsOrientation, basicOrientation: !needsOrientation, basicOrientationDate: !needsOrientation ? new Date().toISOString().split('T')[0] : '', paperworkCompleted: !needsOrientation, paperworkCompletedDate: !needsOrientation ? new Date().toISOString().split('T')[0] : '', orientationHarper: !needsOrientation && center.toLowerCase().includes('harper'), orientationAnthony: !needsOrientation && center.toLowerCase().includes('anthony'), address, city, state: mstate, zip: mzip, billingMethod: billing, access247, badgeNumber, startDate, discountCode, discountExpiration, monthlyRate: finalMonthlyRate }) });
        const result = await res.json();
        if (result.success) {
