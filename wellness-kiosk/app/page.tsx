...
if (['MILITARY', 'MILITARY FAMILY', 'HD6', 'HD6 FAMILY', 'FIRST DAY FREE', 'LIFETIME', 'LIFETIME FAMILY'].includes(p)) return 0;    return r[p] ? (r[p][b] || r[p]['Month-to-Month']) : 0;
  ...
  const duplicateBadges = (() => {
    const badgeMap = {};
    members.forEach(m => {
      if (m.badgeNumber && m.badgeNumber.trim() !== '') {
        const badge = m.badgeNumber.trim();
        if (!badgeMap[badge]) badgeMap[badge] = [];
        badgeMap[badge].push({ id: m.id, name: `${m.firstName} ${m.lastName}`, center: m.center });
      }
    });
    const dupes = {};
    Object.entries(badgeMap).forEach(([badge, list]) => {
      if (list.length > 1) dupes[badge] = list;
    });
    return dupes;
  })();

  // --- INACTIVE MEMBER FLAGS (for rollout monitoring) ---
  const inactiveWithVisits = members.filter(m => m.status !== 'ACTIVE' && m.visits > 0);
...
const handleAddMemberSubmit = async (e) => {
    ...
    if (badgeNumber) {
      const existing = members.find(m => m.badgeNumber && m.badgeNumber.trim() === badgeNumber.trim());
      if (existing && !window.confirm(`⚠️ Badge #${badgeNumber} is already assigned to ${existing.firstName} ${existing.lastName}. Continue anyway?`)) { setIsAdding(false); return; }
    }
    ...
    <div className="flex min-h-screen bg-[#f0f2f5] font-sans text-slate-800">
      ...
      <main className="flex-1 p-10 h-screen overflow-y-auto relative print:m-0 print:p-0 print:h-auto print:overflow-visible">
        ...
        {activeTab === 'dashboard' && (() => {
          ...
          const duplicateBadgeItems = Object.entries(duplicateBadges).flatMap(([badge, list]) => list.map(m => ({name: m.name, detail: `Badge #${badge} shared with ${list.filter(x => x.id !== m.id).map(x => x.name).join(', ')}`, type: 'badge', id: m.id})));           const briefingItems = [...duplicateBadgeItems,...overdueMembers.map(m => ({name:`${m.firstName} ${m.lastName}`,detail:`Overdue since ${m.nextPayment}`,type:'overdue',id:m.id})),...dueTodayMembers.map(m => ({name:`${m.firstName} ${m.lastName}`,detail:'Payment due today',type:'due',id:m.id})),...orientationMembers.map(m => ({name:`${m.firstName} ${m.lastName}`,detail:'Needs facility orientation',type:'orientation',id:m.id})),...expiringThisWeek.map(m => ({name:`${m.firstName} ${m.lastName}`,detail:`Expires ${m.nextPayment}`,type:'expiring',id:m.id}))];
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#001f3f]/90 backdrop-blur-md">
           <div className="bg-white rounded-[2rem] w-full max-w-4xl flex overflow-hidden shadow-2xl relative">
              ...
              <div className="flex-1 p-16 overflow-y-auto">
                  {selectedMember.badgeNumber && duplicateBadges[selectedMember.badgeNumber.trim()] && (<div className="mb-6 bg-rose-50 border-l-4 border-rose-500 p-4 rounded-r-lg"><p className="text-rose-800 font-bold text-sm">⚠️ Duplicate Badge #{selectedMember.badgeNumber} — also assigned to: {duplicateBadges[selectedMember.badgeNumber.trim()].filter(x => x.id !== selectedMember.id).map(x => x.name).join(', ')}</p></div>)}
                 {selectedMember.status !== 'ACTIVE' && selectedMember.status !== 'EXPIRING' && (<div className="mb-6 bg-amber-50 border-l-4 border-amber-500 p-4 rounded-r-lg"><p className="text-amber-800 font-bold text-sm">⚠️ This member is currently {selectedMember.status} — not an active membership.</p></div>)}
