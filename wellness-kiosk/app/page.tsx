...
const THERAPY_DEPTS = ['Physical Therapy', 'Occupational Therapy', 'Cardiopulmonary Therapy', 'Speech Therapy'];
const REFERRING_PROVIDERS = ['Dr. Kibar', 'Dr. Leavitt', 'Karlie Parker', 'Megan Ricke', 'Madi Koster', 'Mattie Passet', 'Renee Pence', 'Elizabeth Caudillo', 'Leah Gerdes (Hospitalist)'];
// Turns the specific name into a countable category for reporting.
const passSourceFor = (name) => {
  if (!name) return 'Paid at Desk';
  if (name === 'Staff Comp') return 'Staff Comp';
  if (name === 'Former Member') return 'Former Member';
  if (THERAPY_DEPTS.indexOf(name) !== -1) return 'Therapy';
  if (REFERRING_PROVIDERS.indexOf(name) !== -1) return 'Provider';
  return 'Other';
};
const PassSourceSelect = ({ id, defaultValue, className }) => {
  const known = ['', 'Staff Comp'].concat(THERAPY_DEPTS).concat(REFERRING_PROVIDERS);
  const legacy = defaultValue && known.indexOf(defaultValue) === -1 ? defaultValue : null;
  return (
    <select id={id} defaultValue={defaultValue || ''} className={className}>
      <option value="">Paid at desk (no referral)</option>
      <option value="Staff Comp">Staff comp</option>
      {legacy && <option value={legacy}>{legacy} (existing)</option>}
      <optgroup label="Therapy Department">
        {THERAPY_DEPTS.map(d => <option key={d} value={d}>{d}</option>)}
      </optgroup>
      <optgroup label="Referring Provider">
        {REFERRING_PROVIDERS.map(pv => <option key={pv} value={pv}>{pv}</option>)}
      </optgroup>
    </select>
  );
};
...
export default function WellnessHub() {
  ...
  const [paymentModal, setPaymentModal] = useState(null);   const [checkNumber, setCheckNumber] = useState(''); const [proratePayment, setProratePayment] = useState(false); const [customPayAmount, setCustomPayAmount] = useState(''); const [paymentNote, setPaymentNote] = useState('');
...
const [searchQuery, setSearchQuery] = useState('');       const [memberFilter, setMemberFilter] = useState('all');     const [recentCutoffDate, setRecentCutoffDate] = useState(() => { var d = new Date(); d.setDate(d.getDate() - 30); return d.toISOString().split('T')[0]; });
  ...
  const [existingFamSearch, setExistingFamSearch] = useState('');
...
const [visitorDupMatch, setVisitorDupMatch] = useState(null);
  const [cardRequestType, setCardRequestType] = useState('Fob');
...
const [fobCenterFilter, setFobCenterFilter] = useState('all');  
      ...
      if (result.success) {
        setMembers(prev => prev.map(m => {
          ...
          if ('Turns 18 Date' in fields) updated.turns18Date = fields['Turns 18 Date'] || '';
                if ('Parent Permission Form' in fields) updated.parentPermission = fields['Parent Permission Form'];
                if ('May Attend Alone' in fields) updated.mayAttendAlone = fields['May Attend Alone'];
                if ('Parent Permission Form Date' in fields) updated.parentPermissionDate = fields['Parent Permission Form Date'] || '';
                if ('Auto-Pay ACH' in fields) updated.achAutoPay = fields['Auto-Pay ACH'];
                if ('ACH Draft Day' in fields) updated.achDraftDay = fields['ACH Draft Day'];
                               if ('Corporate ACH' in fields) updated.corpAchPay = fields['Corporate ACH'];
                if ('Family Primary' in fields) updated.familyPrimary = fields['Family Primary'];
          return updated;
        }));
...
useEffect(() => { localStorage.setItem('wellnessUsagePrefs', JSON.stringify(usageBasedCorps)); }, [usageBasedCorps]);
  useEffect(() => { localStorage.setItem('wellnessDismissedDupes', JSON.stringify(dismissedDuplicatePairs)); }, [dismissedDuplicatePairs]);
...
const getBaseRate = (p, b) => {
  ...
  const inFamily = (m) => !!(m && (m.familyGroupId || m.familyName));
  const isFamilyPrimary = (m) => !inFamily(m) || !!m.familyPrimary;
  const familyOf = (m) => !m ? [] : members.filter(x => (m.familyGroupId && x.familyGroupId === m.familyGroupId) || (m.familyName && x.familyName && x.familyName === m.familyName));
  const setFamilyPrimary = async (member) => {
    const fam = familyOf(member);
    const rate = getBaseRate(member.type, member.billingMethod || 'Month-to-Month');
    for (const fm of fam) {
    ...
    setMembers(prev => prev.map(x => fam.some(f => f.airtableId === x.airtableId) ? Object.assign({}, x, { familyPrimary: x.airtableId === member.airtableId, monthlyRate: x.airtableId === member.airtableId ? rate : 0, billingMethod: x.airtableId === member.airtableId ? (member.billingMethod || 'Month-to-Month') : '' }) : x));
    return rate;
  };
...
const handleAddMemberSubmit = async (e) => {
    ...
    const orientDone = e.target.orientDone?.checked || false;
    const needsOrientation = !orientDone;
    const basicOrient = orientDone;
    const paperworkDone = e.target.paperwork?.checked || false;
    ...
    if (isFamily && !familyFlow) {
      try {
        const res = await fetch('/api/add-family-member', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ firstName, lastName, email, phone, plan, center, corporateSponsor: sponsor, needsOrientation, basicOrientation: !needsOrientation || basicOrient, basicOrientationDate: (!needsOrientation || basicOrient) ? new Date().toISOString().split('T')[0] : '', paperworkCompleted: !needsOrientation || paperworkDone, paperworkCompletedDate: (!needsOrientation || paperworkDone) ? new Date().toISOString().split('T')[0] : '', orientationHarper: !needsOrientation && center.toLowerCase().includes('harper'), orientationAnthony: !needsOrientation && center.toLowerCase().includes('anthony'), startDate, address, city, state: mstate, zip: mzip, billingMethod: billing, access247, badgeNumber, discountCode, discountExpiration, monthlyRate: finalMonthlyRate }) });
        const result = await res.json();
        if (result.success) { setFamilyFlow({ familyRecordId: result.familyRecordId, familyName: result.familyName, lastName, plan, center, email, phone, corporateSponsor: sponsor, addedMembers: [{ name: `${firstName} ${lastName}`, pin: result.pin, isPrimary: true }] }); setNewMemberPin({ name: `${firstName} ${lastName}`, pin: result.pin }); if (requestCardOnAdd && result.memberAirtableId) { try { const cardRes = await fetch('/api/request-card', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ memberAirtableId: result.memberAirtableId, requestedBy: user ? user.name : 'Unknown', note: requestCardNoteOnAdd || 'New family member sign-up' }) }); const cardData = await cardRes.json().catch(function(){ return {}; }); if (cardRes.ok && cardData.success !== false) { showToast('Card request added to Kristen\'s queue.', 'success', 4000); fetch('/api/get-card-queue').then(res => res.json()).then(data => { if (data.records) { setCardQueue(data.records.map(function(r) { var memberLink = r.fields['Member'] || []; var memberRecId = Array.isArray(memberLink) ? memberLink[0] || '' : memberLink; return { airtableId: r.id, memberRecId: String(memberRecId).trim(), requestedBy: r.fields['Requested By'] || '', requestedDate: r.fields['Requested Date'] || '', note: r.fields['Note'] || '', status: r.fields['Status'] || 'Pending', printedDate: r.fields['Printed Date'] || '', issuedDate: r.fields['Issued Date'] || '', printType: r.fields['Print Type'] || 'Card', statusNote: r.fields['Status Note'] || '' }; })); } }); } else { showToast('Card request FAILED: ' + (cardData.error || 'Airtable rejected it') + '. Try from their profile.', 'warning', 8000); } } catch (cardErr) { showToast('Member created, but card request failed (network). Try from their profile.', 'warning', 5000); } setRequestCardOnAdd(false); setRequestCardNoteOnAdd(''); } } else { alert('Error: ' + result.error); }
      } catch (err) { alert('Network error. Please try again.'); }
    } else if (isFamily && familyFlow) {
      try {
        // FIX: We are now sending `lastName` instead of forcing `familyFlow.lastName`
        const res = await fetch('/api/add-family-member', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ firstName, lastName, email: email || familyFlow.email, phone: phone || familyFlow.phone, plan: familyFlow.plan, center: familyFlow.center, familyRecordId: familyFlow.familyRecordId, corporateSponsor: familyFlow.corporateSponsor, needsOrientation, basicOrientation: !needsOrientation || basicOrient, basicOrientationDate: (!needsOrientation || basicOrient) ? new Date().toISOString().split('T')[0] : '', paperworkCompleted: !needsOrientation || paperworkDone, paperworkCompletedDate: (!needsOrientation || paperworkDone) ? new Date().toISOString().split('T')[0] : '', orientationHarper: !needsOrientation && familyFlow.center.toLowerCase().includes('harper'), orientationAnthony: !needsOrientation && familyFlow.center.toLowerCase().includes('anthony'), address, city, state: mstate, zip: mzip, access247, badgeNumber, discountCode, discountExpiration, monthlyRate: 0 }) });
        const result = await res.json();
        // FIX: Also updated the success state to use `lastName` instead of `familyFlow.lastName`
        if (result.success) { const updated = { ...familyFlow, addedMembers: [...familyFlow.addedMembers, { name: `${firstName} ${lastName}`, pin: result.pin, isPrimary: false }] }; setFamilyFlow(updated); setNewMemberPin({ name: `${firstName} ${lastName}`, pin: result.pin }); if (requestCardOnAdd && result.memberAirtableId) { try { const cardRes = await fetch('/api/request-card', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ memberAirtableId: result.memberAirtableId, requestedBy: user ? user.name : 'Unknown', note: requestCardNoteOnAdd || 'New family member sign-up' }) }); const cardData = await cardRes.json().catch(function(){ return {}; }); if (cardRes.ok && cardData.success !== false) { showToast('Card request added to Kristen\'s queue.', 'success', 4000); fetch('/api/get-card-queue').then(res => res.json()).then(data => { if (data.records) { setCardQueue(data.records.map(function(r) { var memberLink = r.fields['Member'] || []; var memberRecId = Array.isArray(memberLink) ? memberLink[0] || '' : memberLink; return { airtableId: r.id, memberRecId: String(memberRecId).trim(), requestedBy: r.fields['Requested By'] || '', requestedDate: r.fields['Requested Date'] || '', note: r.fields['Note'] || '', status: r.fields['Status'] || 'Pending', printedDate: r.fields['Printed Date'] || '', issuedDate: r.fields['Issued Date'] || '', printType: r.fields['Print Type'] || 'Card', statusNote: r.fields['Status Note'] || '' }; })); } }); } else { showToast('Card request FAILED: ' + (cardData.error || 'Airtable rejected it') + '. Try from their profile.', 'warning', 8000); } } catch (cardErr) { showToast('Member created, but card request failed (network). Try from their profile.', 'warning', 5000); } setRequestCardOnAdd(false); setRequestCardNoteOnAdd(''); } } else { alert('Error: ' + result.error); }
      } catch (err) { alert('Network error. Please try again.'); }
    } else {
      try {
        const res = await fetch('/api/add-member', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ firstName, lastName, email, phone, plan, center, corporateSponsor: sponsor, needsOrientation, basicOrientation: !needsOrientation || basicOrient, basicOrientationDate: (!needsOrientation || basicOrient) ? new Date().toISOString().split('T')[0] : '', paperworkCompleted: !needsOrientation || paperworkDone, paperworkCompletedDate: (!needsOrientation || paperworkDone) ? new Date().toISOString().split('T')[0] : '', orientationHarper: !needsOrientation && center.toLowerCase().includes('harper'), orientationAnthony: !needsOrientation && center.toLowerCase().includes('anthony'), address, city, state: mstate, zip: mzip, billingMethod: billing, access247, badgeNumber, startDate, discountCode, discountExpiration, monthlyRate: finalMonthlyRate }) });
        const result = await res.json();
        if (result.success) {
 ...
 // Core logger - shared by the Visitors tab button and Staff Check-In.
 const logVisitorVisitAt = async (v, visitISO, quiet) => {
    const counts = v.passesRemaining !== null && v.passesRemaining !== undefined;
    if (counts && v.passesRemaining <= 0) { alert(v.firstName + ' has no passes left. Renew their pass first.'); return false; }
    const centerLabel = v.center || (viewingCenter === 'harper' ? 'Harper Wellness Center' : 'Anthony Wellness Center');
    const todayStr = new Date().toDateString();
    const isBack = new Date(visitISO).toDateString() !== todayStr;
    const dupe = visits.some(x => x.name === (v.firstName + ' ' + v.lastName) && Math.abs(new Date(x.time) - new Date(visitISO)) < 30 * 60 * 1000);
    if (dupe && !window.confirm(v.firstName + ' already has a visit logged near that time.\n\nLog another one anyway?')) return false;
    try {
      const res = await fetch('/api/visitor-checkin', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ visitorAirtableId: v.airtableId, center: centerLabel, decrementPass: counts, visitDate: visitISO }) });
      const result = await res.json();
      if (!result.success) { alert('Could not log visit: ' + (result.error || 'unknown error')); return false; }
      const newRemaining = (result.passesRemaining !== undefined && result.passesRemaining !== null) ? result.passesRemaining : (counts ? v.passesRemaining - 1 : null);
      setVisitors(prev => prev.map(x => x.airtableId === v.airtableId ? Object.assign({}, x, { totalVisits: (x.totalVisits || 0) + 1, passesRemaining: newRemaining }) : x));
      setVisits(prev => { const updated = [{ name: v.firstName + ' ' + v.lastName + ' (Visitor)', center: centerLabel, time: visitISO, type: 'VISITOR - ' + (v.passType || 'Pass'), method: isBack ? 'Staff Entry - Backdated' : 'Staff Entry' }].concat(prev); updated.sort((a, b) => new Date(b.time) - new Date(a.time)); return updated; });
      if (!quiet) showToast('Visit logged for ' + v.firstName + ' on ' + new Date(visitISO).toLocaleDateString() + '.' + (counts ? ' ' + newRemaining + ' left.' : ''), 'success', 4000);
      return true;
    } catch (e) { alert('Network error: ' + e.message); return false; }
  };
 const handleLogVisitorVisit = async (v) => {
    const counts = v.passesRemaining !== null && v.passesRemaining !== undefined;
    if (counts && v.passesRemaining <= 0) { alert(v.firstName + ' has no passes left. Renew their pass first.'); return; }
    const todayISO = new Date().toLocaleDateString('en-CA');
    const passLine = counts ? ' This uses one pass (' + v.passesRemaining + ' left, will be ' + (v.passesRemaining - 1) + ').' : '';
    const entered = window.prompt('Log a visit for ' + v.firstName + ' ' + v.lastName + '.' + passLine + '\n\nDate of visit (MM/DD/YYYY) - leave as today unless you are catching up:', formatMDY(todayISO));
    if (entered === null) return;
    const iso = parseMDY(entered.trim());
    if (!iso) { alert('Please enter the date as MM/DD/YYYY, for example 07/15/2026.'); return; }
    if (iso > todayISO) { alert('That date is in the future. Please enter today or an earlier date.'); return; }
    await logVisitorVisitAt(v, new Date(iso + 'T12:00:00').toISOString());
  };
 ...
 const handleRenewVisitor = async (visitor, newPassType, passCount) => {
  ...
  const handleConvertToVisitor = async (member) => {
    ...
    const handleSetPin = async (member, newPin) => {
      if (!/^[0-9]{4}$/.test(newPin)) { alert('PIN must be exactly 4 digits.'); return; }
      if (newPin === '0000' || newPin === '1111') { alert('Please avoid 0000 or 1111 - they are too easy to guess.'); return; }
      try {
        const res = await fetch('/api/update-pin', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ recordId: member.airtableId, newPin }) });
        const result = await res.json().catch(() => ({}));
        if (res.ok && result.success !== false) {
          setMembers(prev => prev.map(m => m.airtableId === member.airtableId ? { ...m, password: newPin } : m));
          setSelectedMember(prev => prev && prev.airtableId === member.airtableId ? { ...prev, password: newPin } : prev);
          showToast(member.firstName + "'s PIN is now " + newPin + '. Please give it to them.', 'success', 8000);
        } else { alert('Could not update PIN: ' + (result.error || 'unknown error')); }
      } catch (err) { alert('Network error - could not update PIN.'); }
    };
    const handleResetPin = (member) => {
      const entered = window.prompt('Set a new 4-digit PIN for ' + member.firstName + ' ' + member.lastName + '.\n\nType the 4 digits you want, or leave blank to generate a random one:', '');
      if (entered === null) return;
      const pin = entered.trim() === '' ? String(Math.floor(1000 + Math.random() * 9000)) : entered.trim();
      handleSetPin(member, pin);
    };
  useEffect(() => {
    if (kioskLang !== 'es') return;
    const t = setTimeout(() => setKioskLang('en'), 90000);
  ...
  useEffect(() => {
    var memberRefreshTimer = setInterval(function() {
      fetch('/api/members').then(function(r) { return r.json(); }).then(function(data) {
        if (data.records) {
          var mapped = data.records.filter(function(r) { return r.fields['First Name'] && r.fields['First Name'] !== ''; }).map(function(r) {
            var planText = r.fields['Plan Name'] ? (Array.isArray(r.fields['Plan Name']) ? r.fields['Plan Name'][0] : r.fields['Plan Name']) : 'UNKNOWN PLAN';
            var rawPassword = String(r.fields['Password'] || '').trim();
            var finalPassword = (rawPassword === '' || rawPassword.includes('ERROR')) ? '1111' : rawPassword;
            return { airtableId: r.id, id: r.fields['Member ID'] || r.id, firstName: r.fields['First Name'] || 'Unknown', lastName: r.fields['Last Name'] || '', email: r.fields['Email'] || '', phone: r.fields['Phone'] || '', password: finalPassword, status: (r.fields['Membership Status'] || 'ACTIVE').toUpperCase(), type: String(planText).toUpperCase().trim(), center: r.fields['Home Center'] || 'Anthony', visits: Number(r.fields['Total Visits'] || 0), nextPayment: r.fields['Next Payment Due'] || null, sponsor: !!r.fields['Corporate Sponsor'], sponsorName: r.fields['Corporate Sponsor'] ? String(r.fields['Corporate Sponsor']).trim() : '', needsOrientation: !!r.fields['Needs Orientation'], orientationAnthony: r.fields['Orientation Anthony'] !== undefined ? !!r.fields['Orientation Anthony'] : !r.fields['Needs Orientation'], orientationHarper: r.fields['Orientation Harper'] !== undefined ? !!r.fields['Orientation Harper'] : !r.fields['Needs Orientation'], familyName: r.fields['Family Name'] ? (Array.isArray(r.fields['Family Name']) ? r.fields['Family Name'][0] : r.fields['Family Name']) : '', familyGroupId: r.fields['Family Group'] ? (Array.isArray(r.fields['Family Group']) ? r.fields['Family Group'][0] : r.fields['Family Group']) : '', billingMethod: r.fields['Billing Method'] || '', familyPrimary: !!r.fields['Family Primary'], monthlyRate: r.fields['Monthly Rate'] || '', access247: !!r.fields['24/7 Access'], badgeNumber: r.fields['Badge Number'] || '', startDate: r.fields['Start Date'] || null, notes: r.fields['Notes'] || '', discountCode: r.fields['Discount Code'] || '', discountExpiration: r.fields['Discount Expiration'] || null, address: r.fields['Street Address'] || '', city: r.fields['City'] || '', state: r.fields['State'] || 'KS', zip: r.fields['Zip'] || '', paymentMethod: Array.isArray(r.fields['Payment Method']) ? r.fields['Payment Method'][r.fields['Payment Method'].length - 1] : (r.fields['Payment Method'] || ''), checkNumber: r.fields['Check Number'] || '', inactive: !!r.fields['Inactive'], merged: !!r.fields['Merged'], legacyMemberIds: r.fields['Legacy Member IDs'] || '', isMinor: !!r.fields['Is Minor'], turns18Date: r.fields['Turns 18 Date'] || '', parentPermission: !!r.fields['Parent Permission Form'], parentPermissionDate: r.fields['Parent Permission Form Date'] || '', mayAttendAlone: !!r.fields['May Attend Alone'], achAutoPay: !!r.fields['Auto-Pay ACH'], corpAchPay: !!r.fields['Corporate ACH'], achDraftDay: r.fields['ACH Draft Day'] || '3rd', basicOrientation: !!r.fields['Basic Orientation'], basicOrientationDate: r.fields['Basic Orientation Date'] || '', paperworkCompleted: !!r.fields['Paperwork Completed'], paperworkCompletedDate: r.fields['Paperwork Completed Date'] || '', accessOrientation24: !!r.fields['24/7 Access Orientation'], accessOrientationDate24: r.fields['24/7 Access Orientation Date'] || '', firstDayFree: !!r.fields['First Day Free'], firstDayFreeDate: r.fields['First Day Free Date'] || '', onboardingNotes: r.fields['Onboarding Notes'] || '', photoUrl: r.fields['Photo URL'] || (r.fields['Photo'] && r.fields['Photo'].length > 0 ? r.fields['Photo'][0].url : '') };
          }).filter(function(m) { return !m.merged; });
          setMembers(mapped);
        }
      }).catch(function() {});
    }, 300000);
    return function() { clearInterval(memberRefreshTimer); };
  }, []);
  const blockedCountRef = useRef(0);
  useEffect(() => {
    if (!user) return;
    blockedCountRef.current = blockedCheckins.filter(function(b) { return b.outcome === 'Pending' && b.reason !== 'Cooldown (2-hr)'; }).length;
    var blockedTimer = setInterval(function() {
      fetch('/api/get-blocked-checkins').then(function(r) { return r.json(); }).then(function(data) {
        if (data.records) {
          var mapped = data.records.map(function(r) { var memberLink = r.fields['Member'] || []; var memberRecId = Array.isArray(memberLink) ? memberLink[0] || '' : memberLink; return { airtableId: r.id, memberRecId: String(memberRecId).trim(), memberId: r.fields['Member ID'] || '', memberName: r.fields['Member Name'] || 'Unknown', timestamp: r.fields['Timestamp'] || '', reason: r.fields['Reason'] || '', center: r.fields['Center'] || '', outcome: r.fields['Outcome'] || 'Pending', notes: r.fields['Notes'] || '' }; });
          var newPendingCount = mapped.filter(function(b) { return b.outcome === 'Pending' && b.reason !== 'Cooldown (2-hr)'; }).length;
          if (newPendingCount > blockedCountRef.current) {
            try {
              ...
              setTimeout(function() { var osc2 = ctx.createOscillator(); var gain2 = ctx.createGain(); osc2.connect(gain2); gain2.connect(ctx.destination); osc2.frequency.value = 1100; osc2.type = 'sine'; gain2.gain.value = 0.3; osc2.start(); gain2.gain.exponentialRampToValueAtTime(0.01, ctx.currentTime + 0.4); osc2.stop(ctx.currentTime + 0.4); }, 300);
            } catch (e) {}
            var newest = mapped.filter(function(b) { return b.outcome === 'Pending' && b.reason !== 'Cooldown (2-hr)'; }).sort(function(a, b) { return new Date(b.timestamp) - new Date(a.timestamp); })[0];
            if (newest) showToast('Blocked at kiosk: ' + newest.memberName + ' — ' + newest.reason, 'error', 8000);
          }
          blockedCountRef.current = newPendingCount;
          setBlockedCheckins(mapped);
        }
      }).catch(function() {});
    }, 60000);
    return function() { clearInterval(blockedTimer); };
  }, [user]);
  const tKiosk = (s) => {
  ...
  const logVisitForBlocked = async (mem, blockedRecordId, memberName, blockedAt) => {
    if (!mem) { alert('Member record not found. Open their profile and check them in from the Staff Check-In tab.'); return; }
    // Use the time they were actually flagged at the kiosk, not the time of this click.
    let when = null;
    if (blockedAt) {
      const bs = String(blockedAt);
      const parsed = (bs.length === 10 && bs.indexOf('T') === -1) ? new Date(bs + 'T00:00:00') : new Date(bs);
      if (!isNaN(parsed.getTime())) when = parsed;
    }
    if (!when) when = new Date();
    const visitISO = when.toISOString();
    const whenLabel = when.toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) + ' at ' + when.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
    const dupe = visits.some(v => v.name === (mem.firstName + ' ' + mem.lastName) && Math.abs(new Date(v.time) - when) < 30 * 60 * 1000);
    if (dupe && !window.confirm(memberName + ' already has a visit logged near ' + whenLabel + '.\n\nLog another one anyway?')) return;
    if (!dupe && !window.confirm('Log a visit for ' + memberName + ' on ' + whenLabel + '?\n\nThat is when they were flagged at the kiosk.')) return;
    try {
      const label = 'Director Override (Flagged but Allowed)';
      const res = await fetch('/api/visits', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ airtableId: mem.airtableId, center: mem.center, time: visitISO, method: label }) });
      const result = await res.json();
      if (!result.success) { alert('Could not log visit: ' + (result.error || 'unknown error')); return; }
      setVisits(prev => { const updated = [{ name: mem.firstName + ' ' + mem.lastName, center: mem.center, time: visitISO, type: mem.type, method: label }].concat(prev); updated.sort((a, b) => new Date(b.time) - new Date(a.time)); return updated; });
      setMembers(prev => prev.map(x => x.airtableId === mem.airtableId ? Object.assign({}, x, { visits: (x.visits || 0) + 1 }) : x));
      if (blockedRecordId) {
        try { await fetch('/api/update-blocked-checkin', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ recordId: blockedRecordId, outcome: 'Dismissed by Staff' }) }); } catch (e) {}
        setBlockedCheckins(prev => prev.map(b => b.airtableId === blockedRecordId ? Object.assign({}, b, { outcome: 'Dismissed by Staff' }) : b));
      }
      showToast('Visit logged for ' + memberName + ' (' + whenLabel + ').', 'success', 4000);
    } catch (e) { alert('Network error: ' + e.message); }
  };
 const getStoplight = (member) => { if (!member || !member.nextPayment) return 'green'; const comped = ['HD6', 'HD6 FAMILY', 'HCHF', 'MILITARY', 'MILITARY FAMILY', 'FIRST DAY FREE', 'LIFETIME', 'LIFETIME FAMILY']; if (comped.includes(member.type)) return 'green'; if (member.inactive) return 'red'; const due = new Date(member.nextPayment + 'T00:00:00'); const today = new Date(); const diffMs = today - due; const daysPastDue = Math.ceil(diffMs / (1000*60*60*24)); const daysUntilDue = Math.ceil((due - today) / (1000*60*60*24)); if (member.achAutoPay || member.corpAchPay) { if (daysPastDue > 7) return 'red'; return 'green'; } if (daysPastDue > 5) return 'red'; if (daysPastDue >= 1) return 'yellow'; return 'green'; };
const filteredMembers = scopedMembers.filter(m => { if (!(m.firstName + ' ' + m.lastName + ' ' + m.id).toLowerCase().includes(searchQuery.toLowerCase())) return false; const today = new Date(); const firstOfNextMonth = new Date(today.getFullYear(), today.getMonth() + 1, 1); const paidThisPeriod = (m.achAutoPay || m.corpAchPay || (m.familyName && scopedMembers.some(fm => fm.familyName === m.familyName && fm.achAutoPay))) ? (!!m.nextPayment && new Date(m.nextPayment + 'T00:00:00') >= new Date(today.getFullYear(), today.getMonth(), today.getDate() - 7)) : (m.nextPayment && new Date(m.nextPayment + 'T00:00:00') >= firstOfNextMonth); const isComped = ['HD6', 'HD6 FAMILY', 'HCHF', 'MILITARY', 'MILITARY FAMILY', 'FIRST DAY FREE', 'LIFETIME', 'LIFETIME FAMILY', 'HERITAGE ESTATES'].includes(m.type); if (memberFilter === 'paid') return paidThisPeriod || isComped; if (memberFilter === 'unpaid') return !paidThisPeriod && !isComped; if (memberFilter === 'overdue') return getStoplight(m) === 'red'; if (memberFilter === '247') return m.access247; if (memberFilter === 'orientation') return m.needsOrientation; if (memberFilter === 'onboarding') return !m.basicOrientation || !m.paperworkCompleted; if (memberFilter === 'inactive') return m.inactive; if (memberFilter === 'corporate') return m.type.includes('CORPORATE') || m.sponsorName; if (memberFilter === 'family') return m.type.includes('FAMILY') || m.familyName; if (memberFilter === 'notes') return m.notes && m.notes.trim() !== ''; if (memberFilter === 'minors') return m.isMinor; if (memberFilter === 'minors-parent') return m.isMinor && !m.mayAttendAlone; if (memberFilter === 'recent') { if (!m.startDate) return false; return new Date(m.startDate + 'T00:00:00') >= new Date(recentCutoffDate + 'T00:00:00'); } return true; }).sort((a, b) => a.lastName.localeCompare(b.lastName) || a.firstName.localeCompare(b.firstName));
...
return exp >= today && !v.convertedToMember && v.orientationComplete && v.passActivated;
    ...
    if (m) {
      ...
      if (needsAnthony || needsHarper) {
        ...
        fetch('/api/log-blocked-checkin', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ memberAirtableId: m.airtableId, memberId: m.id, memberName: m.firstName + ' ' + m.lastName, reason: centerName + ' Orientation', center: centerName }) }).catch(function(){});
        setKioskMessage({ 
      ...
      if (light === 'red') {
        var isCorpMember = m.type.includes('CORPORATE') || m.sponsorName;
        if (!isCorpMember) {
          fetch('/api/log-blocked-checkin', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ memberAirtableId: m.airtableId, memberId: m.id, memberName: m.firstName + ' ' + m.lastName, reason: 'Past Due Payment', center: ((m.center && m.center.toLowerCase().includes('harper')) ? 'Harper' : 'Anthony') }) }).catch(function(){});
...
var onboardingIncomplete = m.needsOrientation && (!m.basicOrientation || !m.paperworkCompleted || (m.isMinor && !m.parentPermission));
      ...
      if (light === 'red') {
        setKioskMessage({ text: `Welcome, ${m.firstName}!`, type: 'warning', subtext: "You're all set! Friendly reminder: your employer's payment is past due. Please remind them to submit it. Thank you!", photoUrl: m.photoUrl });
      } else if (light === 'yellow') {
        setKioskMessage({ text: `Welcome, ${m.firstName}!`, type: 'success', subtext: 'Friendly reminder: your payment is past due. Please stop by the front desk when you get a chance.', photoUrl: m.photoUrl });
      } else if (m.isMinor && !m.mayAttendAlone) {
        setKioskMessage({ text: `Welcome, ${m.firstName}!`, type: 'warning', subtext: 'Parent or guardian must be present.', photoUrl: m.photoUrl });
      } else if (m.nextPayment && !['HD6', 'HD6 FAMILY', 'HCHF', 'MILITARY', 'MILITARY FAMILY', 'FIRST DAY FREE', 'LIFETIME', 'LIFETIME FAMILY', 'HERITAGE ESTATES'].includes(m.type) && (() => { var dd = new Date(m.nextPayment + 'T00:00:00'); var dl = Math.ceil((dd - new Date()) / (1000*60*60*24)); return dl >= 0 && dl <= 5; })()) {
        setKioskMessage({ text: `Welcome, ${m.firstName}!`, type: 'success', subtext: 'Friendly reminder: your payment is coming due soon.', photoUrl: m.photoUrl });
      } else {
      ...
      try {
        ...
        const result = await res.json();
        if (!result.success) {
          setVisits(prev => prev.filter((v, i) => i > 0 || v.name !== (m.firstName + ' ' + m.lastName) || v.time !== currentTime));
          setMembers(prev => prev.map(member => member.id === id ? { ...member, visits: member.visits - 1 } : member));
...
showToast('Network error.', 'error', 3000);
  ...
  const handleClassCheckIn = async (mem, className, classCenter, classDate, classTime) => {
    ...
    // Check if member already has a visit on this date NEAR the class time (within 1 hour)
    const classDateTime = new Date(classDate + 'T' + classTimeStr);
    const sameDayVisits = visits.filter(function(v) {
      return v.name === fullName && new Date(v.time).toDateString() === targetDateStr;
    });
    // Already on this class roster?
    const alreadyInClass = sameDayVisits.find(function(v) { return v.method === classMethod; });
    if (alreadyInClass) {
      showToast(mem.firstName + ' is already on the class roster.', 'warning', 3000);
      return false;
    }
    // Find the closest non-class visit within 1 hour of the class time
    var existingVisit = null;
    var closestDiff = Infinity;
    sameDayVisits.forEach(function(v) {
      if (v.method && v.method.startsWith('Class:')) return;
      var diff = Math.abs(new Date(v.time) - classDateTime);
      if (diff < 60 * 60 * 1000 && diff < closestDiff) {
        closestDiff = diff;
        existingVisit = v;
      }
    });
  ...
  if (existingVisit) {
      // They have a non-class visit within 1 hour of the class — relabel it
      try {
            ...
            <div className="flex-1 min-h-0 flex flex-col items-center justify-center px-8 py-4 relative">
              ...
              {kioskMessage.text && (
                <div onClick={() => setKioskMessage({ text: '', type: '', subtext: '' })} className={`fixed inset-0 z-[100] flex flex-col items-center justify-center text-white cursor-pointer p-10 ${kioskMessage.type === 'success' ? 'bg-gradient-to-br from-green-500 to-green-700' : kioskMessage.type === 'warning' ? 'bg-gradient-to-br from-amber-500 to-amber-600' : 'bg-gradient-to-br from-red-500 to-red-700'}`}>
                  ...
                  {kioskMessage.cooldownBlocked && (<button onClick={(e) => { e.stopPropagation(); processCheckIn(kioskMessage.memberId, 'Repeat Visit (Same Day)', true); }} className="mt-12 bg-white text-amber-700 px-16 py-8 rounded-3xl text-5xl font-black shadow-2xl hover:bg-amber-50 transition-colors border-4 border-white">{tKiosk('Yes, Check Me In')}</button>)}
                  <div className={(kioskMessage.cooldownBlocked ? "mt-8 text-2xl" : "mt-12 text-xl") + " font-bold uppercase tracking-widest opacity-70"}>{kioskMessage.cooldownBlocked ? tKiosk('Or tap anywhere to cancel') : tKiosk('Tap to continue')}</div>
                </div>
              )}
            </div>
        ...
        {isScanning && (
          <div className="fixed inset-0 bg-[#001226]/95 backdrop-blur-2xl z-[200] flex flex-col items-center justify-center p-6">
            ...
            <div className="mb-12 text-center">
              <h2 className="text-white text-4xl font-black mb-4 tracking-[0.2em] uppercase">{kioskLang === 'es' ? 'Escáner QR' : 'Badge Scanner'}</h2>
              <p className="text-[#1080ad] font-bold tracking-widest animate-pulse uppercase text-3xl">{kioskLang === 'es' ? 'Alinee el código QR' : 'Align QR Code within the markers'}</p>
            </div>
            ...
            <button onClick={() => setIsScanning(false)} className="mt-16 text-white/40 font-black text-xl uppercase tracking-[0.3em]">{kioskLang === 'es' ? 'Cancelar Escaneo' : 'Cancel Scan'}</button>
          </div>
        )}
  ...
  return (
    <div className="flex min-h-screen bg-[#f0f2f5] font-sans text-slate-800">
      ...
      <main className="flex-1 p-10 h-screen overflow-y-auto relative print:m-0 print:p-0 print:h-auto print:overflow-visible">
                ...
                return (<div className={'grid gap-6 mb-8 ' + (cols.length === 1 ? 'grid-cols-1' : 'grid-cols-1 md:grid-cols-2')}>
                  {cols.map(col => (
                    <div key={col.name} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                      ...
                      {col.list.length === 0 ? (<p className="text-sm text-slate-400 italic bg-slate-50 p-4 rounded-xl">No blocked sign-ins.</p>) : (
                        <div className="space-y-2 max-h-[280px] overflow-y-auto pr-1">
                          {col.list.map(b => { const mem = members.find(m => m.airtableId === b.memberRecId); return (
                            <div key={b.airtableId} onClick={() => mem && setSelectedMember(mem)} className="flex justify-between items-center gap-2 bg-slate-50 hover:bg-red-50 p-3 rounded-xl border border-slate-100 cursor-pointer transition-colors">
                              <div className="min-w-0"><p className="font-bold text-slate-800 text-sm truncate">{b.memberName}</p><p className="text-[10px] text-slate-400">{b.timestamp ? new Date(b.timestamp).toLocaleDateString('en-US', {month:'short', day:'numeric'}) + ' ' + new Date(b.timestamp).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'}) : ''}</p></div>
                              <span className={'shrink-0 px-2 py-1 rounded text-[9px] font-black uppercase tracking-tight ' + reasonCls(b.reason)}>{b.reason}</span>
                              <button onClick={(e) => { e.stopPropagation(); logVisitForBlocked(mem, b.airtableId, b.memberName, b.timestamp); }} className="shrink-0 bg-white border border-slate-200 text-slate-500 hover:bg-[#1080ad] hover:text-white hover:border-[#1080ad] px-2 py-1 rounded text-[9px] font-black uppercase tracking-tight transition-colors" title="Log a visit - they were let in anyway">+ Visit</button>
                            </div>); })}
                        </div>
                      )}
                    </div>
        ...
        {activeTab === 'members' && (<div className="space-y-6"><div className="flex justify-between items-center mb-8"><div><h2 className="text-3xl font-bold text-[#001f3f] tracking-tight">Members</h2></div><div className="flex gap-3"><button onClick={() => setShowDuplicatesModal(true)} className="bg-white border border-slate-200 text-[#001f3f] px-6 py-2 rounded-xl font-bold text-sm shadow-sm flex items-center gap-2 hover:bg-slate-50 transition-all" title="Find and merge duplicate member records"><Users size={16}/> Find Duplicates</button><button onClick={handleExportCSV} className="bg-white border border-slate-200 text-[#001f3f] px-6 py-2 rounded-xl font-bold text-sm shadow-sm flex items-center gap-2 hover:bg-slate-50 transition-all"><Download size={16}/> Export CSV</button><button onClick={() => { setShowAddModal(true); setNewMemberPin(null); setFamilyFlow(null); setSelectedSponsor(''); setCustomSponsor(''); setFirstPayment({ open: false, prorated: false, method: null, memberData: null }); }} className="bg-[#001f3f] text-white px-6 py-2 rounded-xl font-bold text-sm shadow-xl shadow-blue-900/10 flex items-center gap-2 hover:bg-blue-900 transition-colors"><Plus size={16}/> Add Member</button></div></div><div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 space-y-4"><div className="relative"><Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300" size={16} /><input className="pl-12 pr-4 py-2 border rounded-xl text-sm w-full outline-none" placeholder="Search..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} /></div><div className="flex flex-wrap gap-2 items-center">{[{k:'all',label:'All Members'},{k:'recent',label:'Recent'},{k:'paid',label:'Paid'},{k:'unpaid',label:'Unpaid'},{k:'overdue',label:'Overdue'},{k:'247',label:'24/7 Access'},{k:'orientation',label:'Needs Orientation'},{k:'onboarding',label:'Needs Onboarding'},{k:'inactive',label:'Inactive'},{k:'corporate',label:'Corporate'},{k:'family',label:'Family Plans'},{k:'minors',label:'Minors'},{k:'minors-parent',label:'Minors - Needs Parent'},{k:'notes',label:'Has Notes'}].map(f => (<button key={f.k} onClick={() => setMemberFilter(memberFilter === f.k ? 'all' : f.k)} className={`px-3 py-1.5 rounded-lg text-[11px] font-bold transition-all ${memberFilter === f.k ? 'bg-[#001f3f] text-white shadow-md' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'}`}>{f.label}</button>))}{memberFilter === 'recent' && (<div className="flex items-center gap-2 ml-2 bg-blue-50 border border-blue-200 px-3 py-1.5 rounded-lg"><span className="text-[10px] font-black text-[#1080ad] uppercase tracking-widest">Joined since:</span><input type="date" value={recentCutoffDate} onChange={e => setRecentCutoffDate(e.target.value)} className="bg-white border border-blue-200 rounded px-2 py-1 text-[11px] font-bold text-[#001f3f] outline-none focus:border-[#1080ad]" /><span className="text-[10px] font-bold text-blue-600">({filteredMembers.length} member{filteredMembers.length !== 1 ? 's' : ''})</span></div>)}</div></div>{loading ? (<div className="text-center py-20 text-slate-300 font-medium italic">Syncing Airtable...</div>) : apiError ? (<div className="bg-red-50 border-2 border-red-200 text-red-700 p-10 rounded-2xl text-center shadow-sm"><AlertCircle size={48} className="mx-auto mb-4 text-red-500" /><h3 className="text-2xl font-black mb-2">Airtable Refused Connection</h3><p className="font-mono text-sm bg-white p-4 rounded-lg border border-red-100 max-w-2xl mx-auto">{apiError}</p></div>) : (<div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-visible pb-24"><table className="w-full text-left border-collapse"><thead className="bg-slate-50 text-[11px] font-black text-slate-400 uppercase tracking-widest border-b"><tr><th className="px-6 py-4 w-56">Member</th><th className="px-4 py-4">ID</th><th className="px-4 py-4 w-36">Type</th><th className="px-4 py-4 w-28">Status</th><th className="px-4 py-4 w-28">Payment</th><th className="px-4 py-4 w-24">Info</th></tr></thead><tbody className="text-sm">{filteredMembers.map(m => { const light = getStoplight(m); return (<tr key={m.id} className="border-b hover:bg-slate-50/80 cursor-pointer" onClick={() => setSelectedMember(m)}><td className="px-6 py-4"><div className="flex items-center gap-3"><MemberPhoto src={m.photoUrl} name={m.firstName} size={32} /><div><p className="font-bold text-slate-800">{m.firstName} {m.lastName}</p><p className="text-[11px] text-slate-400">{m.email}{m.needsOrientation && <span className="ml-2 px-2 py-0.5 rounded text-[9px] font-black bg-blue-100 text-blue-700 uppercase">Orientation</span>}{(!m.basicOrientation || !m.paperworkCompleted) && <span className="ml-2 px-2 py-0.5 rounded text-[9px] font-black bg-amber-100 text-amber-700 uppercase">Onboarding</span>}{m.isMinor && <span className={'ml-2 px-2 py-0.5 rounded text-[9px] font-black uppercase ' + (m.mayAttendAlone ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-700')}>{m.mayAttendAlone ? 'Minor · OK alone' : '⚠ Minor · Parent'}</span>}{m.familyName && (() => { const fam = members.filter(f => f.familyName === m.familyName && f.airtableId !== m.airtableId); return <span className="ml-2 px-2 py-0.5 rounded text-[9px] font-black bg-purple-100 text-purple-700">{m.familyName}{fam.length > 0 && <span className="font-medium text-purple-500">: {fam.map(f => f.firstName).join(', ')}</span>}</span>; })()}{m.sponsorName && <span className="ml-2 px-2 py-0.5 rounded text-[9px] font-black bg-orange-100 text-orange-700 uppercase">{m.sponsorName}</span>}{m.access247 && <span className="ml-2 px-2 py-0.5 rounded text-[9px] font-black bg-amber-100 text-amber-700 uppercase">24/7</span>}{m.inactive && <span className="ml-2 px-2 py-0.5 rounded text-[9px] font-black bg-red-100 text-red-600 uppercase">Inactive</span>}</p>{m.notes && (<p className="mt-1.5 text-[10px] font-medium text-amber-700 bg-amber-50 border border-amber-200 px-2 py-1 rounded-md truncate max-w-[220px]" title={m.notes}><strong>Note:</strong> {m.notes}</p>)}</div></div></td><td className="px-4 py-4 font-mono text-slate-400 text-xs">{m.id}</td><td className="px-4 py-4"><span className="px-3 py-1 rounded-full bg-blue-50 text-blue-600 text-[10px] font-black tracking-tight">{m.type}</span></td><td className="px-4 py-4"><span className={`px-3 py-1 rounded-full text-[10px] font-black ${m.inactive?'bg-red-100 text-red-600':light==='green'?'bg-green-100 text-green-600':light==='yellow'?'bg-yellow-100 text-yellow-600':'bg-red-100 text-red-600'}`}>{m.inactive?'INACTIVE':light==='green'?'ACTIVE':light==='yellow'?'GRACE':'LOCKED'}</span></td><td className="px-4 py-4 text-center">{(() => { const isCorp = m.sponsorName && (m.type.includes('CORPORATE') || m.type.includes('HD6') || m.type.includes('HCHF')); const isComped = ['HD6', 'HD6 FAMILY', 'HCHF', 'MILITARY', 'MILITARY FAMILY', 'FIRST DAY FREE', 'LIFETIME', 'LIFETIME FAMILY', 'HERITAGE ESTATES'].includes(m.type); if (isComped) return <div className="relative group inline-flex flex-col items-center"><span className="px-3 py-1 rounded-full text-[10px] font-black bg-slate-100 text-slate-500">COMPED</span><div className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 bg-[#001f3f] text-white text-[10px] font-bold px-3 py-1.5 rounded-lg whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none shadow-lg z-10">Comped - no payment required</div></div>; const today = new Date(); const firstOfNextMonth = new Date(today.getFullYear(), today.getMonth() + 1, 1); const paidThisPeriod = (m.achAutoPay || m.corpAchPay || (m.familyName && members.some(fm => fm.familyName === m.familyName && fm.achAutoPay))) ? (!!m.nextPayment && new Date(m.nextPayment + 'T00:00:00') >= new Date(today.getFullYear(), today.getMonth(), today.getDate() - 7)) : (m.nextPayment && new Date(m.nextPayment + 'T00:00:00') >= firstOfNextMonth); let isPaid = paidThisPeriod; let paidLabel = paidThisPeriod ? `Paid: ${m.paymentMethod || 'Logged'}` : 'Not paid this month'; if (isCorp) { const sponsor = corporatePartners.find(cp => cp.sponsorMatch === m.sponsorName); const isMemHarper = m.center && m.center.toLowerCase().includes('harper'); const isMemAnthony = m.center && m.center.toLowerCase().includes('anthony'); const unifiedPaid = sponsor && sponsor.paidMonths && sponsor.paidMonths.split(',').find(str => str.startsWith(reportMonth)); const harperPaid = sponsor && sponsor.paidMonthsHarper && sponsor.paidMonthsHarper.split(',').find(str => str.startsWith(reportMonth)); const anthonyPaid = sponsor && sponsor.paidMonthsAnthony && sponsor.paidMonthsAnthony.split(',').find(str => str.startsWith(reportMonth)); const corpPaidEntry = unifiedPaid || (isMemHarper && harperPaid) || (isMemAnthony && anthonyPaid); if (corpPaidEntry) { isPaid = true; paidLabel = `Corp Paid: ${corpPaidEntry.split(':')[1] || 'Logged'} (${m.sponsorName})`; } else if (!paidThisPeriod) { paidLabel = `Awaiting ${m.sponsorName} payment`; } } const pillClass = isPaid ? (isCorp && !paidThisPeriod ? 'bg-purple-100 text-purple-700' : 'bg-green-100 text-green-700') : 'bg-red-100 text-red-700'; const pillText = isPaid ? (isCorp && !paidThisPeriod ? 'CORP PAID' : 'PAID') : (isCorp ? 'CORP PENDING' : 'NOT PAID'); return (<div className="relative group inline-flex flex-col items-center"><span className={`px-3 py-1 rounded-full text-[10px] font-black ${pillClass}`}>{pillText}</span><div className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 bg-[#001f3f] text-white text-[10px] font-bold px-3 py-1.5 rounded-lg whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none shadow-lg z-10">{paidLabel}{m.nextPayment && <span className="block text-white/50">Due: {m.nextPayment}</span>}</div></div>); })()}</td><td className="px-4 py-4"><button className="p-2 bg-[#1080ad] text-white rounded-lg shadow-md"><QrCode size={16}/></button></td></tr>);})}</tbody></table></div>)}</div>)}
        ...
        {activeTab === 'classes' && (() => {
         const allClasses = [
            ...
            { name: 'Water Aerobics', center: 'harper', days: 'Mon, Wed, Fri', time: '12:15 PM', capacity: 50, color: 'border-[#1080ad]', startDate: '2026-06-08', endDate: '2026-07-06', instructor: 'Loretta', makeupDates: ['2026-06-30', '2026-07-02'] }
          ];
          ...
          return (
            <div className="space-y-6">
              ...
              {activeClass && (() => {
                var classVisits = filteredVisits.filter(function(v) { return new Date(v.time).toDateString() === new Date(checkinDate + 'T00:00:00').toDateString() && v.method === 'Class: ' + activeClass.name; });
                return (
                  <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#001f3f]/90 backdrop-blur-md">
                    <div className="bg-white rounded-3xl w-full max-w-4xl p-10 relative shadow-2xl flex flex-col md:flex-row gap-8">
                      <button onClick={function() { setActiveClass(null); setKioskInput(''); }} className="absolute top-6 right-6 text-slate-300 hover:text-red-500 transition-all z-10"><X size={24}/></button>                       <button onClick={function() { var targetDateStr = new Date(checkinDate + 'T00:00:00').toDateString(); var currentRoster = filteredVisits.filter(function(v) { return new Date(v.time).toDateString() === targetDateStr && v.method === 'Class: ' + activeClass.name; }); if (currentRoster.length === 0) { alert('No attendees logged for this class on the selected date.'); return; } var csv = ['Name,Type,Time'].concat(currentRoster.map(function(v) { return '"' + v.name + '","' + (v.type || '') + '","' + new Date(v.time).toLocaleTimeString() + '"'; })).join('\n'); var b = new Blob([csv], {type: 'text/csv'}); var u = window.URL.createObjectURL(b); var a = document.createElement('a'); a.href = u; a.download = activeClass.name.replace(/\s+/g, '_') + '_' + checkinDate + '.csv'; a.click(); window.URL.revokeObjectURL(u); }} className="absolute top-6 right-20 bg-slate-100 hover:bg-slate-200 text-slate-600 px-3 py-2 rounded-xl text-xs font-bold transition-colors flex items-center gap-1 z-10"><Download size={14}/> Export</button>
                      ...
                      <div className="w-full md:w-80 bg-slate-50 rounded-2xl p-6 border border-slate-100 flex flex-col h-[400px]">
                        ...
                        <div className="flex-1 overflow-y-auto space-y-3 pr-2">{classVisits.length === 0 ? (<p className="text-slate-400 italic text-sm text-center mt-10">Waiting for class check-ins...</p>) : (classVisits.map((v, i) => (<div key={i} className="bg-white p-3 rounded-xl border border-slate-100 shadow-sm transition-all duration-300 flex justify-between items-center"><div><p className="font-bold text-slate-800 text-sm">{v.name}</p><p className="text-[10px] font-bold text-[#f59e0b] uppercase mt-1">{new Date(v.time).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})} • Checked In</p></div><button onClick={async () => { if (!window.confirm('Remove ' + v.name + ' from this class? Their visit will revert to a regular check-in.')) return; var matchMem = members.find(function(m) { return (m.firstName + ' ' + m.lastName) === v.name; }); if (!matchMem) { showToast('Member not found.', 'error', 3000); return; } try { var res = await fetch('/api/update-visit-method', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ memberAirtableId: matchMem.airtableId, visitTime: v.time, visitCenter: v.center, newMethod: 'Manual Entry' }) }); var result = await res.json(); if (result.success) { setVisits(function(prev) { return prev.map(function(visit) { return (visit.name === v.name && visit.time === v.time && visit.center === v.center) ? Object.assign({}, visit, { method: 'Manual Entry' }) : visit; }); }); showToast(v.name + ' removed from class (kept as regular visit).', 'success', 3000); } else { showToast('Update failed: ' + (result.error || 'Unknown error'), 'error', 4500); } } catch (err) { showToast('Network error.', 'error', 3000); } }} className="text-slate-300 hover:text-red-500 transition-colors p-1 shrink-0" title="Remove from class"><X size={14}/></button></div>)))}</div>
                      </div>
                    </div>
                  </div>
                );
        ...
        {activeTab === 'visitors' && (<div className="space-y-6">
          <div className="flex justify-between items-center mb-8">
            ...
            <button onClick={() => { setVisitorDupMatch(null); setShowAddVisitorModal(true); }} className="bg-[#001f3f] text-white px-6 py-2 rounded-xl font-bold text-sm shadow-xl shadow-blue-900/10 flex items-center gap-2 hover:bg-blue-900 transition-colors"><Plus size={16}/> Add Visitor</button>
          </div>
          <div className="grid grid-cols-4 gap-6">
            <ProStatCard value={visitors.filter(v => { const exp = new Date(v.expirationDate + 'T23:59:59'); const hasPasses = v.passesRemaining !== null && v.passesRemaining !== undefined && v.passesRemaining > 0; const outOfPasses = v.passesRemaining !== null && v.passesRemaining !== undefined && v.passesRemaining <= 0; return !outOfPasses && (exp >= new Date() || hasPasses); }).length} label="Active Passes" color="#16a34a" />
            <ProStatCard value={visitors.filter(v => { const exp = new Date(v.expirationDate + 'T23:59:59'); const hasPasses = v.passesRemaining !== null && v.passesRemaining !== undefined && v.passesRemaining > 0; const outOfPasses = v.passesRemaining !== null && v.passesRemaining !== undefined && v.passesRemaining <= 0; return outOfPasses || (exp < new Date() && !hasPasses); }).length} label="Expired" color="#dc2626" />
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-visible pb-24">
            <table className="w-full text-left border-collapse">
              <thead className="bg-slate-50 text-[11px] font-black text-slate-400 uppercase tracking-widest border-b"><tr><th className="px-6 py-4">Visitor</th><th className="px-4 py-4">Pass Type</th><th className="px-4 py-4">Pass Source</th><th className="px-4 py-4">Good For</th><th className="px-4 py-4">Status</th><th className="px-4 py-4">Visits</th><th className="px-4 py-4">Actions</th></tr></thead>
              <tbody className="text-sm">{(() => {
                const stillValid = (x) => {
                  const outP = x.passesRemaining !== null && x.passesRemaining !== undefined && x.passesRemaining <= 0;
                  const dExp = new Date(x.expirationDate + 'T23:59:59') < new Date();
                  return !outP && !dExp;
                };
                const groupMap = new Map();
                visitors.forEach(x => {
                  const key = ((x.firstName || '') + '|' + (x.lastName || '') + '|' + (x.phone || x.email || '')).toLowerCase().trim();
                  if (!groupMap.has(key)) groupMap.set(key, []);
                  groupMap.get(key).push(x);
                });
                const visitorGroups = Array.from(groupMap.values()).map(list => {
                  const ranked = list.slice().sort((a, b) => {
                    const av = stillValid(a) ? 1 : 0, bv = stillValid(b) ? 1 : 0;
                    if (av !== bv) return bv - av;
                    return new Date(b.purchaseDate || 0) - new Date(a.purchaseDate || 0);
                  });
                  return { rep: ranked[0], passCount: list.length, totalVisits: list.reduce((s, x) => s + (x.totalVisits || 0), 0), totalPaid: list.reduce((s, x) => s + (parseFloat(String(x.amountPaid).replace(/[^0-9.]/g, '')) || 0), 0) };
                }).sort((a, b) => (a.rep.lastName || '').localeCompare(b.rep.lastName || '') || (a.rep.firstName || '').localeCompare(b.rep.firstName || ''));
                return visitorGroups.length === 0 ? (<tr><td colSpan="7" className="text-center py-12 text-slate-400 font-medium italic">No visitors yet.</td></tr>) : visitorGroups.map(g => { const v = g.rep; const dateExpired = new Date(v.expirationDate + 'T23:59:59') < new Date(); const hasPassesLeft = v.passesRemaining !== null && v.passesRemaining !== undefined && v.passesRemaining > 0; const outOfPasses = v.passesRemaining !== null && v.passesRemaining !== undefined && v.passesRemaining <= 0; const expired = outOfPasses || (dateExpired && !hasPassesLeft); return (<tr key={v.airtableId} className="border-b hover:bg-slate-50/80"><td className="px-6 py-4"><p className="font-bold text-slate-800 cursor-pointer hover:text-[#1080ad]" onClick={() => setSelectedVisitor(v)}>{v.firstName} {v.lastName}</p><p className="text-[11px] text-slate-400">{v.email || v.phone || 'No contact info'}{!v.orientationComplete && <span className="ml-2 px-2 py-0.5 rounded text-[9px] font-black bg-blue-100 text-blue-700 uppercase">Needs Orientation</span>}</p></td><td className="px-4 py-4"><span className={`px-3 py-1 rounded-full text-[10px] font-black ${v.passType === 'Day Pass' ? 'bg-blue-50 text-blue-600' : 'bg-purple-50 text-purple-600'}`}>{v.passType}</span></td><td className="px-4 py-4 text-xs">{(() => { const cat = passSourceFor(v.referringProvider); const cls = cat === 'Therapy' ? 'bg-teal-100 text-teal-700' : cat === 'Provider' ? 'bg-indigo-100 text-indigo-700' : cat === 'Staff Comp' ? 'bg-amber-100 text-amber-700' : cat === 'Former Member' ? 'bg-purple-100 text-purple-700' : 'bg-slate-100 text-slate-500'; return (<><span className={'px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-tight ' + cls}>{cat}</span>{v.referringProvider && cat !== 'Staff Comp' && cat !== 'Former Member' && <p className="text-[11px] text-slate-600 font-bold mt-1">{v.referringProvider}</p>}</>); })()}</td><td className="px-4 py-4 text-xs">{(v.passesRemaining !== null && v.passesRemaining !== undefined) ? (<span className={outOfPasses ? 'text-red-500 font-bold' : 'text-slate-700 font-bold'}>{v.passesRemaining} {v.passesRemaining === 1 ? 'visit' : 'visits'} left</span>) : (<span className={dateExpired ? 'text-red-500 font-bold' : 'text-slate-600'}>{v.expirationDate ? 'Through ' + new Date(v.expirationDate + 'T00:00:00').toLocaleDateString('en-US', {month:'short',day:'numeric'}) : 'No limit set'}</span>)}<p className="text-[10px] text-slate-300 mt-0.5">{(v.passesRemaining !== null && v.passesRemaining !== undefined) ? 'counted by visit' : 'counted by date'}</p></td><td className="px-4 py-4"><span className={`px-3 py-1 rounded-full text-[10px] font-black ${expired ? 'bg-red-100 text-red-600' : !v.passActivated ? 'bg-amber-100 text-amber-600' : 'bg-green-100 text-green-600'}`}>{outOfPasses && !dateExpired ? 'NO PASSES LEFT' : expired ? 'EXPIRED' : !v.passActivated ? 'PENDING' : 'ACTIVE'}</span></td><td className="px-4 py-4"><span className="font-black text-[#1080ad]">{g.totalVisits}</span>{(g.passCount > 1 || g.totalPaid > 0) && <p className="text-[10px] text-slate-400 font-bold mt-0.5">{g.passCount > 1 ? g.passCount + ' passes' : '1 pass'}{g.totalPaid > 0 ? ' · $' + g.totalPaid.toFixed(2) + ' total' : ''}</p>}</td><td className="px-4 py-4 flex gap-2">
                <button onClick={() => handleLogVisitorVisit(v)} className="px-3 py-2 bg-[#1080ad] text-white rounded-lg shadow-md hover:bg-[#0d6d94] text-[10px] font-black uppercase tracking-tight whitespace-nowrap" title="Record a visit and use one pass">Log Visit</button>
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-3">
              ...
              {corporatePartners.filter(c => c.name.toLowerCase().includes(corpSearch.toLowerCase())).map(corp => {
                 ...
                 var famPrimary = {}; corpMembers.forEach(function(cm) { if (cm.familyName) { var cmRate = parseFloat(String(cm.monthlyRate).replace(/[^0-9.]/g, '')) || 0; if (!famPrimary[cm.familyName]) { famPrimary[cm.familyName] = { id: cm.airtableId, rate: cmRate }; } else { if (cmRate > famPrimary[cm.familyName].rate) { famPrimary[cm.familyName] = { id: cm.airtableId, rate: cmRate }; } } } }); const enrichedMembers = corpMembers.map(mem => { const memVisits = currentPeriodVisits.filter(v => v.name.toLowerCase() === `${mem.firstName} ${mem.lastName}`.toLowerCase()); totalPeriodVisits += memVisits.length; var rawRate = parseFloat(String(mem.monthlyRate).replace(/[^0-9.]/g, '')) || 0; var isDependent = mem.familyName && famPrimary[mem.familyName] && famPrimary[mem.familyName].id !== mem.airtableId; var rate = isDependent ? 0 : rawRate; let memberOwed = 0; let activeMonthsCount = 0; if (isUsageBased) { targetMonths.forEach(mIdx => { const visitedInMonth = memVisits.some(v => new Date(v.time).getMonth() === mIdx); if (visitedInMonth) { memberOwed += rate; activeMonthsCount++; } }); } else { if (mem.status === 'ACTIVE') { memberOwed = rate * targetMonths.length; activeMonthsCount = targetMonths.length; } } totalOwed += memberOwed; return { ...mem, periodVisits: memVisits.length, memberOwed, activeMonthsCount }; });
                 const activeMembersCount = enrichedMembers.filter(m => m.memberOwed > 0 || m.status === 'ACTIVE').length;
                 const isInternal = corp.name.toUpperCase().includes('HD6') || corp.name.toUpperCase().includes('HCHF') || corp.sponsorMatch.toUpperCase().includes('HD6') || corp.sponsorMatch.toUpperCase().includes('HCHF');
                 const splitCorpNames = ['Harper Industries', 'USD 361', 'Harper County'];
                 const isSplitPaid = splitCorpNames.some(sc => corp.name.toLowerCase().includes(sc.toLowerCase()) || corp.sponsorMatch.toLowerCase().includes(sc.toLowerCase()));
                 const paidMatch = corp.paidMonths ? corp.paidMonths.split(',').find(str => str.startsWith(reportMonth)) : null;
                 const paidMatchHarper = corp.paidMonthsHarper ? corp.paidMonthsHarper.split(',').find(str => str.startsWith(reportMonth)) : null;
                 const paidMatchAnthony = corp.paidMonthsAnthony ? corp.paidMonthsAnthony.split(',').find(str => str.startsWith(reportMonth)) : null;
                 const isPaidHarper = isInternal || !!paidMatchHarper;
                 const isPaidAnthony = isInternal || !!paidMatchAnthony;
                 const isPaid = isSplitPaid ? (isPaidHarper && isPaidAnthony) : (isInternal || !!paidMatch);
                 const corpPayMethod = paidMatch && paidMatch.includes(':') ? paidMatch.split(':')[1] : '';
                 const corpPayMethodHarper = paidMatchHarper && paidMatchHarper.includes(':') ? paidMatchHarper.split(':')[1] : '';
                 const corpPayMethodAnthony = paidMatchAnthony && paidMatchAnthony.includes(':') ? paidMatchAnthony.split(':')[1] : '';
                 const togglePayment = async (targetCenter) => {
                   if (isSplitPaid && targetCenter) {
                     const fieldKey = targetCenter === 'harper' ? 'paidMonthsHarper' : 'paidMonthsAnthony';
                     const airtableField = targetCenter === 'harper' ? 'Paid Months Harper' : 'Paid Months Anthony';
                     const currentValue = corp[fieldKey] || '';
                     const currentlyPaid = targetCenter === 'harper' ? isPaidHarper : isPaidAnthony;
                     if (currentlyPaid) {
                       const newValue = currentValue.split(',').filter(mn => !mn.startsWith(reportMonth)).join(',');
                       setCorporatePartners(prev => prev.map(c => c.id === corp.id ? { ...c, [fieldKey]: newValue } : c));
                       try { await fetch('/api/update-corporate', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ recordId: corp.id, [airtableField]: newValue }) }); } catch (err) { alert('Failed to sync payment status to Airtable.'); }
                     } else {
                       setCorpPaymentModal({ ...corp, reportMonth, displayPeriod, splitCenter: targetCenter });
                     }
                   } else {
                     if (isPaid) {
                       ...
                       try { await fetch('/api/update-corporate', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ recordId: corp.id, paidMonths: newPaidMonths }) }); } catch (err) { alert('Failed to sync payment status to Airtable.'); }
                     } else {
                     ...
                     <button onClick={() => setExpandedCorpId(expandedCorpId === corp.id ? null : corp.id)} className="w-full p-5 flex items-center justify-between text-left hover:bg-slate-50/50 transition-colors rounded-2xl">
                       <div className="flex items-center gap-4 min-w-0">
                         ...
                         <div className="min-w-0">
                           ...
                           <p className="text-[11px] text-slate-400 font-bold">{corpMembers.length} enrolled · {totalPeriodVisits} visits · {isInternal ? 'No Charge' : '$' + totalOwed.toFixed(2) + ' owed'}</p>
                         </div>
                       </div>
                       <div className="flex items-center gap-3 shrink-0">
                         {isInternal ? (<span className="px-3 py-1 rounded-full text-[10px] font-black tracking-widest bg-slate-100 text-slate-600">INTERNAL · NO CHARGE</span>) : isSplitPaid ? (<div className="flex gap-1.5"><span className={`px-2 py-1 rounded-full text-[9px] font-black tracking-widest ${isPaidAnthony ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>ANT {isPaidAnthony ? '✓' : 'UNPAID'}</span><span className={`px-2 py-1 rounded-full text-[9px] font-black tracking-widest ${isPaidHarper ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>HRP {isPaidHarper ? '✓' : 'UNPAID'}</span></div>) : (<span className={`px-3 py-1 rounded-full text-[10px] font-black tracking-widest ${isPaid ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>{isPaid ? `PAID${corpPayMethod ? ` · ${corpPayMethod.toUpperCase()}` : ''}` : 'UNPAID'}</span>)}
                       ...
                       <div className="px-6 pb-6 border-t border-slate-100">
                         <div className="pt-5 space-y-4">
                           ...
                           <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                             ...
                             <div className="bg-green-50 p-3 rounded-lg border border-green-100"><span className="text-[10px] font-bold text-green-600 uppercase block">{isInternal ? 'Charge' : 'Total Owed'}</span><span className="text-lg font-black text-[#16a34a]">{isInternal ? 'No Charge' : '$' + totalOwed.toFixed(2)}</span></div>
                           ...
                           {/* Employee Roster */}
                           {(() => {
                             const splitCorps = ['Harper Industries', 'USD 361', 'Harper County'];
                             const isSplitCorp = splitCorps.some(sc => corp.name.toLowerCase().includes(sc.toLowerCase()) || corp.sponsorMatch.toLowerCase().includes(sc.toLowerCase()));
                             if (isSplitCorp) {
                               const anthonyEms = enrichedMembers.filter(em => em.center && em.center.toLowerCase().includes('anthony'));
                               const harperEms = enrichedMembers.filter(em => em.center && em.center.toLowerCase().includes('harper'));
                               return (<div className="space-y-3">
                                 {anthonyEms.length > 0 && (<div className="bg-blue-50 rounded-xl border border-blue-100 overflow-hidden"><div className="px-3 py-2 border-b border-blue-100 flex justify-between"><span className="text-[9px] font-black text-blue-600 uppercase tracking-widest">Anthony Wellness Center</span><span className="text-[9px] font-black text-blue-600">{anthonyEms.length} member{anthonyEms.length !== 1 ? 's' : ''} — ${anthonyEms.reduce((s, em) => s + em.memberOwed, 0).toFixed(2)}</span></div><div className="max-h-36 overflow-y-auto p-2 space-y-1">{anthonyEms.map(em => (<div key={em.id} className="flex justify-between items-center bg-white p-2 rounded-lg border border-blue-100 shadow-sm"><div><p className="text-xs font-bold text-slate-800">{em.firstName} {em.lastName}</p><p className="text-[9px] text-slate-400 uppercase tracking-widest">{em.type} • {em.periodVisits} visit{em.periodVisits !== 1 ? 's' : ''}</p></div><span className={`text-xs font-black ${em.memberOwed > 0 ? 'text-[#16a34a]' : 'text-slate-400'}`}>${em.memberOwed.toFixed(2)}</span></div>))}</div></div>)}
                                 {harperEms.length > 0 && (<div className="bg-orange-50 rounded-xl border border-orange-100 overflow-hidden"><div className="px-3 py-2 border-b border-orange-100 flex justify-between"><span className="text-[9px] font-black text-orange-600 uppercase tracking-widest">Harper Wellness Center</span><span className="text-[9px] font-black text-orange-600">{harperEms.length} member{harperEms.length !== 1 ? 's' : ''} — ${harperEms.reduce((s, em) => s + em.memberOwed, 0).toFixed(2)}</span></div><div className="max-h-36 overflow-y-auto p-2 space-y-1">{harperEms.map(em => (<div key={em.id} className="flex justify-between items-center bg-white p-2 rounded-lg border border-orange-100 shadow-sm"><div><p className="text-xs font-bold text-slate-800">{em.firstName} {em.lastName}</p><p className="text-[9px] text-slate-400 uppercase tracking-widest">{em.type} • {em.periodVisits} visit{em.periodVisits !== 1 ? 's' : ''}</p></div><span className={`text-xs font-black ${em.memberOwed > 0 ? 'text-[#16a34a]' : 'text-slate-400'}`}>${em.memberOwed.toFixed(2)}</span></div>))}</div></div>)}
                               </div>);
                             }
                             return (<div className="bg-slate-50 rounded-xl border border-slate-100 overflow-hidden"><div className="max-h-48 overflow-y-auto p-3 space-y-2">{enrichedMembers.length === 0 ? <p className="text-xs text-slate-400 italic p-2">No members enrolled.</p> : enrichedMembers.map(em => (<div key={em.id} className="flex justify-between items-center bg-white p-2 rounded-lg border border-slate-100 shadow-sm"><div><p className="text-xs font-bold text-slate-800">{em.firstName} {em.lastName}</p><p className="text-[9px] text-slate-400 uppercase tracking-widest">{em.type} • {em.periodVisits} visit{em.periodVisits !== 1 ? 's' : ''}</p></div><span className={`text-xs font-black ${em.memberOwed > 0 ? 'text-[#16a34a]' : 'text-slate-400'}`}>${em.memberOwed.toFixed(2)}</span></div>))}</div></div>);
                           })()}
                           {/* Action Buttons */}
                           <div className="flex flex-col gap-2 pt-2">
                             <div className="flex gap-2">
                               <div className="relative flex-1">{(() => { var splitCorpNames = ['Harper Industries', 'USD 361', 'Harper County']; var isSplit = splitCorpNames.some(function(sc) { return corp.name.toLowerCase().includes(sc.toLowerCase()) || corp.sponsorMatch.toLowerCase().includes(sc.toLowerCase()); }); if (!isSplit) { return <button onClick={function() { printInvoiceForCenter(viewingCenter === 'harper' ? 'harper' : 'anthony'); }} className="w-full bg-[#16a34a] text-white py-3 rounded-xl text-sm font-bold shadow-sm flex items-center justify-center gap-2 hover:bg-green-700 transition-colors"><Printer size={16}/> Invoice</button>; } return (<><button onClick={function() { setInvoiceMenu(invoiceMenu === corp.id ? null : corp.id); }} className="w-full bg-[#16a34a] text-white py-3 rounded-xl text-sm font-bold shadow-sm flex items-center justify-center gap-2 hover:bg-green-700 transition-colors"><Printer size={16}/> Invoice</button>{invoiceMenu === corp.id && (<><div className="fixed inset-0 z-40" onClick={function() { setInvoiceMenu(null); }}></div><div className="absolute left-0 bottom-full mb-1 bg-white border border-slate-200 shadow-xl rounded-xl z-50 overflow-hidden w-56"><button onClick={function() { printInvoiceForCenter('anthony'); setInvoiceMenu(null); }} className="w-full text-left px-4 py-3 text-sm font-bold text-[#1080ad] hover:bg-blue-50 border-b border-slate-100 flex items-center gap-2"><MapPin size={14}/> Anthony Invoice</button><button onClick={function() { printInvoiceForCenter('harper'); setInvoiceMenu(null); }} className="w-full text-left px-4 py-3 text-sm font-bold text-[#dd6d22] hover:bg-orange-50 border-b border-slate-100 flex items-center gap-2"><MapPin size={14}/> Harper Invoice</button><button onClick={function() { printInvoiceForCenter('anthony'); setTimeout(function() { printInvoiceForCenter('harper'); }, 500); setInvoiceMenu(null); }} className="w-full text-left px-4 py-3 text-sm font-bold text-[#001f3f] hover:bg-slate-50 flex items-center gap-2"><Printer size={14}/> Both Invoices</button></div></>)}</>); })()}</div>
                             ...
                             {!isInternal && !isSplitPaid && (<button onClick={() => togglePayment()} className={`w-full py-3 rounded-xl text-sm font-bold shadow-sm transition-colors border ${isPaid ? 'bg-white border-slate-200 text-slate-500 hover:bg-slate-50' : 'bg-blue-50 border-blue-200 text-blue-700 hover:bg-blue-100'}`}>{isPaid ? 'Mark as Unpaid' : 'Mark as Paid'}</button>)}                              {!isInternal && isSplitPaid && (<div className="flex gap-2"><button onClick={() => togglePayment('anthony')} className={`flex-1 py-3 rounded-xl text-sm font-bold shadow-sm transition-colors border ${isPaidAnthony ? 'bg-white border-slate-200 text-slate-500 hover:bg-slate-50' : 'bg-blue-50 border-blue-200 text-blue-700 hover:bg-blue-100'}`}>{isPaidAnthony ? `Anthony Paid · ${corpPayMethodAnthony || 'Logged'}` : 'Mark Anthony Paid'}</button><button onClick={() => togglePayment('harper')} className={`flex-1 py-3 rounded-xl text-sm font-bold shadow-sm transition-colors border ${isPaidHarper ? 'bg-white border-slate-200 text-slate-500 hover:bg-slate-50' : 'bg-orange-50 border-orange-200 text-orange-700 hover:bg-orange-100'}`}>{isPaidHarper ? `Harper Paid · ${corpPayMethodHarper || 'Logged'}` : 'Mark Harper Paid'}</button></div>)}
                           </div>
                         </div>
                       </div>
                     )}
...
{activeTab === 'payments' && (() => {
        ...
        if (viewingCenter !== 'both') {
            var vc2 = viewingCenter.toLowerCase();
            periodPayments = periodPayments.filter(function(p) {
              var payCenter = p.center ? p.center.toLowerCase() : '';
              if (payCenter) return payCenter.includes(vc2);
              var mem = members.find(function(m) { return m.airtableId === p.memberRecId; });
              if (mem) return mem.center && mem.center.toLowerCase().includes(vc2);
              var vis = visitors.find(function(v) { return v.airtableId === p.memberRecId; });
              if (vis) return vis.center && vis.center.toLowerCase().includes(vc2);
              return false;
            });
          }
...
<th className="px-4 py-4 w-28"></th>                    </tr>
                    ...
                    {periodPayments.length === 0 ? (
                      ...
                      return (
                        <tr key={p.airtableId} className="border-b hover:bg-slate-50/80">
                          ...
                          <td className="px-4 py-4"><span title={p.method === 'Unknown' ? 'Payment method not recorded — this is likely a historical visitor record created before payment tracking was added.' : p.method === 'Other' ? 'Non-standard payment — see notes' : ''} className={"px-3 py-1 rounded-full text-[10px] font-black " + (p.method === 'Cash' ? 'bg-green-100 text-green-700' : p.method.includes('Check') ? 'bg-amber-100 text-amber-700' : p.method === 'Card' ? 'bg-purple-100 text-purple-700' : p.method === 'Other' ? 'bg-slate-200 text-slate-700 cursor-help' : p.method === 'Unknown' ? 'bg-slate-100 text-slate-500 cursor-help' : 'bg-blue-100 text-blue-700')}>{displayMethod}</span></td>
                          ...
                          <td className="px-4 py-4">
                           {user && user.role !== 'business' && (memberMatch || isActualVisitor) && (<div className="flex gap-1 justify-end"><button onClick={function() { setEditPaymentModal(Object.assign({}, p, isActualVisitor ? { _isVisitor: true, _visitorAirtableId: p.memberRecId } : {})); }} className="text-slate-300 hover:text-[#1080ad] transition-colors p-1" title="Edit this payment"><FileText size={14}/></button>{!p.isRefund && memberMatch && (<button onClick={function() { setRefundModal({ member: memberMatch, originalPayment: p }); setRefundAmount(String(parseFloat(p.amount) || 0)); setRefundReason(''); setRefundMethod(p.method || 'Cash'); setRefundCheckNumber(''); setRefundDate(new Date().toISOString().split('T')[0]); }} className="text-slate-300 hover:text-orange-500 transition-colors p-1" title="Issue a refund"><RotateCcw size={14}/></button>)}
                              {memberMatch && <button onClick={function() { if (!window.confirm('Delete this $' + (parseFloat(p.amount) || 0).toFixed(2) + ' ' + displayMethod + ' payment for ' + displayName + '?')) return; fetch('/api/delete-payment', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ paymentId: p.airtableId }) }).then(function(r) { return r.json(); }).then(function(result) { if (result.success) { setPayments(function(prev) { return prev.filter(function(pay) { return pay.airtableId !== p.airtableId; }); }); showToast('Payment deleted.', 'success', 3000); } else { alert('Error: ' + (result.error || 'Could not delete.')); } }).catch(function() { alert('Network error.'); }); }} className="text-slate-300 hover:text-red-500 transition-colors p-1" title="Delete this payment"><Trash2 size={14}/></button>}</div>
                            )}
                          </td>
                        </tr>
                      );
 ...
 const isCorp = m.type.includes('CORPORATE') || m.sponsorName;
         ...
         if (isCorp) {
              ...
              const isMemHarper = m.center && m.center.toLowerCase().includes('harper');
              const isMemAnthony = m.center && m.center.toLowerCase().includes('anthony');
                ...
                <div className="flex gap-3 flex-wrap">
                  ...
                  <button onClick={() => {
                    ...
                    if (isCustomRange) {
                      ...
                      const [pStr, yStr] = reportMonth.split('-');
                      if (pStr.startsWith('Q')) { const q = parseInt(pStr.replace('Q', '')); displayPeriod = 'Q' + q + ' ' + yStr; } else { displayPeriod = new Date(parseInt(yStr), parseInt(pStr) - 1).toLocaleDateString('en-US', { month: 'long', year: 'numeric' }); }
                    }
                    ...
                    const isCorpMem = (m) => m.type.includes('CORPORATE') || m.sponsorName;
                    const amountForMember = (m) => { if (isCorpMem(m)) return parseFloat(String(m.monthlyRate).replace(/[^0-9.]/g, '')) || 0; var paid = payments.filter(p => p.memberRecId === m.airtableId && !p.isRefund && p.date && isInPeriod(p.date)).reduce((s, p) => s + (parseFloat(String(p.amount).replace(/[^0-9.]/g, '')) || 0), 0); return paid > 0 ? paid : (parseFloat(String(m.monthlyRate).replace(/[^0-9.]/g, '')) || 0); };
                    const calcTotal = (mems) => mems.reduce((sum, m) => sum + amountForMember(m), 0);
                    ...
                    const getPayMethod = (m) => { if (m.type.includes('CORPORATE') || m.sponsorName) { const sponsor = corporatePartners.find(cp => cp.sponsorMatch === m.sponsorName); if (sponsor) { const findMatch = (field) => { if (!sponsor[field]) return null; const entries = sponsor[field].split(','); if (isCustomRange) return entries.find(e => rangeMonthKeys.some(k => e.startsWith(k))); return entries.find(e => e.startsWith(reportMonth)); }; const match = findMatch('paidMonths') || findMatch('paidMonthsHarper') || findMatch('paidMonthsAnthony'); if (match) return match.includes(':') ? match.split(':')[1] : 'Corp Payment'; } } var pm = m.paymentMethod || 'None Logged'; if (pm === 'Check' && m.checkNumber) pm = 'Check #' + m.checkNumber; return pm; };
                    const getPaymentForMember = (m) => {
                      const payList = payments.filter(p => p.memberRecId === m.airtableId);
                      if (payList.length === 0) return null;
                      const periodPay = payList.find(p => p.date && isInPeriod(p.date));
                      return periodPay || payList[payList.length - 1];
                    };
                    const cleanNote = (n) => {
                      if (!n) return '';
                      return n.replace(/\|?\s*(Check #[^\s\-]+ - )?Logged by staff via Wellness Hub\s*\|?/gi, '').replace(/\|\s*\|/g, '|').replace(/^\s*\|\s*|\s*\|\s*$/g, '').trim();
                    };
                    const buildTable = (mems, title, total) => { if (mems.length === 0) return ''; const rows = mems.map(m => { const finalRate = amountForMember(m); const payMethod = getPayMethod(m); const pay = getPaymentForMember(m); const payDate = pay && pay.date ? new Date(pay.date + 'T00:00:00').toLocaleDateString('en-US', {month: 'short', day: 'numeric'}) : '—'; const payNote = pay ? cleanNote(pay.notes) : ''; return `<tr><td style="padding: 10px; border-bottom: 1px solid #e2e8f0;">${m.firstName} ${m.lastName}</td><td style="padding: 10px; border-bottom: 1px solid #e2e8f0;">${m.type}${m.sponsorName ? ` (${m.sponsorName})` : ''}</td><td style="padding: 10px; border-bottom: 1px solid #e2e8f0;">${payMethod}</td><td style="padding: 10px; border-bottom: 1px solid #e2e8f0; color: #64748b; font-size: 11px;">${payDate}</td><td style="padding: 10px; border-bottom: 1px solid #e2e8f0; color: #475569; font-size: 11px; font-style: ${payNote ? 'normal' : 'italic'};">${payNote || '—'}</td><td style="padding: 10px; border-bottom: 1px solid #e2e8f0; font-weight:bold; color: #16a34a;">$${finalRate.toFixed(2)}</td></tr>`; }).join(''); return `<h3 style="color:#003d6b; margin-top:30px; margin-bottom:10px; font-size:16px; text-transform:uppercase; letter-spacing:1px;">${title}</h3><table><thead><tr><th>Member Name</th><th>Plan Type</th><th>Payment Method</th><th>Payment Date</th><th>Notes</th><th>Final Rate</th></tr></thead><tbody>${rows}</tbody><tfoot><tr class="total-row"><td colspan="5" class="total-lbl">${title} Revenue:</td><td class="total-val">$${total.toFixed(2)}</td></tr></tfoot></table>`; };
...
const periodRefunds = payments.filter(p => { if (!p.date || !p.isRefund || !isInPeriod(p.date)) return false; if (viewingCenter === 'both') return true; const mem = members.find(mm => mm.airtableId === p.memberRecId); if (mem) return mem.center && mem.center.toLowerCase().includes(viewingCenter); const vis = visitors.find(vv => vv.airtableId === p.memberRecId); if (vis) return vis.center && vis.center.toLowerCase().includes(viewingCenter); return false; });
                  ...
                  // Revenue calculations based on actual payments — pulls from Payments table AND Visitor purchase records
                   const monthStandardPayments = payments.filter(p => {
                    ...
                    const methodColors = { Cash: '#16a34a', Check: '#f59e0b', Card: '#8b5cf6', ACH: '#1080ad', Unknown: '#64748b', Other: '#94a3b8' };
...
${(function() { var classNames = ['Low-Impact Aerobics', 'Sit & Get Fit', 'Modified Sit & Get Fit', 'Low Impact Aerobics', 'Chair Class', 'Water Aerobics']; var classStats = classNames.map(function(cn) { var cv = monthVisits.filter(function(v) { return v.method === 'Class: ' + cn; }); if (cv.length === 0) return null; var days = {}; cv.forEach(function(v) { days[new Date(v.time).toDateString()] = true; }); var sess = Object.keys(days).length; return { name: cn, total: cv.length, sessions: sess, avg: sess > 0 ? Math.round(cv.length / sess) : 0 }; }).filter(Boolean); if (classStats.length === 0) return ''; var rows = classStats.map(function(c) { return '<tr><td class="stat-label">' + c.name + '</td><td class="stat-value">' + c.total + '</td><td class="stat-value">' + c.sessions + '</td><td class="stat-value">' + c.avg + '</td></tr>'; }).join(''); return '<div class="section-title">Class Attendance</div><table><thead><tr><th>Class Name</th><th style="text-align:right">Attendees</th><th style="text-align:right">Sessions</th><th style="text-align:right">Avg/Session</th></tr></thead><tbody>' + rows + '</tbody></table>'; })()}
...
<div class="footer"><span>Prepared by Patterson Health Center · Wellness Hub</span><span>${new Date().toLocaleDateString('en-US', {month:'long',day:'numeric',year:'numeric'})}</span></div>
                  ...
                  <button onClick={() => {
                    ...
                    if (activePaidMembers.length === 0) { alert('No completed payments found to report.'); return; }
                    const getPayMethod = (m) => { if (m.type.includes('CORPORATE') || m.sponsorName) { const sponsor = corporatePartners.find(cp => cp.sponsorMatch === m.sponsorName); if (sponsor) { const isMemHarper = m.center && m.center.toLowerCase().includes('harper'); const isMemAnthony = m.center && m.center.toLowerCase().includes('anthony'); const unifiedMatch = sponsor.paidMonths && sponsor.paidMonths.split(',').find(str => str.startsWith(reportMonth)); const harperMatch = sponsor.paidMonthsHarper && sponsor.paidMonthsHarper.split(',').find(str => str.startsWith(reportMonth)); const anthonyMatch = sponsor.paidMonthsAnthony && sponsor.paidMonthsAnthony.split(',').find(str => str.startsWith(reportMonth)); const match = unifiedMatch || (isMemHarper && harperMatch) || (isMemAnthony && anthonyMatch); if (match) return match.includes(':') ? match.split(':')[1] : 'Corp Payment'; } } var pm = m.paymentMethod || 'None Logged'; if (pm === 'Check' && m.checkNumber) pm = 'Check #' + m.checkNumber; return pm; };
                  ...
                  <div className="space-y-6">
                    <div>
                      <div className="flex justify-between items-end mb-2">
                        ...
                        <div className="text-right"><p className="text-xs font-bold text-slate-400 uppercase tracking-widest">Goal</p><p className="text-lg font-black text-slate-400">${viewingCenter === 'harper' ? '1,000' : viewingCenter === 'anthony' ? '4,000' : '5,000'}</p></div>
                      </div>
                    </div>
        ...
        {activeTab === 'cardqueue' && (() => {           const cancelledCardRequests = cardQueue.filter(c => c.status === 'Cancelled');           const pendingRequests = cardQueue.filter(c => c.status === 'Pending').sort((a, b) => new Date(b.requestedDate) - new Date(a.requestedDate));           const printedRequests = cardQueue.filter(c => c.status === 'Printed').sort((a, b) => new Date(b.printedDate || b.requestedDate) - new Date(a.printedDate || a.requestedDate)); const issuedRequests = cardQueue.filter(c => c.status === 'Issued').sort((a, b) => new Date(b.issuedDate || b.printedDate || b.requestedDate) - new Date(a.issuedDate || a.printedDate || a.requestedDate));           const handleMarkPrinted = async function(req) {             try {               const res = await fetch('/api/update-card-request', {                 method: 'POST',                 headers: { 'Content-Type': 'application/json' },                 body: JSON.stringify({ recordId: req.airtableId, status: 'Printed' })               });               const result = await res.json();               if (result.success) {                 setCardQueue(prev => prev.map(c => c.airtableId === req.airtableId ? Object.assign({}, c, { status: 'Printed', printedDate: new Date().toISOString().split('T')[0] }) : c));                 showToast('Marked as printed.', 'success', 3000);               } else {                 alert('Error: ' + (result.error || 'Could not update.'));               }             } catch (err) {               alert('Network error.');             }           }; const handleMarkIssued = async function(req) { try { const res = await fetch('/api/update-card-request', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ recordId: req.airtableId, status: 'Issued', issuedDate: new Date().toISOString().split('T')[0] }) }); const result = await res.json(); if (result.success) { setCardQueue(prev => prev.map(c => c.airtableId === req.airtableId ? Object.assign({}, c, { status: 'Issued', issuedDate: new Date().toISOString().split('T')[0] }) : c)); showToast('Marked as issued.', 'success', 3000); } else { alert('Error: ' + (result.error || 'Could not update.')); } } catch (err) { alert('Network error.'); } };
          const handleRevertToPending = async function(req) { try { const res = await fetch('/api/update-card-request', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ recordId: req.airtableId, status: 'Pending' }) }); const result = await res.json(); if (result.success) { setCardQueue(prev => prev.map(c => c.airtableId === req.airtableId ? Object.assign({}, c, { status: 'Pending', printedDate: '', issuedDate: '' }) : c)); showToast('Reverted to pending.', 'success', 3000); } else { alert('Error: ' + (result.error || 'Could not update.')); } } catch (err) { alert('Network error.'); } };
          const handleSetStatusNote = async function(req) {
            var entered = window.prompt('Add a note the requesting director will see on their dashboard.\n\nFor example: "Waiting on card stock" or "Printing Friday."', req.statusNote || '');
            if (entered === null) return;
            try {
              var res = await fetch('/api/update-card-request', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ recordId: req.airtableId, statusNote: entered.trim() }) });
              var result = await res.json();
              if (result.success) {
                setCardQueue(prev => prev.map(c => c.airtableId === req.airtableId ? Object.assign({}, c, { statusNote: entered.trim() }) : c));
                showToast(entered.trim() ? 'Note saved — the director will see it.' : 'Note cleared.', 'success', 3000);
              } else { alert('Error: ' + (result.error || 'Could not save note.')); }
            } catch (err) { alert('Network error.'); }
          };
          const handleChangePrintType = async function(req, newType) { try { const res = await fetch('/api/update-card-request', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ recordId: req.airtableId, printType: newType }) }); const result = await res.json(); if (result.success) { setCardQueue(prev => prev.map(c => c.airtableId === req.airtableId ? Object.assign({}, c, { printType: newType }) : c)); showToast('Changed to ' + newType + '.', 'success', 3000); } else { alert('Error: ' + (result.error || 'Could not update.')); } } catch (err) { alert('Network error.'); } };           const handlePrintNow = function(req) {             const m = members.find(mem => mem.airtableId === req.memberRecId);             if (!m) { alert('Member not found. Their record may have been deleted.'); return; }             const isHarper = m.center && m.center.toLowerCase().includes('harper');             const bgGradient = isHarper ? 'linear-gradient(135deg, #f59e0b, #dd6d22)' : 'linear-gradient(135deg, #1080ad, #003d6b)';             const qrColor = isHarper ? 'dd6d22' : '003d6b';             const qrUrl = 'https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=' + encodeURIComponent(m.id) + '&color=' + qrColor + '&bgcolor=ffffff';             const html = '<!DOCTYPE html><html><head><title>VIP Access Card - ' + m.firstName + ' ' + m.lastName + '</title><link href="https://fonts.googleapis.com/css2?family=Montserrat:wght@300;500;700;900&display=swap" rel="stylesheet"><style>@page{size:3.375in 2.125in;margin:0}@media print{*{-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}.card-page{page-break-after:always}}body{font-family:Montserrat,Arial,sans-serif;margin:0;padding:0;background:#fff}.card-page{width:3.375in;height:2.125in;display:flex;flex-direction:row;overflow:hidden;box-sizing:border-box;position:relative}.card-page.front{background:' + bgGradient + ';color:#fff}.qr-section{width:1.4in;height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:0.15in;box-sizing:border-box}.qr-box{background:#fff;padding:0.08in;border-radius:0.08in;display:flex;align-items:center;justify-content:center;box-shadow:0 2px 8px rgba(0,0,0,0.3)}.qr-img{width:1in;height:1in;display:block}.qr-label{font-size:7px;font-weight:700;color:rgba(255,255,255,0.85);text-transform:uppercase;letter-spacing:1.5px;margin-top:0.08in}.info-section{flex:1;padding:0 0.15in 0 0.1in;display:flex;flex-direction:column;justify-content:center;align-items:flex-start;z-index:10}.center-name{font-size:22px;font-weight:900;text-transform:uppercase;margin-bottom:0;line-height:1;text-shadow:0 2px 4px rgba(0,0,0,0.5);letter-spacing:1px}.center-sub{font-size:11px;font-weight:300;text-transform:uppercase;margin-top:4px;letter-spacing:3px;color:rgba(255,255,255,0.95);margin-bottom:0.12in}.member-id{font-size:9px;font-family:monospace;font-weight:900;background:rgba(0,0,0,0.3);padding:0.04in 0.08in;border-radius:0.04in;border:1px solid rgba(255,255,255,0.15);letter-spacing:1px}.card-page.back{background:#fff;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:0.2in;text-align:center}.back-header{font-size:11px;font-weight:900;color:#003d6b;text-transform:uppercase;letter-spacing:2px;margin-bottom:0.15in}.addresses{display:flex;gap:0.25in;justify-content:center;width:100%}.back-address{font-size:9px;color:#475569;line-height:1.5;text-align:center}.back-address strong{display:block;color:#003d6b;font-weight:900;font-size:10px;margin-bottom:3px;text-transform:uppercase;letter-spacing:0.5px}.back-footer{font-size:7px;color:#94a3b8;text-align:center;line-height:1.4;margin-top:0.18in;padding:0 0.1in}</style></head><body><div class="card-page front"><div class="qr-section"><div class="qr-box"><img class="qr-img" src="' + qrUrl + '" /></div><div class="qr-label">Scan to Enter</div></div><div class="info-section"><div class="center-name">' + (isHarper ? 'Harper' : 'Anthony') + '</div><div class="center-sub">Wellness Center</div><div class="member-id">ID: ' + m.id + '</div></div></div><div class="card-page back"><div class="back-header">If Found, Please Return</div><div class="addresses"><div class="back-address"><strong>Anthony Wellness</strong>309 W Main St<br/>Anthony, KS 67003<br/>(620) 842-5190</div><div class="back-address"><strong>Harper Wellness</strong>615 W 12th St<br/>Harper, KS 67058<br/>(620) 896-1202</div></div><div class="back-footer">This card is the property of Patterson Health Center. It is non-transferable and intended for the exclusive use of the member named on the reverse side. If found, please return to one of the addresses above. Thank you!</div></div></body></html>';             const w = window.open('', '_blank');             w.document.write(html);             w.document.close();             setTimeout(() => w.print(), 1000);           };           return (             <div className="space-y-6">               <div className="flex justify-between items-center mb-8">                 <div>                   <h2 className="text-3xl font-bold text-[#001f3f] tracking-tight">Card Print Queue</h2>                   <p className="text-slate-400 font-medium">Member cards requested by directors</p>                 </div>                 <div className="flex items-center gap-3">
          <button type="button" onClick={async () => {
                    const fobPending = pendingRequests.filter(r => r.printType === 'Fob' || r.printType === 'Both');
                    if (selectedFobs.length === 0) { alert('Check the boxes next to the fobs you want on this card first.'); return; }
                    const batch = pendingRequests.filter(function(r) { return selectedFobs.indexOf(r.airtableId) !== -1; }).slice(0, 3);
                    const memberBatch = batch.map(req => {
                      const m = members.find(mem => mem.airtableId === req.memberRecId);
                      return m ? { req, m } : null;
                    }).filter(Boolean);
                    if (memberBatch.length === 0) { alert('None of the selected fob requests have valid member records.'); return; }
if (memberBatch.length < 3 && !window.confirm('You selected ' + memberBatch.length + '. The other ' + (3 - memberBatch.length) + ' slot(s) will print blank, and that part of the card is wasted.\n\nPrint anyway?')) return;
                    if (!window.confirm('Print 3-up keytag card for ' + memberBatch.length + ' member(s)?\n\n' + memberBatch.map(x => '• ' + x.m.firstName + ' ' + x.m.lastName).join('\n') + '\n\nThis uses 1 blank 3-up keytag CR80 card.')) return;
                    const renderKeytag = (data) => {
                      if (!data) { return '<div class="keytag empty"><div class="empty-msg">Blank</div></div>'; }
                      const m = data.m;
                      ...
                      var tagName = (m.firstName || '') + (m.lastName ? ' ' + m.lastName.charAt(0).toUpperCase() + '.' : '');
                      var nameSize = tagName.length <= 10 ? 13 : tagName.length <= 13 ? 11.5 : tagName.length <= 16 ? 10 : tagName.length <= 20 ? 8.5 : 7.5;
                      return '<div class="keytag"><div class="accent-bar" style="background:' + accentColor + '"></div><div class="info"><div class="name" style="font-size:' + nameSize + 'px">' + tagName + '</div><div class="id">' + m.id + '</div><div class="center" style="color:' + accentColor + '">' + centerLabel + ' Wellness Center</div></div><img class="qr" src="' + qrUrl + '" /></div>';
                    };
                    const keytags = [0, 1, 2].map(i => renderKeytag(memberBatch[i])).join('');
                    const html = '<!DOCTYPE html><html><head><title>3-Up Keytags Batch</title><link href="https://fonts.googleapis.com/css2?family=Montserrat:wght@400;700;900&display=swap" rel="stylesheet"><style>@page{size:2.125in 3.375in;margin:0}@media print{*{-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}}body{font-family:Montserrat,Arial,sans-serif;margin:0;padding:0;background:#fff}.card{width:2.125in;height:3.375in;display:flex;flex-direction:column;box-sizing:border-box}.keytag{flex:1;display:flex;flex-direction:row;align-items:center;padding:0.04in 0.05in 0.04in 0.44in;box-sizing:border-box;position:relative;overflow:hidden}.keytag:last-child{border-bottom:none}.keytag.empty{background:#f8fafc;justify-content:center;align-items:center}.empty-msg{font-size:8px;color:#cbd5e1;text-transform:uppercase;letter-spacing:2px;font-weight:700}.accent-bar{position:absolute;left:0.32in;top:0.05in;bottom:0.05in;width:0.05in;border-radius:2px}.photo{width:0.55in;height:0.55in;object-fit:cover;border-radius:4px;flex-shrink:0;margin-right:0.08in}.photo.initials{display:flex;align-items:center;justify-content:center;color:#fff;font-weight:900;font-size:16px;letter-spacing:1px}.info{flex:1;display:flex;flex-direction:column;justify-content:center;overflow:hidden;padding-right:0.05in}.name{font-weight:900;color:#001f3f;line-height:1.1;white-space:nowrap;overflow:visible;letter-spacing:-0.3px;margin-bottom:1px}.id{font-family:monospace;font-size:10px;font-weight:700;color:#64748b;letter-spacing:0.5px}.center{font-size:6px;font-weight:900;letter-spacing:0.2px;margin-top:3px;white-space:nowrap}.qr{width:0.62in;height:0.62in;flex-shrink:0;margin-left:0.06in}</style></head><body><div class="card">' + keytags + '</div></body></html>';
                    ...
                    for (const item of memberBatch) {
                      try {
                        await fetch('/api/update-card-request', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ recordId: item.req.airtableId, status: 'Printed' }) });
                      } catch (err) { console.error('Failed to mark printed:', err); }
                    }
                    setCardQueue(prev => prev.map(c => memberBatch.some(x => x.req.airtableId === c.airtableId) ? Object.assign({}, c, { status: 'Printed', printedDate: new Date().toISOString().split('T')[0] }) : c));
                    setSelectedFobs([]);
                    showToast('Marked ' + memberBatch.length + ' as printed.', 'success', 3000);
                  }} className="bg-[#003d6b] text-white px-4 py-2 rounded-xl text-sm font-bold shadow-sm flex items-center gap-2 hover:bg-[#001f3f] transition-colors"><Printer size={14}/> Print 3-Up Keytags{selectedFobs.length ? ' (' + selectedFobs.length + ')' : ''}</button>
                  <button onClick={() => {
              const countStr = window.prompt('How many keytag cards need backs? (each card prints 3 tag backs)', '1');
              if (!countStr) return;
              const count = parseInt(countStr, 10);
              if (!count || count < 1) { alert('Enter a valid number.'); return; }
              const backTag = '<div class="kt-back"><img src="/wellness_corner_2.png" /><div class="biz">Property of Patterson Health Center</div><div class="ret">If found, please call:<br/>Harper Wellness Center 620-896-1202<br/>Anthony Wellness Center 620-842-5190</div></div>';
              let pages = '';
              for (let i = 0; i < count; i++) { pages += '<div class="card">' + backTag + backTag + backTag + '</div>'; }
              const html = '<!DOCTYPE html><html><head><title>Keytag Backs (' + count + ')</title><link href="https://fonts.googleapis.com/css2?family=Montserrat:wght@400;700;900&display=swap" rel="stylesheet"><style>@page{size:2.125in 3.375in;margin:0}@media print{*{-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}.card{page-break-after:always}}body{font-family:Montserrat,Arial,sans-serif;margin:0;padding:0;background:#fff}.card{width:2.125in;height:3.375in;display:flex;flex-direction:column;box-sizing:border-box}.kt-back{flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;box-sizing:border-box;padding:0.08in;overflow:hidden;transform:rotate(180deg)}.kt-back img{height:0.5in;max-width:88%;object-fit:contain;margin-bottom:0.04in}.biz{font-size:7px;font-weight:900;color:#003d6b;letter-spacing:0.3px;text-align:center;line-height:1.2}.ret{font-size:6px;font-weight:700;color:#64748b;letter-spacing:0.2px;text-align:center;line-height:1.3;margin-top:0.03in}</style></head><body>' + pages + '</body></html>';
              const w = window.open('', 'phcPrint');
              if (!w) { alert('Your browser blocked the print window. Please allow pop-ups for this site, then click print again.'); return; }
              w.document.open(); w.document.write(html); w.document.close(); w.focus();
              setTimeout(() => { try { w.print(); } catch (e) {} }, 1200);
            }} className="bg-white border border-slate-200 text-slate-600 px-4 py-2 rounded-xl text-sm font-bold shadow-sm flex items-center gap-2 hover:bg-slate-50 transition-colors"><Printer size={14}/> Print 3-Up Backs</button>
<button onClick={() => { const countStr = window.prompt('How many card backs do you need to print?', '1'); if (!countStr) return; const count = parseInt(countStr, 10); if (!count || count < 1) { alert('Enter a valid number.'); return; } let pages = ''; for (let i = 0; i < count; i++) { pages += '<div class="card-page back"><div class="back-header">If Found, Please Return</div><div class="addresses"><div class="back-address"><strong>Anthony Wellness</strong>309 W Main St<br/>Anthony, KS 67003<br/>(620) 842-5190</div><div class="back-address"><strong>Harper Wellness</strong>615 W 12th St<br/>Harper, KS 67058<br/>(620) 896-1202</div></div><div class="back-footer">This card is the property of Patterson Health Center. It is non-transferable and intended for the exclusive use of the member named on the reverse side. If found, please return to one of the addresses above. Thank you!</div></div>'; } const html = '<!DOCTYPE html><html><head><title>Card Backs Batch (' + count + ')</title><link href="https://fonts.googleapis.com/css2?family=Montserrat:wght@300;500;700;900&display=swap" rel="stylesheet"><style>@page{size:3.375in 2.125in;margin:0}@media print{*{-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}.card-page{page-break-after:always}}body{font-family:Montserrat,Arial,sans-serif;margin:0;padding:0;background:#fff}.card-page{width:3.375in;height:2.125in;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:0.2in;text-align:center;background:#fff;box-sizing:border-box}.back-header{font-size:11px;font-weight:900;color:#003d6b;text-transform:uppercase;letter-spacing:2px;margin-bottom:0.15in}.addresses{display:flex;gap:0.25in;justify-content:center;width:100%}.back-address{font-size:9px;color:#475569;line-height:1.5;text-align:center}.back-address strong{display:block;color:#003d6b;font-weight:900;font-size:10px;margin-bottom:3px;text-transform:uppercase;letter-spacing:0.5px}.back-footer{font-size:7px;color:#94a3b8;text-align:center;line-height:1.4;margin-top:0.18in;padding:0 0.1in}</style></head><body>' + pages + '</body></html>'; const w = window.open('', 'phcPrint'); if (!w) { alert('Your browser blocked the print window. Please allow pop-ups for this site, then click print again.'); return; } w.document.open(); w.document.write(html); w.document.close(); w.focus(); setTimeout(() => { try { w.print(); } catch (e) {} }, 800);}} className="bg-[#dba51f] text-white px-4 py-2 rounded-xl text-sm font-bold shadow-sm flex items-center gap-2 hover:bg-amber-600 transition-colors"><Printer size={14}/> Print Backs Batch</button><div className="bg-white px-4 py-2 border border-slate-200 rounded-xl text-sm font-bold text-slate-500 shadow-sm flex items-center gap-2"><Bell size={14} className="text-[#dba51f]"/> {pendingRequests.length} pending</div></div>               </div>                              {/* PENDING REQUESTS - SPLIT BY TYPE */}
              {(() => {
                const cardRequests = pendingRequests.filter(r => r.printType === 'Card' || r.printType === 'Both');
                const fobRequests = pendingRequests.filter(r => r.printType === 'Fob' || r.printType === 'Both');
                
                const renderRequestRow = (req, context) => {
                  const m = members.find(mem => mem.airtableId === req.memberRecId);
                  if (!m) {
                    return (
                      <div key={req.airtableId + '-' + context} className="bg-red-50 border border-red-200 rounded-xl p-4 flex items-center gap-4">
                        <AlertCircle size={32} className="text-red-400 shrink-0" />
                        <div className="flex-1 min-w-0">
                          <p className="font-bold text-red-700 text-sm">Orphan Request — No member linked</p>
                          <p className="text-[11px] font-bold text-red-500 uppercase tracking-widest">Requested by {req.requestedBy || 'Unknown'} • {req.requestedDate ? new Date(req.requestedDate).toLocaleDateString('en-US', {weekday: 'short', month: 'short', day: 'numeric'}) : 'Unknown date'}</p>
                          {req.note && (<p className="mt-2 text-xs font-medium text-red-800 bg-white border border-red-200 px-3 py-2 rounded-lg"><strong>Note:</strong> {req.note}</p>)}
                        </div>
                        <button onClick={() => handleMarkPrinted(req)} className="bg-red-50 text-red-700 px-4 py-2 rounded-xl text-xs font-bold hover:bg-red-100 transition-colors flex items-center gap-2 border border-red-200 shrink-0"><X size={14}/> Dismiss</button>
                      </div>
                    );
                  }
                  const isHarper = m.center && m.center.toLowerCase().includes('harper');
                  const typeColor = req.printType === 'Fob' ? 'bg-purple-100 text-purple-700' : req.printType === 'Both' ? 'bg-emerald-100 text-emerald-700' : 'bg-blue-100 text-blue-700';
                  return (
                    <div key={req.airtableId + '-' + context} className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-center gap-4">
                      <MemberPhoto src={m.photoUrl} name={m.firstName} size={48} />
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1 flex-wrap">
                          <p className="font-bold text-[#001f3f] text-base">{m.firstName} {m.lastName}</p>
                          <span className="px-2 py-0.5 rounded-full text-[9px] font-black bg-blue-100 text-blue-700 uppercase">{m.id}</span>
                          <span className={"px-2 py-0.5 rounded-full text-[9px] font-black uppercase " + (isHarper ? "bg-orange-100 text-orange-700" : "bg-blue-100 text-blue-700")}>{isHarper ? 'Harper' : 'Anthony'}</span>
                          <select value={req.printType || 'Card'} onChange={(e) => handleChangePrintType(req, e.target.value)} className={"px-2 py-0.5 rounded-full text-[9px] font-black uppercase border-0 cursor-pointer outline-none " + typeColor}><option value="Card">Card</option><option value="Fob">Fob</option><option value="Both">Both</option></select>
                        </div>
                        <p className="text-[11px] font-bold text-slate-500 uppercase tracking-widest">Requested by {req.requestedBy} • {req.requestedDate ? new Date(req.requestedDate).toLocaleDateString('en-US', {weekday: 'short', month: 'short', day: 'numeric'}) + ' at ' + new Date(req.requestedDate).toLocaleTimeString([], {hour: '2-digit', minute: '2-digit'}) : 'Unknown date'}</p>
                        {req.note && (<p className="mt-2 text-xs font-medium text-amber-800 bg-white border border-amber-200 px-3 py-2 rounded-lg"><strong>Note:</strong> {req.note}</p>)}
                      </div>
                      <div className="flex flex-col gap-2 shrink-0">
                        {context === 'card' && <button onClick={() => handlePrintNow(req)} className="bg-[#1080ad] text-white px-4 py-2 rounded-xl text-xs font-bold hover:bg-blue-700 transition-colors flex items-center gap-2 shadow-sm"><QrCode size={14}/> Print Card</button>}
                        <button onClick={() => handleMarkPrinted(req)} className="bg-green-50 text-green-700 px-4 py-2 rounded-xl text-xs font-bold hover:bg-green-100 transition-colors flex items-center gap-2 border border-green-200"><CheckCircle size={14}/> Mark Done</button>
                        <button onClick={() => handleSetStatusNote(req)} className="bg-white text-slate-500 px-4 py-2 rounded-xl text-xs font-bold hover:bg-slate-50 transition-colors flex items-center gap-2 border border-slate-200" title="Leave a note the requesting director can see">{req.statusNote ? '✎ Edit Note' : '+ Add Note'}</button>
                      </div>
                    </div>
                  );
                };
                  ...
                  <>
                    <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
                      <div className="flex items-center justify-between mb-4">
                        <h3 className="text-lg font-bold text-[#001f3f] flex items-center gap-2"><QrCode size={18} className="text-[#1080ad]"/> Pending Cards ({cardRequests.length})</h3>
                      </div>
                      {cardRequests.length === 0 ? (
                        <div className="text-center py-8 text-slate-400 font-medium italic">No pending card requests.</div>
                      ) : (
                        <div className="space-y-3">{cardRequests.map(req => renderRequestRow(req, 'card'))}</div>
                      )}
                    </div>
                    ...
                    <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200 mt-6">
                      ...
                      {cancelledCardRequests.length > 0 && (<div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 mt-6"><h3 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-3">Cancelled by a director ({cancelledCardRequests.length}) — do not print</h3><div className="space-y-2">{cancelledCardRequests.map(function(req) { var m = members.find(function(x) { return x.airtableId === req.memberRecId; }); return (<div key={req.airtableId} className="flex justify-between items-center bg-slate-50 border border-slate-200 rounded-xl p-3 opacity-70"><div><p className="font-bold text-slate-600 text-sm line-through">{m ? m.firstName + ' ' + m.lastName : 'Unknown'}</p><p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">{req.printType || 'Card'} · {req.statusNote || 'cancelled'}</p></div><button onClick={function() { handleRevertToPending(req); }} className="text-[11px] font-bold text-slate-400 hover:text-[#1080ad] underline">Restore</button></div>); })}</div></div>)}
                      {fobRequests.length === 0 ? (
                        <div className="text-center py-8 text-slate-400 font-medium italic">No pending fob requests.</div>
                      ) : (
                        <div className="space-y-3">{fobRequests.filter(function(r) { if (fobCenterFilter === 'all') return true; var m = members.find(function(x) { return x.airtableId === r.memberRecId; }); var isHarper = m && m.center && m.center.toLowerCase().includes('harper'); return fobCenterFilter === 'harper' ? isHarper : !isHarper; }).map(function(req) { var isSel = selectedFobs.indexOf(req.airtableId) !== -1; return (<div key={req.airtableId + '-fobrow'} className="flex items-start gap-3"><button type="button" onClick={function() { setSelectedFobs(function(prev) { return isSel ? prev.filter(function(id) { return id !== req.airtableId; }) : (prev.length >= 3 ? prev : prev.concat([req.airtableId])); }); }} className={"mt-6 w-7 h-7 shrink-0 rounded-lg border-2 flex items-center justify-center font-black text-sm transition-colors " + (isSel ? "bg-[#8b5cf6] border-[#8b5cf6] text-white" : "bg-white border-slate-300 text-transparent hover:border-[#8b5cf6]")} title={isSel ? "Remove from this card" : "Add to this card"}>✓</button><div className="flex-1 min-w-0">{renderRequestRow(req, 'fob')}</div></div>); })}</div>
                      )}
                    </div>
                  </>
                );
              })()}
                      {/* PRINTED REQUESTS */}               {printedRequests.length > 0 && (                 <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200">                   <h3 className="text-lg font-bold text-[#001f3f] mb-4 flex items-center gap-2"><CheckCircle size={18} className="text-[#16a34a]"/> Printed History ({printedRequests.length})</h3>                   <div className="space-y-2">                     {printedRequests.slice(0, 30).map(req => {                       const m = members.find(mem => mem.airtableId === req.memberRecId);                       const memberName = m ? (m.firstName + ' ' + m.lastName) : 'Unknown member';                       return (                         <div key={req.airtableId} className="bg-slate-50 border border-slate-100 rounded-xl p-3 flex items-center gap-4">                           <CheckCircle size={16} className="text-[#16a34a] shrink-0" />                           <div className="flex-1 min-w-0">                             <p className="font-bold text-slate-700 text-sm">{memberName}</p>                             <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Requested by {req.requestedBy} • Printed {req.printedDate ? new Date(req.printedDate + 'T00:00:00').toLocaleDateString('en-US', {month: 'short', day: 'numeric', year: 'numeric'}) : 'unknown'}</p>                           </div>                           <button onClick={() => handleMarkIssued(req)} className="shrink-0 bg-[#1080ad] text-white px-3 py-1.5 rounded-lg text-[11px] font-bold hover:bg-[#0d6d94] transition-colors">Mark Issued</button>
                          <button onClick={() => handleRevertToPending(req)} className="shrink-0 bg-amber-50 text-amber-700 px-3 py-1.5 rounded-lg text-[11px] font-bold hover:bg-amber-100 transition-colors border border-amber-200">Revert to Pending</button>                         </div>                       );                     })}                     {printedRequests.length > 30 && <p className="text-xs text-slate-400 text-center mt-3 font-bold">Showing last 30 printed requests</p>}                   </div>                 </div>               )}               {issuedRequests.length > 0 && (                 <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200 mt-6">                   <h3 className="text-lg font-bold text-[#001f3f] mb-4 flex items-center gap-2"><CreditCard size={18} className="text-[#1080ad]"/> Issued to Members ({issuedRequests.length})</h3>                   <div className="space-y-2">                     {issuedRequests.slice(0, 30).map(req => {                       const m = members.find(mem => mem.airtableId === req.memberRecId);                       const memberName = m ? (m.firstName + ' ' + m.lastName) : 'Unknown member';                       return (                         <div key={req.airtableId} className="bg-blue-50 border border-blue-100 rounded-xl p-3 flex items-center gap-4">                           <CreditCard size={16} className="text-[#1080ad] shrink-0" />                           <div className="flex-1 min-w-0">                             <p className="font-bold text-slate-700 text-sm">{memberName}</p>                             <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Issued {req.issuedDate ? new Date(req.issuedDate + 'T00:00:00').toLocaleDateString('en-US', {month: 'short', day: 'numeric', year: 'numeric'}) : ''}</p>                           </div>                           <span className="shrink-0 text-[10px] font-black text-[#1080ad] bg-blue-100 px-3 py-1 rounded-full uppercase tracking-wider">Issued</span>
                          <button onClick={() => handleRevertToPending(req)} className="shrink-0 bg-amber-50 text-amber-700 px-3 py-1.5 rounded-lg text-[11px] font-bold hover:bg-amber-100 transition-colors border border-amber-200">Revert to Pending</button>                         </div>                       );                     })}                     {issuedRequests.length > 30 && <p className="text-xs text-slate-400 text-center mt-3 font-bold">Showing last 30 issued cards</p>}                   </div>                 </div>               )}             </div>           );         })()}          {activeTab === 'mycards' && (() => {
          var mine = cardQueue.filter(function(c) {
          ...
          }).sort(function(a, b) { return new Date(b.requestedDate || 0) - new Date(a.requestedDate || 0); });
          var groups = [
          ...
          var markIssued = async function(req) {
            try {
              ...
              var result = await res.json();
              if (result.success) {
                setCardQueue(prev => prev.map(c => c.airtableId === req.airtableId ? Object.assign({}, c, { status: 'Issued', issuedDate: new Date().toISOString().split('T')[0] }) : c));
                showToast('Marked as given to the member.', 'success', 3000);
              } else { alert('Error: ' + (result.error || 'Could not update.')); }
            } catch (err) { alert('Network error.'); }
          };
          return (
            <div className="space-y-6">
              <div className="flex justify-between items-center mb-8">
                <div>
                  <h2 className="text-3xl font-bold text-[#001f3f] tracking-tight">Card Queue</h2>
                  <p className="text-slate-400 font-medium">Cards and fobs requested for your center. Kristen prints and delivers them.</p>
                </div>
                <div className="bg-white px-4 py-2 border border-slate-200 rounded-xl text-sm font-bold text-slate-500 shadow-sm flex items-center gap-2"><QrCode size={14} className="text-[#dba51f]"/> {mine.length} total</div>
              </div>
              {mine.length === 0 ? (
                <div className="bg-white p-12 rounded-2xl shadow-sm border border-slate-200 text-center">
                  <QrCode size={40} className="text-slate-200 mx-auto mb-4" />
                  <p className="text-lg font-black text-slate-400 mb-1">No card requests yet.</p>
                  <p className="text-sm text-slate-400">Open a member's profile and click <strong>Request Card</strong> to add one here.</p>
                </div>
              ) : groups.map(function(g) {
                var list = mine.filter(function(c) { return c.status === g.key; });
                if (list.length === 0) return null;
                return (
                  <div key={g.key} className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
                    <h3 className="text-lg font-bold text-[#001f3f] flex items-center gap-2 mb-1"><span className="w-2.5 h-2.5 rounded-full" style={{background: g.color}}></span> {g.title} ({list.length})</h3>
                    <p className="text-xs text-slate-400 font-medium mb-4">{g.blurb}</p>
                    <div className="space-y-3">
                      {list.map(function(req) {
                        var m = members.find(function(x) { return x.airtableId === req.memberRecId; });
                        var isHarper = m && m.center && m.center.toLowerCase().includes('harper');
                        return (
                          <div key={req.airtableId} className={'rounded-xl p-4 flex items-center gap-4 border ' + g.bg + ' ' + g.border}>
                            {m ? <MemberPhoto src={m.photoUrl} name={m.firstName} size={44} /> : <AlertCircle size={28} className="text-slate-300 shrink-0" />}
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2 mb-1 flex-wrap">
                                <p className="font-bold text-[#001f3f]">{m ? m.firstName + ' ' + m.lastName : 'Member record missing'}</p>
                                {m && <span className="px-2 py-0.5 rounded-full text-[9px] font-black bg-slate-100 text-slate-500 uppercase">{m.id}</span>}
                                <span className="px-2 py-0.5 rounded-full text-[9px] font-black bg-purple-100 text-purple-700 uppercase">{req.printType || 'Card'}</span>
                                {m && <span className={'px-2 py-0.5 rounded-full text-[9px] font-black uppercase ' + (isHarper ? 'bg-orange-100 text-orange-700' : 'bg-blue-100 text-blue-700')}>{isHarper ? 'Harper' : 'Anthony'}</span>}
                              </div>
                              <p className="text-[11px] font-bold text-slate-500 uppercase tracking-widest">Requested by {req.requestedBy || 'Unknown'} · {req.requestedDate ? new Date(req.requestedDate).toLocaleDateString('en-US', {month: 'short', day: 'numeric', year: 'numeric'}) : 'no date'}{req.printedDate ? ' · printed ' + new Date(req.printedDate + 'T00:00:00').toLocaleDateString('en-US', {month: 'short', day: 'numeric'}) : ''}{req.issuedDate ? ' · issued ' + new Date(req.issuedDate + 'T00:00:00').toLocaleDateString('en-US', {month: 'short', day: 'numeric'}) : ''}</p>
                              {req.note && <p className="mt-2 text-xs font-medium text-slate-600 bg-white border border-slate-200 px-3 py-2 rounded-lg"><strong>Your note:</strong> {req.note}</p>}
                              {req.statusNote && <p className="mt-2 text-xs font-bold text-[#dba51f] bg-white border border-amber-200 px-3 py-2 rounded-lg">Kristen: {req.statusNote}</p>}
                            </div>
        ...
        {activeTab === 'notif' && (<div className="space-y-6"><div className="flex justify-between items-center mb-8"><div><h2 className="text-3xl font-bold text-[#001f3f] tracking-tight">Notifications</h2><p className="text-slate-400 font-medium">Payment reminders</p></div><button className="bg-[#dd6d22] text-white px-8 py-3 rounded-xl font-bold shadow-lg flex items-center gap-2"><Bell size={20}/> Send All Due</button></div><ProListCard title="Due for Reminder"><div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden mt-4"><table className="w-full text-left border-collapse"><thead className="bg-slate-50 text-[11px] font-black text-slate-400 uppercase tracking-widest border-b"><tr><th className="px-8 py-4 w-64">Member</th><th className="px-8 py-4 w-40">Type</th><th className="px-8 py-4 w-32">Status</th><th className="px-8 py-4 w-32">Due</th><th className="px-8 py-4 w-24">Actions</th></tr></thead><tbody className="text-sm">{scopedMembers.filter(m => m.status !== 'ACTIVE').map(m => (<tr key={m.id} className="border-b"><td className="px-8 py-5"><p className="font-bold text-slate-800">{m.firstName} {m.lastName}</p><p className="text-[11px] text-slate-400">{m.email}</p></td><td className="px-8 py-5"><span className="px-3 py-1 rounded-full bg-blue-50 text-blue-600 text-[10px] font-black">{m.type}</span></td><td className="px-8 py-5"><span className={`px-3 py-1 rounded-full text-[10px] font-black ${m.status==='OVERDUE'?'bg-red-100 text-red-600':'bg-amber-100 text-amber-600'}`}>{m.status}</span></td><td className="px-8 py-5 text-slate-600 font-medium">{m.nextPayment}</td><td className="px-8 py-5 flex gap-2"><button className="p-2 bg-[#1080ad] text-white rounded-lg shadow-md"><Mail size={16}/></button><button className="p-2 bg-[#dd6d22] text-white rounded-lg shadow-md"><Phone size={16}/></button></td></tr>))}</tbody></table></div></ProListCard>{(() => { const uniqueByMember = new Map(); blockedCheckins.filter(b => b.outcome === 'Pending' && b.reason !== 'Cooldown (2-hr)').forEach(b => { const mem = members.find(m => m.airtableId === b.memberRecId); if (viewingCenter !== 'both' && b.center && b.center.toLowerCase() !== viewingCenter.toLowerCase()) return; const key = b.memberRecId || b.memberName; if (!uniqueByMember.has(key) || new Date(b.timestamp) > new Date(uniqueByMember.get(key).timestamp)) { uniqueByMember.set(key, b); } }); const flagged = Array.from(uniqueByMember.values()).sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp)); const resolved = showResolvedFlags ? blockedCheckins.filter(b => b.outcome && b.outcome !== 'Pending' && b.reason !== 'Cooldown (2-hr)' && (viewingCenter === 'both' || !b.center || b.center.toLowerCase() === viewingCenter.toLowerCase())).sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp)).slice(0, 50) : []; if (flagged.length === 0 && !showResolvedFlags) return null; const dismissRecord = async (recordId, memberRecId) => { try { var allPending = blockedCheckins.filter(b => b.outcome === 'Pending' && (b.memberRecId === memberRecId || b.airtableId === recordId)); var promises = allPending.map(function(b) { return fetch('/api/update-blocked-checkin', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ recordId: b.airtableId, outcome: 'Dismissed by Staff' }) }).then(function(r) { return r.json(); }); }); var results = await Promise.all(promises); setBlockedCheckins(prev => prev.map(b => allPending.some(p => p.airtableId === b.airtableId) ? Object.assign({}, b, { outcome: 'Dismissed by Staff' }) : b)); showToast('Dismissed ' + allPending.length + ' flag' + (allPending.length !== 1 ? 's' : '') + '.', 'success', 3000); } catch (err) { alert('Network error.'); } }; return (<div className="mt-8"><div className="flex items-center justify-between mb-2"><h3 className="text-lg font-bold text-[#001f3f]">Blocked at Kiosk ({flagged.length} active)</h3><label className="flex items-center gap-2 cursor-pointer"><input type="checkbox" checked={showResolvedFlags} onChange={(e) => setShowResolvedFlags(e.target.checked)} className="w-4 h-4 accent-[#1080ad]" /><span className="text-sm font-bold text-slate-500">Show resolved</span></label></div>{flagged.length > 0 && (<div className="bg-white rounded-2xl shadow-sm border border-red-200 overflow-hidden"><table className="w-full text-left border-collapse"><thead className="bg-red-50 text-[11px] font-black text-red-500 uppercase tracking-widest border-b border-red-200"><tr><th className="px-8 py-4 w-64">Member</th><th className="px-8 py-4 w-40">Center</th><th className="px-8 py-4">Reason</th><th className="px-8 py-4 w-40">Last Attempt</th><th className="px-8 py-4 w-36">Actions</th></tr></thead><tbody className="text-sm">{flagged.map(b => { const mem = members.find(m => m.airtableId === b.memberRecId); return (<tr key={b.airtableId} className="border-b hover:bg-red-50/40 cursor-pointer" onClick={() => mem && setSelectedMember(mem)}><td className="px-8 py-5"><p className="font-bold text-slate-800">{b.memberName}</p><p className="text-[11px] text-slate-400 font-mono">{b.memberId || '—'}</p></td><td className="px-8 py-5"><span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase ${b.center === 'Harper' ? 'bg-orange-100 text-orange-700' : 'bg-blue-100 text-blue-700'}`}>{b.center}</span></td><td className="px-8 py-5"><span className={'px-2 py-1 rounded text-[10px] font-black uppercase tracking-tight ' + (b.reason === 'Past Due Payment' ? 'bg-red-100 text-red-700' : b.reason === 'Inactive Membership' ? 'bg-slate-200 text-slate-700' : ((b.reason || '').indexOf('No Passes') !== -1 ? 'bg-orange-100 text-orange-700' : 'bg-amber-100 text-amber-700'))}>{b.reason}</span></td><td className="px-8 py-5 text-slate-600 font-medium text-xs">{b.timestamp ? new Date(b.timestamp).toLocaleDateString('en-US', {month: 'short', day: 'numeric', year: 'numeric'}) + ' ' + new Date(b.timestamp).toLocaleTimeString([], {hour: '2-digit', minute: '2-digit'}) : '—'}</td><td className="px-8 py-5 flex gap-2"><button onClick={(e) => { e.stopPropagation(); mem && setSelectedMember(mem); }} className="p-2 bg-[#dba51f] text-white rounded-lg shadow-md" title="Open profile"><CheckCircle size={16}/></button><button onClick={(e) => { e.stopPropagation(); logVisitForBlocked(mem, b.airtableId, b.memberName, b.timestamp); }} className="px-3 py-2 bg-[#1080ad] text-white rounded-lg shadow-md hover:bg-[#0d6d94] text-[11px] font-black uppercase tracking-tight" title="Log a visit - they were let in anyway">Log Visit</button><button onClick={(e) => { e.stopPropagation(); if (window.confirm('Dismiss this flag for ' + b.memberName + '?')) dismissRecord(b.airtableId, b.memberRecId); }} className="p-2 bg-slate-200 text-slate-600 rounded-lg shadow-md hover:bg-slate-300" title="Dismiss"><X size={16}/></button></td></tr>); })}</tbody></table></div>)}{showResolvedFlags && (<div className="mt-4"><h4 className="text-sm font-bold text-slate-400 uppercase tracking-widest mb-2">Resolved ({resolved.length}{resolved.length === 50 ? '+' : ''})</h4>{resolved.length === 0 ? (<p className="text-sm text-slate-400 italic">No resolved flags found.</p>) : (<div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden"><table className="w-full text-left border-collapse"><thead className="bg-slate-50 text-[11px] font-black text-slate-400 uppercase tracking-widest border-b"><tr><th className="px-8 py-3">Member</th><th className="px-8 py-3">Center</th><th className="px-8 py-3">Reason</th><th className="px-8 py-3">When</th><th className="px-8 py-3">Outcome</th></tr></thead><tbody className="text-sm">{resolved.map(b => (<tr key={b.airtableId} className="border-b hover:bg-slate-50/40"><td className="px-8 py-3 font-bold text-slate-500">{b.memberName}</td><td className="px-8 py-3"><span className={`px-2 py-0.5 rounded-full text-[9px] font-black uppercase ${b.center === 'Harper' ? 'bg-orange-50 text-orange-500' : 'bg-blue-50 text-blue-500'}`}>{b.center}</span></td><td className="px-8 py-3"><span className="text-[10px] font-bold text-slate-400">{b.reason}</span></td><td className="px-8 py-3 text-slate-400 text-xs">{b.timestamp ? new Date(b.timestamp).toLocaleDateString('en-US', {month: 'short', day: 'numeric'}) + ' ' + new Date(b.timestamp).toLocaleTimeString([], {hour: '2-digit', minute: '2-digit'}) : '—'}</td><td className="px-8 py-3"><span className="px-2 py-1 rounded text-[10px] font-black bg-green-100 text-green-700">{b.outcome}</span></td></tr>))}</tbody></table></div>)}</div>)}</div>); })()}{(() => { const today = new Date().toLocaleDateString('en-CA'); const turned18 = scopedMembers.filter(m => m.isMinor && m.turns18Date && m.turns18Date <= today).sort((a, b) => (b.turns18Date || '').localeCompare(a.turns18Date || '')); if (turned18.length === 0) return null; const clearMinorFlag = async (member) => { try { const res = await fetch('/api/update-onboarding', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ airtableId: member.airtableId, fields: { 'Is Minor': false, 'Turns 18 Date': null } }) }); const result = await res.json(); if (result.success) { setMembers(prev => prev.map(m => m.airtableId === member.airtableId ? { ...m, isMinor: false, turns18Date: '' } : m)); showToast(member.firstName + ' marked as adult. Remember to update paperwork.', 'success', 4000); } else { alert('Failed to update: ' + (result.error || 'Unknown error')); } } catch (err) { alert('Network error.'); } }; return (<div className="mt-8"><ProListCard title={`Reached Age 18 — Paperwork Renewal Needed (${turned18.length})`}><div className="bg-white rounded-2xl shadow-sm border border-orange-200 overflow-hidden mt-4"><table className="w-full text-left border-collapse"><thead className="bg-orange-50 text-[11px] font-black text-orange-600 uppercase tracking-widest border-b border-orange-200"><tr><th className="px-8 py-4 w-64">Member</th><th className="px-8 py-4 w-40">Type</th><th className="px-8 py-4">Turned 18</th><th className="px-8 py-4 w-40">Days Ago</th><th className="px-8 py-4 w-36">Actions</th></tr></thead><tbody className="text-sm">{turned18.map(m => { const daysAgo = Math.floor((new Date(today + 'T00:00:00') - new Date(m.turns18Date + 'T00:00:00')) / (1000 * 60 * 60 * 24)); return (<tr key={m.id} className="border-b hover:bg-orange-50/40 cursor-pointer" onClick={() => setSelectedMember(m)}><td className="px-8 py-5"><p className="font-bold text-slate-800">{m.firstName} {m.lastName}</p><p className="text-[11px] text-slate-400 font-mono">{m.id}</p></td><td className="px-8 py-5"><span className="px-3 py-1 rounded-full bg-blue-50 text-blue-600 text-[10px] font-black">{m.type}</span></td><td className="px-8 py-5 text-slate-600 font-medium text-xs">{new Date(m.turns18Date + 'T00:00:00').toLocaleDateString('en-US', {month: 'short', day: 'numeric', year: 'numeric'})}</td><td className="px-8 py-5"><span className={`px-2 py-1 rounded text-[10px] font-black uppercase ${daysAgo > 30 ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700'}`}>{daysAgo === 0 ? 'Today' : daysAgo + ' day' + (daysAgo !== 1 ? 's' : '') + ' ago'}</span></td><td className="px-8 py-5 flex gap-2"><button onClick={(e) => { e.stopPropagation(); setSelectedMember(m); }} className="p-2 bg-[#dba51f] text-white rounded-lg shadow-md" title="Open profile"><CheckCircle size={16}/></button><button onClick={(e) => { e.stopPropagation(); if (window.confirm('Mark ' + m.firstName + ' ' + m.lastName + ' as an adult? This will clear the Minor flag. Make sure their new adult paperwork is complete.')) clearMinorFlag(m); }} className="p-2 bg-orange-500 text-white rounded-lg shadow-md hover:bg-orange-600" title="Mark as adult"><X size={16}/></button></td></tr>); })}</tbody></table></div></ProListCard></div>); })()}</div>)}

        {activeTab === 'badge' && (<div className="space-y-6"><div className="mb-8"><h2 className="text-3xl font-bold text-[#001f3f] tracking-tight mb-1">Staff Check-In</h2><p className="text-slate-400 font-medium">Log a check-in manually or via scanner. Use custom date/time for after-hours or backdated entries.</p></div><div className="flex gap-8"><div className="bg-white p-12 rounded-3xl shadow-sm border border-slate-200 flex-1 text-center"><div className="flex items-center justify-center gap-3 mb-6"><label className="flex items-center gap-2 bg-blue-50 border border-blue-200 rounded-xl px-4 py-3 cursor-pointer hover:bg-blue-100 transition-colors"><input type="checkbox" id="customTimeCheck" onChange={function(e) { var dateRow = document.getElementById('customTimeRow'); if (dateRow) dateRow.style.display = e.target.checked ? 'flex' : 'none'; }} className="w-4 h-4 rounded border-slate-300 text-[#1080ad]" /><span className="text-xs font-bold text-[#1080ad] uppercase tracking-widest">Custom Date/Time</span></label></div><div id="customTimeRow" style={{display: 'none'}} className="flex items-center justify-center gap-3 mb-6"><input type="date" id="customDate" defaultValue={new Date().toISOString().split('T')[0]} className="p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-[#1080ad] text-sm font-bold text-[#001f3f]" /><input type="time" id="customTime" defaultValue={new Date().toLocaleTimeString('en-US', {hour12: false, hour: '2-digit', minute: '2-digit'})} className="p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-[#1080ad] text-sm font-bold text-[#001f3f]" /></div><p className="text-sm font-bold text-slate-400 mb-4">Enter Name or ID <span className="text-slate-300">(members and visitors)</span>:</p><div className="relative w-full max-w-sm mx-auto mb-10"><div className="flex gap-4"><input className="flex-1 p-4 border rounded-xl outline-none text-xl text-center bg-slate-100 focus:border-[#1080ad] focus:bg-white transition-colors" placeholder="e.g. Smith" value={kioskInput} onChange={(e) => setKioskInput(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') { var useCustom = document.getElementById('customTimeCheck') && document.getElementById('customTimeCheck').checked; if (useCustom) { var cd = document.getElementById('customDate').value; var ct = document.getElementById('customTime').value; var customISO = new Date(cd + 'T' + ct + ':00').toISOString(); var id = kioskInput.toUpperCase().trim(); var m = members.find(function(mem) { return mem.id === id || (mem.firstName + ' ' + mem.lastName).toLowerCase().includes(kioskInput.toLowerCase()); }); if (m) { var scanCenter = viewingCenter === 'both' ? m.center : viewingCenter.charAt(0).toUpperCase() + viewingCenter.slice(1); var dupCheck = visits.some(function(v) { return v.name === (m.firstName + ' ' + m.lastName) && Math.abs(new Date(customISO) - new Date(v.time)) < 30 * 60 * 1000; }); if (dupCheck) { showToast(m.firstName + ' already has a check-in near that time.', 'error', 4000); setKioskInput(''); return; } fetch('/api/visits', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ airtableId: m.airtableId, center: scanCenter, time: customISO, method: 'Director Override (Backdated)' }) }).then(function(r) { return r.json(); }).then(function(result) { if (result.success) { var backdatedDate = new Date(customISO).toDateString(); var todayDate = new Date().toDateString(); setVisits(function(prev) { var updated = [{ name: m.firstName + ' ' + m.lastName, center: scanCenter, time: customISO, type: m.type, method: 'Director Override (Backdated)' }].concat(prev); updated.sort(function(a, b) { return new Date(b.time) - new Date(a.time); }); return updated; }); setMembers(function(prev) { return prev.map(function(mem) { return mem.id === m.id ? Object.assign({}, mem, { visits: mem.visits + 1 }) : mem; }); }); showToast(m.firstName + ' checked in for ' + new Date(customISO).toLocaleDateString() + ' at ' + new Date(customISO).toLocaleTimeString([], {hour: '2-digit', minute: '2-digit'}), 'success', 5000); } }); } else { var vq = kioskInput.toLowerCase().trim(); var vv = visitors.find(function(x) { return ((x.firstName || '') + ' ' + (x.lastName || '')).toLowerCase().indexOf(vq) !== -1; }); if (vv) { logVisitorVisitAt(vv, customISO); } else { setKioskMessage({ text: 'Not found', type: 'error', subtext: 'No member or visitor by that name.' }); setTimeout(function() { setKioskMessage({ text: '', type: '', subtext: '' }); }, 3000); } } setKioskInput(''); } else { processCheckIn(kioskInput, "Staff Scan/Entry"); setKioskInput(''); } } }} /><button onClick={() => { var useCustom = document.getElementById('customTimeCheck') && document.getElementById('customTimeCheck').checked; if (useCustom) { var cd = document.getElementById('customDate').value; var ct = document.getElementById('customTime').value; var customISO = new Date(cd + 'T' + ct + ':00').toISOString(); var id = kioskInput.toUpperCase().trim(); var m = members.find(function(mem) { return mem.id === id || (mem.firstName + ' ' + mem.lastName).toLowerCase().includes(kioskInput.toLowerCase()); }); if (m) { var scanCenter = viewingCenter === 'both' ? m.center : viewingCenter.charAt(0).toUpperCase() + viewingCenter.slice(1); var dupCheck = visits.some(function(v) { return v.name === (m.firstName + ' ' + m.lastName) && Math.abs(new Date(customISO) - new Date(v.time)) < 30 * 60 * 1000; }); if (dupCheck) { showToast(m.firstName + ' already has a check-in near that time.', 'error', 4000); setKioskInput(''); return; } fetch('/api/visits', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ airtableId: m.airtableId, center: scanCenter, time: customISO, method: 'Director Override (Backdated)' }) }).then(function(r) { return r.json(); }).then(function(result) { if (result.success) { var backdatedDate = new Date(customISO).toDateString(); var todayDate = new Date().toDateString(); setVisits(function(prev) { var updated = [{ name: m.firstName + ' ' + m.lastName, center: scanCenter, time: customISO, type: m.type, method: 'Director Override (Backdated)' }].concat(prev); updated.sort(function(a, b) { return new Date(b.time) - new Date(a.time); }); return updated; }); setMembers(function(prev) { return prev.map(function(mem) { return mem.id === m.id ? Object.assign({}, mem, { visits: mem.visits + 1 }) : mem; }); }); showToast(m.firstName + ' checked in for ' + new Date(customISO).toLocaleDateString() + ' at ' + new Date(customISO).toLocaleTimeString([], {hour: '2-digit', minute: '2-digit'}), 'success', 5000); } }); } else { var vq = kioskInput.toLowerCase().trim(); var vv = visitors.find(function(x) { return ((x.firstName || '') + ' ' + (x.lastName || '')).toLowerCase().indexOf(vq) !== -1; }); if (vv) { logVisitorVisitAt(vv, customISO); } else { setKioskMessage({ text: 'Not found', type: 'error', subtext: 'No member or visitor by that name.' }); setTimeout(function() { setKioskMessage({ text: '', type: '', subtext: '' }); }, 3000); } } setKioskInput(''); } else { processCheckIn(kioskInput, "Staff Scan/Entry"); setKioskInput(''); } }} className="bg-[#001f3f] text-white px-8 rounded-xl font-bold hover:bg-blue-900 transition-colors shadow-sm">Check In</button></div>{kioskMatches.length > 0 && (<div className="absolute top-full left-0 w-full mt-2 bg-white border border-slate-200 rounded-xl shadow-2xl z-50 overflow-hidden text-left">{kioskMatches.map(m => (<button key={m._type + (m.airtableId || m.id)} onClick={function() { var useCustom = document.getElementById('customTimeCheck') && document.getElementById('customTimeCheck').checked; if (useCustom) { var cd = document.getElementById('customDate').value; var ct = document.getElementById('customTime').value; var customISO = new Date(cd + 'T' + ct + ':00').toISOString(); var dupCheck = visits.some(function(v) { return v.name === (m.firstName + ' ' + m.lastName) && Math.abs(new Date(customISO) - new Date(v.time)) < 30 * 60 * 1000; }); if (dupCheck) { showToast(m.firstName + ' already has a check-in near that time.', 'error', 4000); setKioskInput(''); return; } var scanCenter = viewingCenter === 'both' ? m.center : viewingCenter.charAt(0).toUpperCase() + viewingCenter.slice(1); fetch('/api/visits', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ airtableId: m.airtableId, center: scanCenter, time: customISO, method: 'Director Override (Backdated)' }) }).then(function(r) { return r.json(); }).then(function(result) { if (result.success) { var backdatedDate = new Date(customISO).toDateString(); var todayDate = new Date().toDateString(); setVisits(function(prev) { var updated = [{ name: m.firstName + ' ' + m.lastName, center: scanCenter, time: customISO, type: m.type, method: 'Director Override (Backdated)' }].concat(prev); updated.sort(function(a, b) { return new Date(b.time) - new Date(a.time); }); return updated; }); setMembers(function(prev) { return prev.map(function(mem) { return mem.id === m.id ? Object.assign({}, mem, { visits: mem.visits + 1 }) : mem; }); }); showToast(m.firstName + ' checked in for ' + new Date(customISO).toLocaleDateString() + ' at ' + new Date(customISO).toLocaleTimeString([], {hour: '2-digit', minute: '2-digit'}), 'success', 5000); } }); setKioskInput(''); } else { setKioskInput(m.firstName + ' ' + m.lastName); } }} className="w-full p-4 border-b border-slate-100 last:border-0 hover:bg-blue-50 transition-colors flex justify-between items-center group"><div><p className="font-bold text-[#001f3f] text-lg">{m.firstName} {m.lastName}</p><p className="text-[10px] text-slate-400 uppercase tracking-widest">{m.phone||'No Phone'}</p></div><div className="bg-[#1080ad] text-white px-3 py-1 rounded-lg text-xs font-bold shadow-sm group-hover:scale-105 transition-transform">Select</div></button>))}</div>)}</div>{kioskMessage.text && (<div className={`mt-8 p-4 rounded-xl text-center font-bold text-lg ${kioskMessage.type==='success'?'bg-green-100 text-green-700':kioskMessage.type==='warning'?'bg-yellow-100 text-yellow-700':'bg-red-100 text-red-700'}`}>{kioskMessage.text}{kioskMessage.subtext && <p className="text-sm mt-1">{kioskMessage.subtext}</p>}{kioskMessage.cooldownBlocked && (<button onClick={() => { if (!window.confirm('Override the 2-hour cooldown and log a second visit for ' + kioskMessage.memberName + '?\n\nThis is for situations where the member legitimately needs to be checked in again (e.g., they had to leave briefly and came back).')) return; processCheckIn(kioskMessage.memberId, 'Director Override (Cooldown Bypass)', true); }} className="mt-4 bg-yellow-600 text-white px-6 py-2 rounded-xl text-sm font-bold hover:bg-yellow-700 transition-colors shadow-sm">Override & Check In Anyway</button>)}{kioskMessage.onboardingBlocked && (<button onClick={() => { if (!window.confirm('Override the onboarding block and check in ' + kioskMessage.memberName + ' anyway?\n\nUse this only if the member has actually completed orientation/paperwork but the system was not updated yet.')) return; processCheckIn(kioskMessage.memberId, 'Director Override (Onboarding Bypass)', true); }} className="mt-4 bg-yellow-600 text-white px-6 py-2 rounded-xl text-sm font-bold hover:bg-yellow-700 transition-colors shadow-sm">Override & Check In Anyway</button>)}</div>)}</div></div></div>)}
        ...
        {activeTab === 'help' && (() => {
          const faqs = [
            { category: "Membership Management", icon: <Users size={20} className="text-[#1080ad]" />, items: [
                ...
                { id: 'm3', q: "How do I log a member's payment?", a: "Open the member's profile. Click the green 'Log Payment' button at the bottom. Choose the amount (Full Month, $16 Rest of Month, $5 Day Pass, or enter a Custom Amount). Then select the payment method (Cash, Check, Card, or ACH). For <b>Check payments</b>, click Check, type the check number, then click the green Submit button (or press Enter). The check number is saved and will appear on the Financial Report and in the Payments tab." },
                ...
                { id: 'm14', q: "How does proration work for new members?", a: "When logging a payment, check the <b>'Prorate First Month'</b> box. The system calculates the amount based on how many days are left in the current month. For example, if a member's rate is $35/month and they join on the 20th with 12 days remaining out of 31, they owe $13.55 for their first month. After that, they pay the full rate. Proration is only for the first month — leave the box unchecked for all future payments." },
                { id: 'm15', q: "How do I print a receipt for a member?", a: "Open the member's profile and click the green 'Print Receipt' button. This prints a branded receipt with the Patterson HC header, member details, payment amount, and method. You can print a receipt anytime — it uses their current monthly rate and last payment method on file." },
                { id: 'm16', q: "How do I mark a member as inactive?", a: "Open their profile and click the <b>'Mark Inactive'</b> button at the bottom (it's next to Reset PIN). Confirm the action. The member will immediately be:<br/><br/>1. <b>Blocked from the kiosk</b> — they won't appear in search results and can't check in.<br/>2. <b>Blocked from the Member Portal</b> — they'll see a message saying their membership is inactive.<br/>3. <b>Flagged in the member list</b> — a red 'INACTIVE' tag appears next to their name and their status column shows INACTIVE.<br/><br/>To reactivate, open their profile and click the green 'Reactivate' button in the red banner at the top, or click the 'Reactivate' button at the bottom." },
                { id: 'm17', q: "What does the INACTIVE status mean?", a: "Inactive is different from Overdue/Locked. <b>Overdue</b> happens automatically when a payment is past due. <b>Inactive</b> is a manual flag set by a director — it completely blocks the member from checking in and from using the Member Portal until a director reactivates them. Use this for members who have canceled, moved away, or need to be temporarily suspended." },
                { id: 'm18', q: "How do I filter the member list?", a: "On the Members tab, below the search bar, you will see a row of filter buttons. Tap any button to show only those members:<br/><br/><b>Paid</b> — members who have paid this month<br/><b>Unpaid</b> — members who have NOT paid this month<br/><b>Overdue</b> — members with locked accounts (red stoplight)<br/><b>24/7 Access</b> — all badge holders<br/><b>Needs Orientation</b> — members awaiting their walkthrough<br/><b>Inactive</b> — manually deactivated members<br/><b>Corporate</b> — all corporate-sponsored members<br/><b>Family Plans</b> — all family plan members<br/><b>Has Notes</b> — anyone with director notes on file<br/><br/>Tap the same button again to go back to All Members. The search bar still works on top of the filter." },
                { id: 'm19', q: "What do the checkmarks and X's mean in the Payment column?", a: "<b style='color:#16a34a'>Green checkmark</b> — member has paid for this month. Hover to see the payment method (Cash, Check, Card, ACH).<br/><b style='color:#8b5cf6'>Purple checkmark</b> — corporate sponsor has paid for this member. Hover to see the company name and method.<br/><b style='color:#ef4444'>Red X</b> — no payment logged for this month yet. Hover to see details.<br/><b style='color:#94a3b8'>Gray checkmark</b> — comped member (HD6, Military, Lifetime, etc.) — no payment required.<br/><br/>Checkmarks reset on the 1st of every month. Everyone starts as an X until their payment is logged for that month." },
                { id: 'm20', q: "How do corporate member payments work?", a: "Corporate members are paid for by their employer, not individually. When you mark a company as 'Paid' on the Corporate tab, all of that company's members will automatically show a purple checkmark in the Payment column. You do NOT need to log individual payments for corporate members — the system handles it based on the company's payment status." },
                { id: 'm21', q: "How do Director Notes work?", a: "Open a member's profile, click 'Edit Member', and type in the Director Notes field at the bottom. When you save, the system automatically adds today's date to the front of your note (like '[Mar 30, 2026] Called about payment'). This way you always know when a note was written. Notes are visible on the member list as a small yellow preview below the member's name, and you can use the 'Has Notes' filter to quickly see all members with notes." },
                { id: 'm22', q: "How do I see who has paid and who hasn't?", a: "Two quick ways:<br/><br/>1. <b>Payment column</b> — scan down the checkmarks and X's on the Members tab. Green check = paid, red X = not paid.<br/>2. <b>Filter buttons</b> — tap 'Paid' to see only members who have paid this month, or tap 'Unpaid' to see everyone who still owes." },                 { id: 'm23', q: "How do I see members who joined recently?", a: "On the Members tab, click the <b>Recent</b> filter button. A blue date picker appears with the label 'Joined since:' — pick any date you want and the list filters to only members who joined on or after that date. Useful for printing welcome letters for new members each month: pick the 1st of last month, click bulk print letters, and only the new joiners will print. The member count next to the date updates live as you change the date." },                 { id: 'm24', q: "Can I edit a payment after it's been logged?", a: "Yes. Go to the <b>Payments tab</b>, find the payment, and click the small <b>edit icon</b> (paper icon) next to the trash icon. A modal opens where you can change the payment date, amount, method, check number, and notes. This is useful for backdating payments to match deposit periods, fixing wrong check numbers, or correcting a method selection. Note: there is no undo — changes save to Airtable immediately." },                 { id: 'm25', q: "How do I match payment dates to PHC's deposit dates?", a: "PHC's business office sets deposit periods that don't always match the calendar month. When you log a payment, the <b>Payment Date</b> field at the top defaults to today, but you can click it and pick any date. Use the date the customer actually paid OR the date PHC's deposit period covers — whichever matches your reporting needs. The Payments tab and Financial Report will both filter by that date, not by when the record was created." }
            ]},
            { category: "Public Kiosk (iPad / Scanner)", icon: <Smartphone size={20} className="text-[#1080ad]" />, items: [
                ...
                { id: 'k3', q: "Why can't a member check in at the kiosk?", a: "Four possible reasons:<br/><br/>1. <b>Membership Inactive</b> — a director has marked them inactive. They won't even appear in kiosk search results. Reactivate them from their profile if needed.<br/>2. <b>Orientation Required</b> — they haven't been oriented yet. Mark it complete in their profile.<br/>3. <b>Payment Overdue (Red)</b> — they're 3+ days past due. Log a payment or talk to them at the front desk.<br/>4. <b>Wrong PIN</b> — reset their PIN from their profile." },
                ...
                { id: 'k9', q: "Can a member check in twice in the same day?", a: "Yes, but not within a 2-hour window. The kiosk blocks duplicate check-ins for the same person within 2 hours and shows 'Already checked in!' This prevents accidental double check-ins after a class. If someone legitimately comes back later in the day (morning workout, then evening class), they can check in again — just outside that 2-hour window." },
                { id: 'k10', q: "How does class attendance work with kiosk check-ins?", a: "Class attendance and kiosk check-ins are now unified. If a member checks in at the kiosk first and you later add them to a class, their existing check-in is just relabeled with the class name — no duplicate visit is created. If they go straight to class without scanning at the kiosk, adding them to the class roster creates their visit for the day. Either way, one person = one visit per day." }
            ]},
            ...
            { category: "Corporate Partners", icon: <Briefcase size={20} className="text-[#8b5cf6]" />, items: [
                ...
                { id: 'corp7', q: "Why does HD6/HCHF show 'No Charge' instead of a dollar amount?", a: "Hospital District No. 6 and Harper County Health Foundation are Patterson Health Center itself — you can't bill yourselves. The system automatically detects HD6 and HCHF and shows their card as <b>'INTERNAL · NO CHARGE'</b> with no Mark as Paid button. The roster and invoice tools still work if you want internal cost tracking, but their members never appear on outstanding-balance reports." }
            ]},
            { category: "Member Cards & Print Queue", icon: <QrCode size={20} className="text-[#dba51f]" />, items: [
                { id: 'card1', q: "How do I request a card to be printed for a member?", a: "Open the member's profile and click the gold <b>Request Card</b> button. Add an optional note (lost original, name change, damaged, etc.) and click Submit. Kristen will be notified by email and the request will appear in the Card Queue tab. She handles the printing and brings the card to your center." },
            ...
            { category: "Visitors & Passes", icon: <Eye size={20} className="text-[#16a34a]" />, items: [
                ...
                { id: 'v6', q: "What happens when a visitor's pass expires?", a: "A visitor is considered expired ONLY if BOTH: (1) the expiration date has passed AND (2) they have no prepaid passes remaining. If they still have prepaid passes left (even after the expiration date), they can still check in. Once both conditions are met, they can no longer check in at the kiosk and won't appear in the search results. Their record stays in the Visitors tab so you can see their history and contact them about converting to a full membership." },
            ...
            { category: "Classes & Check-ins", icon: <Calendar size={20} className="text-[#f59e0b]" />, items: [
                ...
                { id: 'c3', q: "Does a class check-in count as a regular visit?", a: "Class check-ins add to the local class roster for that session but do not create a separate visit in Airtable. Members should be checked in at the front desk when they arrive (which logs the visit), and then added to the class separately. Class rosters are saved to Airtable and can be viewed on the Classes tab under 'Saved Class Rosters'." }
            ]},
            { category: "Reporting & Exports", icon: <FileText size={20} className="text-[#dd6d22]" />, items: [
                ...
                { id: 'r6', q: "What time period does the dashboard show?", a: "The Dashboard always shows today's check-ins in real-time. The heatmap and stat cards reflect all historical data. The Reports tab lets you filter by specific months or quarters." },                 { id: 'r7', q: "What is the Payments tab?", a: "The Payments tab shows a live table of every payment logged for the selected month. It includes the member name, ID, amount, payment method (with check numbers), and date. At the top you will see summary cards showing total collected, number of payments, and a breakdown by method. You can also delete incorrect payments with the trash icon and export everything as a CSV." },                 { id: 'r8', q: "How do I see all check numbers?", a: "Go to the <b>Payments tab</b> in the sidebar. Check payments show as 'Check #1234' in the Method column. You can also see check numbers on the printed Financial Report." },                 { id: 'r9', q: "What is the Class Report?", a: "On the Classes tab, click the gold 'Class Report' button. It prints a detailed monthly report showing each class, total attendees, number of sessions, average per session, and a day-by-day breakdown with attendee names. It uses the same month selected on the Reports tab." },                 { id: 'r10', q: "What is the Monthly Summary report?", a: "On the Reports tab, click the <b>Monthly Summary</b> button. It generates a one-page snapshot of the selected month with total visits, average visits per day, employee/family/day-pass breakdown, corporate breakdown, membership pie chart, class attendance, and key metrics. Great for sharing with the PHC business office or board." }
            ]},
            ...
            { category: "Troubleshooting", icon: <AlertCircle size={20} className="text-[#ef4444]" />, items: [
                ...
                { id: 't6', q: "I accidentally deleted a member", a: "Deletion is permanent in the Wellness Hub. You would need to re-add them as a new member through the Add Member form. Their visit history linked to the old record cannot be recovered, but their visits are still logged in the Visits table in Airtable." },                 { id: 't7', q: "A member says they can't check in but they're not overdue", a: "They may be marked as Inactive. Look for a red 'INACTIVE' tag in the member list, or open their profile and check for a red banner at the top. If they should be active, click 'Reactivate' to restore their access." },                 { id: 't8', q: "A member can't log into the Member Portal", a: "Two possible reasons:<br/><br/>1. <b>Wrong PIN</b> — reset their PIN from their profile in the Director Portal.<br/>2. <b>Marked Inactive</b> — inactive members are blocked from the Member Portal. They'll see a message saying their membership is inactive. Reactivate them if needed." },                 { id: 't9', q: "A member shows a red X but they paid", a: "The payment checkmark resets on the 1st of every month. If they paid last month, they will show an X until this month's payment is logged. Open their profile, click 'Log Payment', and select the method. The X will turn to a green checkmark." },                 { id: 't10', q: "A corporate member shows a red X", a: "Corporate members' payment status is tied to their sponsor company, not individual payments. Go to the Corporate tab, find their company, and click 'Mark as Paid' for the current billing period. All of that company's members will automatically flip from X to purple checkmark." }
            ]}
          ];
      ...
      {selectedVisitor && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#001f3f]/90 backdrop-blur-md">
          <div className="bg-white rounded-[2rem] w-full max-w-2xl p-12 relative shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <span className={`px-4 py-1 rounded-full text-[10px] font-black tracking-widest ${(new Date(selectedVisitor.expirationDate + 'T23:59:59') < new Date() && !(selectedVisitor.passesRemaining > 0)) ? 'bg-red-100 text-red-600' : 'bg-green-100 text-green-600'}`}>{(new Date(selectedVisitor.expirationDate + 'T23:59:59') < new Date() && !(selectedVisitor.passesRemaining > 0)) ? 'EXPIRED' : 'ACTIVE PASS'}</span>
            ...
            {(() => {
              const fullName = ((selectedVisitor.firstName || '') + ' ' + (selectedVisitor.lastName || '')).toLowerCase().trim();
              const key = (x) => ((x.firstName || '') + '|' + (x.lastName || '') + '|' + (x.phone || x.email || '')).toLowerCase().trim();
              const myKey = key(selectedVisitor);
              const allPasses = visitors.filter(x => key(x) === myKey).sort((a, b) => new Date(b.purchaseDate || 0) - new Date(a.purchaseDate || 0));
              const myVisits = visits.filter(x => String(x.name || '').toLowerCase().replace(' (visitor)', '').trim() === fullName).sort((a, b) => new Date(b.time) - new Date(a.time));
              const paidTotal = allPasses.reduce((s, x) => s + (parseFloat(String(x.amountPaid).replace(/[^0-9.]/g, '')) || 0), 0);
              return (<div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Passes Bought ({allPasses.length}) &middot; ${paidTotal.toFixed(2)} total</p>
                  <div className="space-y-2 max-h-56 overflow-y-auto pr-1">
                    {allPasses.map(pp => (<div key={pp.airtableId} className={'flex justify-between items-center p-3 rounded-xl border ' + (pp.airtableId === selectedVisitor.airtableId ? 'bg-purple-50 border-purple-200' : 'bg-slate-50 border-slate-100')}>
                      <div><p className="text-xs font-black text-slate-700">{pp.passType || 'Pass'}</p><p className="text-[10px] text-slate-400 font-bold">{pp.purchaseDate ? new Date(pp.purchaseDate + 'T00:00:00').toLocaleDateString('en-US', {month:'short', day:'numeric', year:'numeric'}) : 'No date'}</p></div>
                      <span className="text-sm font-black text-green-700">${(parseFloat(String(pp.amountPaid).replace(/[^0-9.]/g, '')) || 0).toFixed(2)}</span>
                    </div>))}
                  </div>
                </div>
                <div>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Visit Dates ({myVisits.length})</p>
                  {myVisits.length === 0 ? (<p className="text-xs text-slate-400 italic bg-slate-50 p-4 rounded-xl border border-slate-100">No visits logged yet.</p>) : (
                  <div className="space-y-2 max-h-56 overflow-y-auto pr-1">
                    {myVisits.map((vv, ix) => (<div key={ix} className="flex justify-between items-center bg-slate-50 p-3 rounded-xl border border-slate-100">
                      <p className="text-xs font-black text-slate-700">{new Date(vv.time).toLocaleDateString('en-US', {weekday:'short', month:'short', day:'numeric', year:'numeric'})}</p>
                      <p className="text-[10px] text-slate-400 font-bold">{new Date(vv.time).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'})}</p>
                    </div>))}
                  </div>)}
                </div>
              </div>);
           ...
           <div className="mt-10 flex gap-4">
              ...
              {!selectedVisitor.convertedToMember && (<button onClick={() => { setConvertingVisitor(selectedVisitor); setSelectedVisitor(null); }} className="flex-1 bg-[#8b5cf6] text-white py-4 rounded-xl font-bold shadow-lg hover:bg-purple-700 transition-colors text-sm flex items-center justify-center gap-2"><Users size={16}/> Convert to Member</button>)}
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-[#001f3f]/90 backdrop-blur-md">
           <div className="bg-white rounded-[2rem] w-full max-w-xl p-10 relative shadow-2xl max-h-[90vh] overflow-y-auto">
              ...
              <form onSubmit={async (e) => { e.preventDefault(); const updates = { visitorAirtableId: editingVisitor.airtableId, firstName: e.target.ev_fname.value, lastName: e.target.ev_lname.value, email: e.target.ev_email.value, phone: e.target.ev_phone.value, address: e.target.ev_address.value, city: e.target.ev_city.value, state: e.target.ev_state.value, zip: e.target.ev_zip.value, referringProvider: e.target.ev_provider.value, passSource: passSourceFor(e.target.ev_provider.value), notes: e.target.ev_notes.value, passType: e.target.ev_pass.value, center: e.target.ev_center.value }; try { const res = await fetch('/api/update-visitor', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(updates) }); const result = await res.json(); if (result.success) { setVisitors(prev => prev.map(v => v.airtableId === editingVisitor.airtableId ? { ...v, firstName: updates.firstName, lastName: updates.lastName, email: updates.email, phone: updates.phone, address: updates.address, city: updates.city, state: updates.state, zip: updates.zip, referringProvider: updates.referringProvider, notes: updates.notes, passType: updates.passType, center: updates.center } : v)); setEditingVisitor(null); if (selectedVisitor && selectedVisitor.airtableId === editingVisitor.airtableId) { setSelectedVisitor(prev => ({ ...prev, firstName: updates.firstName, lastName: updates.lastName, email: updates.email, phone: updates.phone, address: updates.address, city: updates.city, state: updates.state, zip: updates.zip, referringProvider: updates.referringProvider, notes: updates.notes, passType: updates.passType, center: updates.center })); } } else { alert('Error: ' + result.error); } } catch (err) { alert('Network error.'); } }} className="space-y-4">
                ...
                <div><label className="text-xs font-bold text-slate-400 uppercase mb-1 block">Pass Source</label><PassSourceSelect id="ev_provider" defaultValue={editingVisitor.referringProvider} className="w-full p-3 bg-slate-50 border rounded-xl text-sm outline-none focus:border-[#8b5cf6] font-bold text-slate-700" /></div>
          ...
          <div className="bg-white rounded-3xl w-full max-w-xl p-10 relative shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={async (e) => { 
              ...
              const formData = { 
                ...
                referringProvider: e.target.vprovider.value, passSource: passSourceFor(e.target.vprovider.value), 
              ...
              <div className="grid grid-cols-2 gap-5"><div><label className="text-xs font-bold text-slate-400 uppercase mb-1 ml-2 block tracking-widest">First Name</label><input id="vfname" required onBlur={() => { const fn = (document.getElementById('vfname') || {}).value || ''; const ln = (document.getElementById('vlname') || {}).value || ''; if (!fn.trim() || !ln.trim()) { setVisitorDupMatch(null); return; } const hit = visitors.filter(x => (x.firstName || '').toLowerCase().trim() === fn.toLowerCase().trim() && (x.lastName || '').toLowerCase().trim() === ln.toLowerCase().trim()); setVisitorDupMatch(hit.length ? hit.sort((a, b) => new Date(b.purchaseDate || 0) - new Date(a.purchaseDate || 0))[0] : null); }} className="w-full p-4 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-[#1080ad] transition-colors" /></div><div><label className="text-xs font-bold text-slate-400 uppercase mb-1 ml-2 block tracking-widest">Last Name</label><input id="vlname" required onBlur={() => { const fn = (document.getElementById('vfname') || {}).value || ''; const ln = (document.getElementById('vlname') || {}).value || ''; if (!fn.trim() || !ln.trim()) { setVisitorDupMatch(null); return; } const hit = visitors.filter(x => (x.firstName || '').toLowerCase().trim() === fn.toLowerCase().trim() && (x.lastName || '').toLowerCase().trim() === ln.toLowerCase().trim()); setVisitorDupMatch(hit.length ? hit.sort((a, b) => new Date(b.purchaseDate || 0) - new Date(a.purchaseDate || 0))[0] : null); }} className="w-full p-4 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-[#1080ad] transition-colors" /></div></div>
              {visitorDupMatch && (<div className="bg-amber-50 border-2 border-amber-300 rounded-2xl p-4"><p className="text-sm font-black text-amber-800">{visitorDupMatch.firstName} {visitorDupMatch.lastName} already visits here.</p><p className="text-xs text-amber-700 mt-1 font-medium">{visitorDupMatch.passType || "Pass"}{visitorDupMatch.purchaseDate ? " bought " + new Date(visitorDupMatch.purchaseDate + "T00:00:00").toLocaleDateString("en-US", {month:"short", day:"numeric", year:"numeric"}) : ""} - {visitorDupMatch.totalVisits || 0} visit{(visitorDupMatch.totalVisits || 0) === 1 ? "" : "s"} on record.</p><p className="text-xs text-amber-700 mt-2 font-medium">Adding a new pass is fine - use their existing contact details so both passes stay under the same person in your list.</p><button type="button" onClick={() => { const set = (id, val) => { const el = document.getElementById(id); if (el) el.value = val || ''; }; set('vemail', visitorDupMatch.email); set('vphone', visitorDupMatch.phone); set('vaddress', visitorDupMatch.address); set('vcity', visitorDupMatch.city); set('vstate', visitorDupMatch.state); set('vzip', visitorDupMatch.zip); showToast('Contact details copied from their existing record.', 'success', 3000); }} className="mt-3 w-full bg-amber-500 text-white py-2.5 rounded-xl font-black text-xs uppercase tracking-widest hover:bg-amber-600 transition-colors">Use their existing details</button></div>)}
              ...
              <div><label className="text-xs font-bold text-slate-400 uppercase mb-1 ml-2 block tracking-widest">Where did this pass come from?</label><PassSourceSelect id="vprovider" className="w-full p-4 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-[#1080ad] transition-colors font-bold text-slate-700" /></div>
     ...
     {showAddClassModal && (
      ...
      {showAddModal && (
           ...
           <div className="bg-white rounded-3xl w-full max-w-xl p-10 relative shadow-2xl max-h-[90vh] overflow-y-auto">
              ...
              {newMemberPin ? (
                <div className="flex flex-col items-center justify-center text-center py-4">
                  ...
                  {/* COLLECT FIRST PAYMENT SECTION */}
                  {!firstPayment.method ? (
                    <div className="w-full mb-6">
                      {!firstPayment.open ? (
                        <button onClick={() => setFirstPayment({ ...firstPayment, open: true })} className="w-full bg-[#16a34a]/10 border-2 border-dashed border-[#16a34a]/30 text-[#16a34a] p-4 rounded-xl font-bold text-sm hover:bg-[#16a34a]/20 transition-all flex items-center justify-center gap-2"><CreditCard size={18}/> Collect First Payment Now</button>
                      ) : (() => {
                        const newMember = members.find(m => m.firstName + ' ' + m.lastName === newMemberPin.name) || {};
                        const fullRate = parseFloat(String(newMember.monthlyRate || '0').replace(/[^0-9.]/g, '')) || 0;
                        const today = new Date();
                        ...
                        return (
                          <div className="bg-white border-2 border-[#16a34a] rounded-2xl p-6">
                            <div className="flex items-center justify-between mb-4">
                              <p className="text-sm font-black text-[#001f3f] flex items-center gap-2"><CreditCard size={16} className="text-[#16a34a]"/> First Payment</p>
                              <button onClick={() => setFirstPayment({ open: false, prorated: false, method: null, memberData: null })} className="text-slate-300 hover:text-red-500 text-xs font-bold">Cancel</button>
                            </div>
                            {fullRate > 0 && (
                              <div className="mb-4">
                                <label onClick={() => setFirstPayment({ ...firstPayment, prorated: !firstPayment.prorated })} className="flex items-center gap-3 bg-blue-50 border border-blue-200 rounded-xl p-3 cursor-pointer hover:bg-blue-100 transition-colors">
                                  <input type="checkbox" checked={firstPayment.prorated} onChange={() => setFirstPayment({ ...firstPayment, prorated: !firstPayment.prorated })} className="w-5 h-5 rounded border-slate-300 text-[#1080ad] focus:ring-[#1080ad]" />
                                  <div className="flex-1">
                                    <p className="text-sm font-bold text-[#001f3f]">Prorate First Month</p>
                                    <p className="text-[10px] text-blue-500">{daysRemaining} of {daysInMonth} days remaining</p>
                                  </div>
                                </label>
                                <div className="mt-3 text-center">
                                  ...
                                  {firstPayment.prorated && <p className="text-[10px] text-slate-400 mt-1">Full rate: ${fullRate.toFixed(2)}/mo</p>}
                                </div>
                              </div>
                            )}
                              ...
                              {['Cash', 'Check', 'Card', 'ACH', 'Other'].map(m => (
                                <button key={m} onClick={async () => {
                                  ...
                                  try {
                                    ...
                                    const result = await res.json();
                                    if (result.success) {
                                      setMembers(prev => prev.map(mem => { if (mem.airtableId === member.airtableId) return { ...mem, status: 'ACTIVE', nextPayment: result.nextPaymentDue }; if (result.familyMembersUpdated > 0 && member.familyName && mem.familyName && mem.familyName === member.familyName) return { ...mem, status: 'ACTIVE', nextPayment: result.nextPaymentDue }; return mem; }));
                    ...
                    <div className="w-full mb-6 bg-green-50 border border-green-200 rounded-xl p-4 text-center">
                      <CheckCircle size={24} className="text-[#16a34a] mx-auto mb-2" />
                      <p className="text-sm font-black text-[#16a34a]">Payment Collected ({firstPayment.method})</p>
                      <p className="text-[10px] text-slate-400 mt-1">Next payment due: {firstPayment.memberData?.nextPayment}</p>
                    </div>
                  )}
        ...
        const filteredOut = mappedMembers.filter(m => !m.merged);
        setMembers(filteredOut); } setLoading(false); }); }} className={`${familyFlow ? 'flex-1' : 'w-full'} bg-[#001f3f] text-white px-8 py-4 rounded-xl font-bold`}>Done</button>
                  </div>
                </div>
              ) : (
                <>
                  ...
                  {familyFlow && (<div className="mb-8 bg-purple-50 border border-purple-200 rounded-2xl p-5"><p className="text-sm font-black text-purple-700 mb-1">Already a member?</p><p className="text-xs text-purple-500 font-medium mb-3">Search an existing member to add them to this family instead of creating a new profile.</p><input value={existingFamSearch} onChange={(e) => setExistingFamSearch(e.target.value)} placeholder="Search existing members by name..." className="w-full p-3 bg-white border border-purple-200 rounded-xl outline-none focus:border-purple-400 text-sm" />{existingFamSearch.trim().length > 1 && (<div className="mt-2 bg-white border border-purple-100 rounded-xl overflow-hidden max-h-56 overflow-y-auto">{members.filter(mm => !mm.familyGroupId && !mm.inactive && (mm.firstName + ' ' + mm.lastName).toLowerCase().includes(existingFamSearch.trim().toLowerCase())).slice(0, 8).map(mm => (<button key={mm.airtableId} type="button" onClick={async () => { if (!window.confirm('Add ' + mm.firstName + ' ' + mm.lastName + ' to ' + familyFlow.familyName + ' as a covered family member? Their billing becomes $0 (covered under the primary).')) return; try { const res = await fetch('/api/link-existing-member', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ memberAirtableId: mm.airtableId, familyRecordId: familyFlow.familyRecordId, plan: familyFlow.plan }) }); const data = await res.json(); if (data.success) { setMembers(prev => prev.map(x => x.airtableId === mm.airtableId ? Object.assign({}, x, { familyGroupId: familyFlow.familyRecordId, familyName: familyFlow.familyName, type: familyFlow.plan, billingMethod: '', monthlyRate: 0, nextPayment: data.nextPayment || x.nextPayment }) : x)); alert(mm.firstName + ' has been added to ' + familyFlow.familyName + '.'); setExistingFamSearch(''); setShowAddModal(false); setFamilyFlow(null); } else { alert('Could not add: ' + (data.error || 'unknown error')); } } catch (e) { alert('Error: ' + e.message); }}} className="w-full px-4 py-3 text-left hover:bg-purple-50 border-b border-purple-50 last:border-0 flex justify-between items-center transition-colors"><span className="font-bold text-[#001f3f] text-sm">{mm.firstName} {mm.lastName}</span><span className="text-[10px] font-mono text-slate-400">{mm.id}</span></button>))}{members.filter(mm => !mm.familyGroupId && !mm.inactive && (mm.firstName + ' ' + mm.lastName).toLowerCase().includes(existingFamSearch.trim().toLowerCase())).length === 0 && (<p className="px-4 py-3 text-xs text-slate-400 italic">No unlinked members match. (Anyone already in a family is hidden.)</p>)}</div>)}<div className="mt-3 text-[10px] text-purple-400 font-black uppercase tracking-widest text-center">&mdash; or create a new profile below &mdash;</div></div>)}
                  <form onSubmit={handleAddMemberSubmit} className="space-y-5">
                     ...
                     <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4 space-y-3"><p className="text-[10px] font-black text-emerald-600 uppercase tracking-widest">Onboarding</p><label className="flex items-center gap-3 cursor-pointer hover:bg-emerald-100 rounded-lg p-2 -m-1 transition-colors"><input type="checkbox" id="orientDone" className="w-5 h-5 rounded border-slate-300 text-emerald-600 accent-emerald-600" /><div><p className="text-sm font-bold text-emerald-800">Orientation Complete</p></div></label><label className="flex items-center gap-3 cursor-pointer hover:bg-emerald-100 rounded-lg p-2 -m-1 transition-colors"><input type="checkbox" id="paperwork" className="w-5 h-5 rounded border-slate-300 text-emerald-600 accent-emerald-600" /><div><p className="text-sm font-bold text-emerald-800">Paperwork Completed</p></div></label></div>
...
{(() => { var showAnthony = (viewingCenter === 'anthony' || viewingCenter === 'both') && !selectedMember.orientationAnthony; var showHarper = (viewingCenter === 'harper' || viewingCenter === 'both') && !selectedMember.orientationHarper; if (!showAnthony && !showHarper) return null; return (<div className="mb-6 bg-blue-50 border-l-4 border-blue-500 p-4 rounded-r-lg"><p className="text-blue-800 font-bold text-sm mb-3">Orientation Needed</p><div className="flex gap-3">{showAnthony && (<button onClick={() => handleMarkOrientation('anthony')} className="flex-1 bg-blue-600 text-white py-3 rounded-lg font-bold text-sm hover:bg-blue-700 transition-colors flex items-center justify-center gap-2"><CheckCircle size={16}/> Complete Anthony Orientation</button>)}{showHarper && (<button onClick={() => handleMarkOrientation('harper')} className="flex-1 bg-orange-600 text-white py-3 rounded-lg font-bold text-sm hover:bg-orange-700 transition-colors flex items-center justify-center gap-2"><CheckCircle size={16}/> Complete Harper Orientation</button>)}</div></div>); })()}
                 {!editMode ? (<>
                     {(() => {
                   if (!inFamily(selectedMember)) return null;
                   const fam = familyOf(selectedMember);
                   const primaries = fam.filter(x => x.familyPrimary);
                   if (primaries.length === 1 && primaries[0].airtableId === selectedMember.airtableId) {
                     return (<div className="mb-6 bg-green-50 border-2 border-green-200 rounded-2xl p-4"><p className="text-sm font-black text-green-800">This person pays for the family.</p><p className="text-xs text-green-700 font-medium mt-1">Everyone else in {selectedMember.familyName || 'this family'} is covered at $0. Their bill is the only one you collect.</p></div>);
                   }
                   if (primaries.length === 1) {
                     return (<div className="mb-6 bg-slate-50 border-2 border-slate-200 rounded-2xl p-4"><p className="text-sm font-black text-slate-700">Covered by {primaries[0].firstName} {primaries[0].lastName}.</p><p className="text-xs text-slate-500 font-medium mt-1">No charge for this person. Collect from {primaries[0].firstName} instead.</p></div>);
                   }
                   if (primaries.length === 0) {
                     return (<div className="mb-6 bg-amber-50 border-2 border-amber-300 rounded-2xl p-4"><p className="text-sm font-black text-amber-800">Nobody in this family is set as the payer.</p><p className="text-xs text-amber-700 font-medium mt-1">Open whoever pays the bill and click <strong>Make This Person the Payer</strong> down below.</p></div>);
                   }
                   return (<div className="mb-6 bg-red-50 border-2 border-red-300 rounded-2xl p-4"><p className="text-sm font-black text-red-800">Two people are set as the payer: {primaries.map(p => p.firstName).join(' and ')}.</p><p className="text-xs text-red-700 font-medium mt-1">Open the one who actually pays and click <strong>Make This Person the Payer</strong>. That will clear the other.</p></div>);
                 })()}
           {(() => {
                   ...
                   return (
                     <div className="mb-8 pb-6 border-b border-slate-100 flex items-center justify-between">
                       <div>
                         <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">{currentMonthName} Balance</p>
                         <p className={`text-3xl font-black tracking-tight ${paidThisPeriod ? 'text-slate-300' : 'text-[#001f3f]'}`}>
                           {paidThisPeriod ? 'Paid' : `$${rate.toFixed(2)}`}
                         </p>
                       </div>
                       <div className="text-right">
                         <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Status</p>
                         <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-black tracking-widest ${paidThisPeriod ? 'bg-slate-100 text-slate-500' : 'bg-orange-50 text-orange-600'}`}>
                           <span className={`w-1.5 h-1.5 rounded-full ${paidThisPeriod ? 'bg-slate-400' : 'bg-orange-500'}`}></span>
                           {paidThisPeriod ? 'CURRENT' : 'OUTSTANDING'}
                         </span>
                       </div>
                     </div>
                   );
              ...
              <p className="text-[10px] font-black text-purple-600 uppercase tracking-widest mb-3">Family Members — {selectedMember.familyName}</p><div className="space-y-2">{famMembers.map(fm => (<div key={fm.airtableId} className="flex justify-between items-center bg-white p-3 rounded-lg border border-purple-100 hover:border-purple-300 hover:bg-purple-50 transition-all group"><button onClick={() => setSelectedMember(fm)} className="flex-1 text-left"><p className="font-bold text-[#001f3f] group-hover:text-[#8b5cf6] transition-colors">{fm.firstName} {fm.lastName}{fm.familyPrimary && <span className="ml-2 px-2 py-0.5 rounded-full text-[8px] font-black bg-green-100 text-green-700 uppercase tracking-widest">Pays</span>}</p><p className="text-[9px] text-slate-400 uppercase tracking-widest">{fm.id} • {fm.type}</p></button><div className="flex items-center gap-2"><ChevronRight size={14} className="text-slate-300 group-hover:text-[#8b5cf6] transition-colors" /><button onClick={async (e) => { e.stopPropagation(); if (!window.confirm("Remove " + fm.firstName + " " + fm.lastName + " from this family group? They will keep their individual membership.")) return; try { var res = await fetch('/api/update-member', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ airtableId: fm.airtableId, familyGroup: null }) }); if (res.ok) { setMembers(function(prev) { return prev.map(function(m) { return m.airtableId === fm.airtableId ? Object.assign({}, m, { familyName: '' }) : m; }); }); alert(fm.firstName + " has been removed from the family group."); } else { alert("Could not remove from family."); } } catch (err) { alert("Network error."); } }} className="text-slate-300 hover:text-red-500 transition-colors p-1 shrink-0" title="Remove from family"><X size={14}/></button></div></div>))}</div>
                 ...
                 <div className="space-y-4 mb-8">
                   ...
                   <div className="flex gap-3 mt-4">
                      <button onClick={async () => { const selectedPlan = document.getElementById('ed_plan').value; const selectedBilling = (selectedPlan === 'CORPORATE' || selectedPlan === 'CORPORATE FAMILY' || selectedPlan === 'CORPORATE STUDENT') ? 'Month-to-Month' : document.getElementById('ed_billing').value; const enteredDiscount = document.getElementById('ed_discount').value; const basePrice = getBaseRate(selectedPlan, selectedBilling); const discountMatch = enteredDiscount.match(/\$(\d+)/); const discountAmount = discountMatch ? parseInt(discountMatch[1], 10) : 0; const finalMonthlyRate = isFamilyPrimary(selectedMember) ? Math.max(0, basePrice - discountAmount) : 0; const updates = { airtableId: selectedMember.airtableId, firstName: document.getElementById('ed_fname').value, lastName: document.getElementById('ed_lname').value, email: document.getElementById('ed_email').value, phone: document.getElementById('ed_phone').value, address: document.getElementById('ed_address').value, city: document.getElementById('ed_city').value, state: document.getElementById('ed_state').value, zip: document.getElementById('ed_zip').value, plan: selectedPlan, billingMethod: selectedBilling, center: document.getElementById('ed_center').value, sponsor: document.getElementById('ed_sponsor').value, access247: document.getElementById('ed_247').checked, badgeNumber: document.getElementById('ed_badge').value, notes: (() => { var newNotes = document.getElementById('ed_notes').value; var oldNotes = selectedMember.notes || ''; if (newNotes === oldNotes || newNotes.trim() === '') return newNotes; if (oldNotes.trim() === '') { var d = new Date(); return '[' + d.toLocaleDateString('en-US', {month:'short', day:'numeric', year:'numeric'}) + '] ' + newNotes; } if (newNotes.length > oldNotes.length && newNotes.startsWith(oldNotes)) { var addedText = newNotes.substring(oldNotes.length).trim(); if (addedText.length > 0) { var d2 = new Date(); return oldNotes + '\n[' + d2.toLocaleDateString('en-US', {month:'short', day:'numeric', year:'numeric'}) + '] ' + addedText; } } return newNotes; })(), discountCode: enteredDiscount, discountExpiration: document.getElementById('ed_discount_exp').value, monthlyRate: finalMonthlyRate, nextPayment: document.getElementById('ed_nextpayment').value || null }; try { const res = await fetch('/api/update-member', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(updates) }); const result = await res.json(); if (result.success || res.ok) { const updated = { ...selectedMember, firstName: updates.firstName, lastName: updates.lastName, email: updates.email, phone: updates.phone, address: updates.address, city: updates.city, state: updates.state, zip: updates.zip, type: updates.plan, billingMethod: updates.billingMethod, center: updates.center, sponsorName: updates.sponsor, access247: updates.access247, badgeNumber: updates.badgeNumber, notes: updates.notes, discountCode: updates.discountCode, discountExpiration: updates.discountExpiration, monthlyRate: updates.monthlyRate, nextPayment: updates.nextPayment }; setSelectedMember(updated); setMembers(prev => prev.map(m => m.airtableId === selectedMember.airtableId ? updated : m)); if (updates.plan !== selectedMember.type && selectedMember.familyName) { const famMembers = members.filter(m => m.familyName && m.familyName === selectedMember.familyName && m.airtableId !== selectedMember.airtableId); if (famMembers.length > 0 && window.confirm('Also change the ' + famMembers.length + ' other family member' + (famMembers.length > 1 ? 's' : '') + ' to ' + updates.plan + '?')) { for (const fm of famMembers) { try { await fetch('/api/update-member', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ airtableId: fm.airtableId, plan: updates.plan, sponsor: updates.sponsor, nextPayment: updates.nextPayment, monthlyRate: 0, billingMethod: '' }) }); } catch (e) {} } setMembers(prev => prev.map(m => (m.familyName && m.familyName === selectedMember.familyName && m.airtableId !== selectedMember.airtableId) ? Object.assign({}, m, { type: updates.plan, sponsorName: updates.sponsor, nextPayment: updates.nextPayment, monthlyRate: 0, billingMethod: '' }) : m)); alert('Updated ' + famMembers.length + ' family member' + (famMembers.length > 1 ? 's' : '') + ' — same plan, same sponsor, same renewal date, and no charge.'); } } setEditMode(false); } else { alert('Airtable Error: ' + result.error); } } catch (err) { alert('Network error.'); } }} className="flex-1 bg-[#1080ad] text-white py-3 rounded-xl font-bold text-sm hover:bg-blue-700 transition-colors">Save Changes</button>
                 ...
                 {user?.role !== 'business' && (<div className="mt-14 space-y-6">
                     <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200"><div className="flex items-center justify-between"><div><h3 className="text-lg font-bold text-[#001f3f] flex items-center gap-2"><Activity size={18} className="text-[#1080ad]"/> Auto-Pay (ACH)</h3><p className="text-xs text-slate-400 font-medium mt-1">Drafts post automatically on the member's chosen day, so this keeps them from reading as unpaid before the draft lands.</p></div><label className="flex items-center gap-2 cursor-pointer shrink-0 ml-4"><input type="checkbox" checked={!!selectedMember.achAutoPay} disabled={savingOnboarding} onChange={(e) => saveOnboardingField(selectedMember, { 'Auto-Pay ACH': e.target.checked })} className="w-5 h-5 accent-[#1080ad]" /><span className="text-sm font-bold text-slate-600">On</span></label></div>{selectedMember.achAutoPay && (<div className="mt-4 flex items-center gap-3"><span className="text-xs font-black uppercase tracking-widest text-slate-400">Draft day</span><select value={selectedMember.achDraftDay || '3rd'} disabled={savingOnboarding} onChange={(e) => saveOnboardingField(selectedMember, { 'ACH Draft Day': e.target.value })} className="text-sm font-bold p-2 rounded-lg border border-slate-200 bg-white text-slate-700 outline-none focus:border-[#1080ad]"><option value="3rd">3rd only</option><option value="18th">18th only</option><option value="Both">Both (3rd & 18th)</option></select></div>)}</div>
                     <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200"><div className="flex items-center justify-between"><div><h3 className="text-lg font-bold text-[#001f3f] flex items-center gap-2"><Briefcase size={18} className="text-[#8b5cf6]"/> Corporate ACH</h3><p className="text-xs text-slate-400 font-medium mt-1">Check this if the member's corporate sponsor pays via ACH on their behalf. Keeps them green and shows as "Corp ACH" on the Payments tab.</p></div><label className="flex items-center gap-2 cursor-pointer shrink-0 ml-4"><input type="checkbox" checked={!!selectedMember.corpAchPay} disabled={savingOnboarding} onChange={(e) => saveOnboardingField(selectedMember, { 'Corporate ACH': e.target.checked })} className="w-5 h-5 accent-[#8b5cf6]" /><span className="text-sm font-bold text-slate-600">On</span></label></div></div>
                    {(() => {
                      ...
                      if (selectedMember.isMinor) items.push({ key: 'parentPermission', dateKey: 'parentPermissionDate', label: 'Parent Permission Form (under 18)', apiField: 'Parent Permission Form', apiDateField: 'Parent Permission Form Date' });
                      const completedCount = items.filter(it => selectedMember[it.key]).length;
                      const requiredMissing = !selectedMember.basicOrientation || !selectedMember.paperworkCompleted || (selectedMember.isMinor && !selectedMember.parentPermission);
                      ...
                      return (
                        <div className={`mb-4 rounded-2xl border-2 p-4 ${isMinor ? 'bg-red-50 border-red-300' : 'bg-slate-50 border-slate-200'}`}>
                          <div className="flex items-center gap-3">
                            ...
                            <span className={`text-xs font-black uppercase tracking-widest ${!isMinor ? 'text-slate-500' : (selectedMember.mayAttendAlone ? 'text-amber-700' : 'text-red-700')}`}>{!isMinor ? 'Under age 18' : (selectedMember.mayAttendAlone ? 'MINOR — May attend without a parent' : '⚠ MINOR — Parent must be present')}</span>
                          </div>
                          {isMinor && (
                            <div className="mt-3">
                              <label className="flex items-center gap-3 cursor-pointer bg-white border-2 rounded-xl px-3 py-2 mb-3 " style={{ borderColor: selectedMember.mayAttendAlone ? '#f59e0b' : '#fecaca' }}><input type="checkbox" checked={!!selectedMember.mayAttendAlone} disabled={savingOnboarding} onChange={(e) => saveOnboardingField(selectedMember, { 'May Attend Alone': e.target.checked })} className="w-4 h-4 rounded border-slate-300 shrink-0 cursor-pointer" /><span className="text-[11px] font-black uppercase tracking-widest text-slate-600">May attend without a parent present</span></label>
                              <div className="flex items-center gap-3"><label className="text-[10px] font-black uppercase tracking-widest text-red-700 shrink-0">Turns 18:</label>
                    ...
                    <button onClick={() => { const goingToEdit = !editMode; setEditMode(goingToEdit); if (goingToEdit) { setTimeout(() => { const el = document.getElementById('ed_fname'); if (el && el.scrollIntoView) el.scrollIntoView({ behavior: 'smooth', block: 'center' }); }, 80); } }} className="bg-blue-50 text-blue-600 hover:bg-blue-600 hover:text-white py-4 rounded-xl font-bold shadow-sm transition-all flex flex-col items-center justify-center gap-2 text-xs"><FileText size={20} /> Edit Member</button>
                    {selectedMember.type.includes('FAMILY') && (<button onClick={async () => { let famRecordId = selectedMember.familyGroupId; if (!famRecordId) { if (!window.confirm('This member has no family group yet. Create one now and make ' + selectedMember.firstName + ' the primary?')) return; try { const rate = parseFloat(String(selectedMember.monthlyRate).replace(/[^0-9.]/g, '')) || 0; const cfg = await fetch('/api/create-family-group', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ airtableId: selectedMember.airtableId, familyName: selectedMember.familyName || (selectedMember.lastName + ' Family'), plan: selectedMember.type, monthlyRate: rate }) }); const cfgData = await cfg.json(); if (!cfgData.success) { alert('Could not create family group: ' + (cfgData.error || 'unknown error')); return; } famRecordId = cfgData.familyRecordId; const newFamName = cfgData.familyName; setMembers(prev => prev.map(m => m.airtableId === selectedMember.airtableId ? Object.assign({}, m, { familyGroupId: famRecordId, familyName: newFamName }) : m)); } catch (e) { alert('Error creating family group: ' + e.message); return; } } setFamilyFlow({ familyRecordId: famRecordId, familyName: selectedMember.familyName || `${selectedMember.lastName} Family`, lastName: selectedMember.lastName, plan: selectedMember.type, center: selectedMember.center, email: selectedMember.email, phone: selectedMember.phone, corporateSponsor: selectedMember.sponsorName, addedMembers: [] }); setShowAddModal(true); setSelectedMember(null); }} className="bg-purple-50 text-purple-600 hover:bg-purple-600 hover:text-white py-4 rounded-xl font-bold shadow-sm transition-all flex flex-col items-center justify-center gap-2 text-xs"><Users size={20} /> Add to Family</button>)}{(selectedMember.familyName || selectedMember.familyGroupId) && !isFamilyPrimary(selectedMember) && (<button onClick={async () => { const fam0 = members.filter(m => (selectedMember.familyGroupId && m.familyGroupId === selectedMember.familyGroupId) || (selectedMember.familyName && m.familyName && m.familyName === selectedMember.familyName)); const oldPrimary = fam0.find(m => m.airtableId !== selectedMember.airtableId && (parseFloat(String(m.monthlyRate).replace(/[^0-9.]/g, '')) || 0) > 0); const rate = oldPrimary ? (parseFloat(String(oldPrimary.monthlyRate).replace(/[^0-9.]/g, '')) || 0) : getBaseRate(selectedMember.type, 'Month-to-Month'); const billing = (oldPrimary && oldPrimary.billingMethod) ? oldPrimary.billingMethod : 'Month-to-Month'; if (!window.confirm('Make ' + selectedMember.firstName + ' ' + selectedMember.lastName + ' the person who pays for this family?\n\nThey will be charged the monthly rate. Everyone else in the family becomes $0 — covered under them.')) return; try { const newRate = await setFamilyPrimary(selectedMember); setSelectedMember(Object.assign({}, selectedMember, { familyPrimary: true, monthlyRate: newRate })); showToast(selectedMember.firstName + ' now pays for the family. Everyone else is $0.', 'success', 5000); } catch (e) { alert('Could not update: ' + e.message); }}} className="bg-amber-50 text-amber-700 hover:bg-amber-500 hover:text-white py-4 rounded-xl font-bold shadow-sm transition-all flex flex-col items-center justify-center gap-2 text-xs"><ShieldCheck size={20} /> Make This Person the Payer</button>)}
                    {(selectedMember.familyName || selectedMember.familyGroupId) && (<button onClick={async () => { var planMap = { 'FAMILY': 'SINGLE', 'SENIOR FAMILY': 'SENIOR', 'CORPORATE FAMILY': 'CORPORATE', 'MILITARY FAMILY': 'MILITARY', 'HD6 FAMILY': 'HD6', 'LIFETIME FAMILY': 'LIFETIME' }; var newPlan = planMap[selectedMember.type] || selectedMember.type; var newRate = getBaseRate(newPlan, selectedMember.billingMethod || 'Month-to-Month'); if (!window.confirm('Remove ' + selectedMember.firstName + ' ' + selectedMember.lastName + ' from their family group?\n\nPlan will change: ' + selectedMember.type + ' \u2192 ' + newPlan + '\nMonthly rate will become: $' + newRate)) return; try { var res = await fetch('/api/update-member', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ airtableId: selectedMember.airtableId, familyGroup: null, plan: newPlan, monthlyRate: newRate }) }); if (res.ok) { var updated = Object.assign({}, selectedMember, { familyName: '', familyGroupId: '', type: newPlan, monthlyRate: newRate }); setSelectedMember(updated); setMembers(function(prev) { return prev.map(function(m) { return m.airtableId === selectedMember.airtableId ? updated : m; }); }); showToast(selectedMember.firstName + ' removed from family group. Plan set to ' + newPlan + '.', 'success', 5000); } else { alert('Could not remove from family group.'); } } catch (err) { alert('Network error: ' + err.message); } }} className="bg-red-50 text-red-600 hover:bg-red-600 hover:text-white py-4 rounded-xl font-bold shadow-sm transition-all flex flex-col items-center justify-center gap-2 text-xs"><X size={20} /> Remove from Family</button>)}
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                     <button onClick={() => setLetterLang(letterLang === 'es' ? 'en' : 'es')} className="col-span-2 md:col-span-3 bg-slate-100 text-slate-700 hover:bg-slate-200 py-2 rounded-xl font-bold shadow-sm transition-all flex items-center justify-center gap-2 text-xs">&#127760; Letter language: {letterLang === 'es' ? 'Español' : 'English'} &mdash; tap to switch</button>
                     <button onClick={() => { 
  ...
  const directorName = isHarper ? 'Patrick Johnson' : 'Deanna Smithhisler'; 
  const addressBlock = selectedMember.address ? `${selectedMember.firstName} ${selectedMember.lastName}<br/>${selectedMember.address}<br/>${selectedMember.city}, ${selectedMember.state} ${selectedMember.zip}` : `${selectedMember.firstName} ${selectedMember.lastName}`; 
  const qrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=180x180&data=${encodeURIComponent(selectedMember.id)}&color=003d6b&bgcolor=ffffff`; 
  const w = window.open('', '_blank'); 
  
  let letterHtml = `<!DOCTYPE html><html><head><title>New Check-In System - ${selectedMember.firstName} ${selectedMember.lastName}</title><style>@page{size:letter portrait;margin:0.4in 0.6in}@media print{html,body{height:auto}*{-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}.page{page-break-inside:avoid;break-inside:avoid}}body{font-family:Arial,sans-serif;color:#1e293b;margin:0;padding:0}.page{max-width:7.5in;margin:0 auto;overflow:hidden}.hdr{background:#003d6b;padding:10px 24px;display:flex;align-items:center;gap:12px}.hdr-logo{height:24px;opacity:.95}.hdr-name{font-size:14px;font-weight:700;color:#fff}.hdr-sub{font-size:8px;color:#8bb8d9;letter-spacing:1px;margin-top:1px}.accent{height:3px;background:linear-gradient(to right,#dba51f,#dd6d22)}.window-addr{margin-top:0.9in;margin-left:0.5in;font-size:12px;line-height:1.5;color:#1e293b;font-weight:600;min-height:0.85in}.body{padding-top:0}.date{font-size:9px;color:#94a3b8;text-align:right;margin-bottom:6px}.greeting{font-size:12px;margin-bottom:6px;font-weight:700;color:#003d6b}.intro{font-size:10.5px;color:#475569;line-height:1.4;margin-bottom:8px}.section-title{font-size:9px;font-weight:900;color:#003d6b;text-transform:uppercase;letter-spacing:1.5px;margin-bottom:4px;padding-bottom:2px;border-bottom:2px solid #dba51f;margin-top:10px}.cred-section{display:flex;align-items:center;gap:24px;justify-content:center;margin:6px 0;padding:8px 16px;background:#f8fafc;border-radius:6px;border:1px solid #e2e8f0}.how-to{font-size:10px;color:#475569;line-height:1.4;margin:6px 0;padding:8px 12px;background:#eff6ff;border-radius:5px;border:1px solid #bfdbfe}.how-to strong{color:#003d6b}.portal-box{margin:8px 0;padding:10px;background:#f0fdf4;border-radius:6px;border:1.5px solid #bbf7d0;text-align:center}.box{border:1.5px solid #003d6b;border-radius:5px;overflow:hidden;margin-bottom:8px}.box-hdr{background:#003d6b;padding:4px 12px;font-size:8px;font-weight:700;color:#fff;letter-spacing:1.5px}.box-body{padding:0 12px}.row{display:flex;justify-content:space-between;padding:4px 0;border-bottom:1px solid #e2e8f0}.row:last-child{border-bottom:none}.lbl{font-size:10px;color:#64748b}.val{font-size:10px;font-weight:700;color:#003d6b}.sign{font-size:10px;margin-bottom:1px;margin-top:10px}.sign-name{font-size:10px;font-weight:700;color:#003d6b}.sign-title{font-size:9px;color:#94a3b8}</style></head><body><div class="page"><div class="hdr"><img src="https://pattersonhc.org/sites/default/files/wellness_white.png" class="hdr-logo" /><div><div class="hdr-name">${centerName}</div><div class="hdr-sub">${centerAddr} | ${centerPhone}</div></div></div><div class="accent"></div><div class="window-addr">${addressBlock}</div><div class="body"><div class="date">${new Date().toLocaleDateString(letterLang === 'es' ? 'es-US' : 'en-US', {month:'long',day:'numeric',year:'numeric'})}</div><div class="greeting">Dear ${selectedMember.firstName},</div><div class="intro">We are excited to announce a new digital check-in system at ${centerName}! You can now check in quickly at the front desk kiosk using your personal QR code or your name and PIN. You can also view your membership details online through our new Member Portal.</div><div class="section-title">Your Check-In Credentials</div><div class="cred-section"><div style="text-align:center"><p style="font-size:8px;font-weight:700;color:#64748b;text-transform:uppercase;letter-spacing:2px;margin:0 0 4px 0">Your QR Badge</p><img src="${qrUrl}" width="95" height="95" /><p style="font-size:8px;color:#64748b;margin:3px 0 0 0">Hold up to the kiosk camera</p></div><div style="text-align:center;padding-left:20px;border-left:1px solid #e2e8f0"><p style="font-size:8px;color:#94a3b8;text-transform:uppercase;letter-spacing:2px;margin:0">Your 4-Digit PIN</p><p style="font-size:24px;font-weight:900;letter-spacing:0.3em;color:#003d6b;margin:2px 0">${selectedMember.password}</p><p style="font-size:8px;color:#94a3b8;margin:0">Use when checking in by name</p><p style="font-size:8px;color:#94a3b8;text-transform:uppercase;letter-spacing:2px;margin:8px 0 0 0">Member ID</p><p style="font-size:14px;font-weight:900;color:#003d6b;font-family:monospace;margin:2px 0 0 0">${selectedMember.id}</p></div></div><div class="how-to"><strong>How to Check In at the Kiosk:</strong><br/>1. <strong>Scan</strong> &mdash; Hold this letter (or your phone) up to the kiosk camera to scan the QR code<br/>2. <strong>Search</strong> &mdash; Or type your name, select yourself, and enter your 4-digit PIN<br/>3. A green &ldquo;Welcome&rdquo; message confirms you are checked in!</div><div class="section-title">Your Online Member Portal</div><div class="intro">View your membership status, visit history, and QR code online anytime. No app download required &mdash; just visit the link below from any phone, tablet, or computer.</div><div class="portal-box"><p style="font-size:8px;font-weight:700;color:#64748b;text-transform:uppercase;letter-spacing:2px;margin:0 0 4px 0">Member Portal</p><p style="font-size:14px;font-weight:900;color:#16a34a;margin:0">wellness-kiosk-7k6p.vercel.app</p><p style="font-size:9px;color:#64748b;margin:4px 0 0 0">Tap &ldquo;Member Portal&rdquo; &bull; Search your name &bull; Enter your 4-digit PIN</p></div><div class="box"><div class="box-hdr">YOUR MEMBERSHIP SUMMARY</div><div class="box-body"><div class="row"><span class="lbl">Member ID</span><span class="val">${selectedMember.id}</span></div><div class="row"><span class="lbl">Membership Type</span><span class="val">${selectedMember.type}</span></div><div class="row"><span class="lbl">Home Center</span><span class="val">${selectedMember.center}</span></div><div class="row"><span class="lbl">Hours</span><span class="val">${centerHours.replace(/&amp;/g, '&')}</span></div></div></div><div style="font-size:9.5px;color:#64748b;line-height:1.4;margin-top:10px">Please keep this letter for your records. Your PIN is private &mdash; do not share it with others. If you have any questions about the new system, please ask the front desk staff and we will be happy to help!</div><div class="sign">We look forward to seeing you!</div><div class="sign-name">${directorName}</div><div class="sign-title">Director, ${centerName}</div></div></div></body></html>`; if (letterLang === 'es') { [["New Check-In System","Nuevo Sistema de Registro"],["We are excited to announce a new digital check-in system at ","¡Nos complace anunciar un nuevo sistema digital de registro en "],["! You can now check in quickly at the front desk kiosk using your personal QR code or your name and PIN. You can also view your membership details online through our new Member Portal.","! Ahora puede registrarse rápidamente en el quiosco de recepción usando su código QR personal o su nombre y PIN. También puede ver los detalles de su membresía en línea a través de nuestro nuevo Portal del Miembro."],["Your Check-In Credentials","Sus Credenciales de Registro"],["Your QR Badge","Su Código QR"],["Hold up to the kiosk camera","Muéstrelo a la cámara del quiosco"],["Your 4-Digit PIN","Su PIN de 4 Dígitos"],["Use when checking in by name","Úselo al registrarse por nombre"],["How to Check In at the Kiosk:","Cómo Registrarse en el Quiosco:"],["<strong>Scan</strong> &mdash; Hold this letter (or your phone) up to the kiosk camera to scan the QR code","<strong>Escanear</strong> &mdash; Muestre esta carta (o su teléfono) a la cámara del quiosco para escanear el código QR"],["<strong>Search</strong> &mdash; Or type your name, select yourself, and enter your 4-digit PIN","<strong>Buscar</strong> &mdash; O escriba su nombre, selecciónese y escriba su PIN de 4 dígitos"],["A green &ldquo;Welcome&rdquo; message confirms you are checked in!","¡Un mensaje verde de &laquo;Bienvenido&raquo; confirma que se ha registrado!"],["Your Online Member Portal","Su Portal del Miembro en Línea"],["View your membership status, visit history, and QR code online anytime. No app download required &mdash; just visit the link below from any phone, tablet, or computer.","Vea el estado de su membresía, su historial de visitas y su código QR en línea en cualquier momento. No necesita descargar ninguna aplicación &mdash; solo visite el enlace de abajo desde cualquier teléfono, tableta o computadora."],["Tap &ldquo;Member Portal&rdquo; &bull; Search your name &bull; Enter your 4-digit PIN","Toque &laquo;Portal del Miembro&raquo; &bull; Busque su nombre &bull; Escriba su PIN de 4 dígitos"],["Member Portal","Portal del Miembro"],["YOUR MEMBERSHIP SUMMARY","RESUMEN DE SU MEMBRESÍA"],[">Member ID<",">Número de Miembro<"],[">Membership Type<",">Tipo de Membresía<"],[">Home Center<",">Centro Principal<"],[">Hours<",">Horario<"],["Please keep this letter for your records. Your PIN is private &mdash; do not share it with others. If you have any questions about the new system, please ask the front desk staff and we will be happy to help!","Conserve esta carta para sus registros. Su PIN es privado &mdash; no lo comparta con otras personas. Si tiene alguna pregunta sobre el nuevo sistema, pregunte al personal de recepción y con gusto le ayudaremos."],["We look forward to seeing you!","¡Esperamos verle pronto!"],["Director, ","Director/a, "],["Dear ","Estimado/a "]].forEach(function(p){ letterHtml = letterHtml.split(p[0]).join(p[1]); }); } w.document.write(letterHtml); 
  
  w.document.close(); 
  setTimeout(() => w.print(), 500); 
}} className="bg-[#003d6b]/10 text-[#003d6b] hover:bg-[#003d6b] hover:text-white py-4 rounded-xl font-bold shadow-sm transition-all flex flex-col items-center justify-center gap-2 text-xs"><FileText size={20} /> Launch Letter</button>
                   <button onClick={() => { const isHarper = selectedMember.center && selectedMember.center.toLowerCase().includes('harper'); const centerName = isHarper ? 'Harper Wellness Center' : 'Anthony Wellness Center'; const centerAddr = isHarper ? '615 W 12th St, Harper, KS 67058' : '309 W Main St, Anthony, KS 67003'; const centerPhone = isHarper ? '(620) 896-1202' : '(620) 842-5190'; const centerHours = isHarper ? 'M-F 8am-12pm &amp; 5pm-8pm, Sat 9am-noon' : 'M-F 7am-8pm, Sat 8am-1pm'; const directorName = isHarper ? 'Patrick Johnson' : 'Deanna Smithhisler'; const addressBlock = selectedMember.address ? `${selectedMember.address}<br/>${selectedMember.city}, ${selectedMember.state} ${selectedMember.zip}` : 'Address not on file'; const qrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=180x180&data=${encodeURIComponent(selectedMember.id)}&color=003d6b&bgcolor=ffffff`; const w = window.open('', '_blank'); let letterHtml = `<!DOCTYPE html><html><head><title>Welcome - ${selectedMember.firstName} ${selectedMember.lastName}</title><style>@page{size:letter portrait;margin:0.4in 0.5in}@media print{html,body{height:auto;margin:0}*{-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}.page{page-break-inside:avoid;break-inside:avoid}}body{font-family:Arial,sans-serif;color:#1e293b;margin:0}.page{max-width:680px;margin:0 auto;overflow:hidden}.hdr{background:#003d6b;padding:14px 32px;display:flex;justify-content:space-between;align-items:center}.hdr-left{display:flex;align-items:center;gap:14px}.hdr-logo{height:30px;opacity:.95}.hdr-name{font-size:16px;font-weight:700;color:#fff}.hdr-sub{font-size:9px;color:#8bb8d9;letter-spacing:1px;margin-top:2px}.accent{height:3px;background:linear-gradient(to right,#dba51f,#dd6d22)}.body{padding:20px 32px}.top-row{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:16px}.addr{font-size:13px;line-height:1.5}.date{font-size:11px;color:#94a3b8}.greeting{font-size:14px;margin-bottom:8px;font-weight:700;color:#003d6b}.intro{font-size:12px;color:#475569;line-height:1.7;margin-bottom:14px}.box{border:1.5px solid #003d6b;border-radius:6px;overflow:hidden;margin-bottom:14px}.box-hdr{background:#003d6b;padding:6px 14px;font-size:9px;font-weight:700;color:#fff;letter-spacing:1.5px}.box-body{padding:0 14px}.row{display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid #e2e8f0}.row:last-child{border-bottom:none}.lbl{font-size:11px;color:#64748b}.val{font-size:11px;font-weight:700;color:#003d6b}.qr-section{text-align:center;margin:14px 0;padding:14px;background:#f8fafc;border-radius:8px;border:1px solid #e2e8f0;display:flex;align-items:center;gap:24px;justify-content:center}.qr-left{text-align:center}.qr-right{text-align:center;padding-left:24px;border-left:1px solid #e2e8f0}.pin-display{font-size:32px;font-weight:900;letter-spacing:0.3em;color:#003d6b;margin:6px 0}.pin-label{font-size:9px;color:#94a3b8;text-transform:uppercase;letter-spacing:2px}.how-to{font-size:11px;color:#475569;line-height:1.7;margin:12px 0;padding:10px 14px;background:#eff6ff;border-radius:6px;border:1px solid #bfdbfe}.how-to strong{color:#003d6b}.sign{font-size:12px;margin-bottom:2px;margin-top:14px}.sign-name{font-size:12px;font-weight:700;color:#003d6b}.sign-title{font-size:10px;color:#94a3b8}.ftr{border-top:2px solid #003d6b;padding:8px 32px;display:flex;justify-content:space-between;align-items:center;margin-top:14px}.ftr-l{font-size:9px;color:#94a3b8}.ftr-r{font-size:9px;color:#1080ad}</style></head><body><div class="page"><div class="hdr"><div class="hdr-left"><img src="https://pattersonhc.org/sites/default/files/wellness_white.png" class="hdr-logo" /><div><div class="hdr-name">${centerName}</div><div class="hdr-sub">${centerAddr} | ${centerPhone}</div></div></div></div><div class="accent"></div><div class="body"><div class="top-row"><div class="addr"><strong>${selectedMember.firstName} ${selectedMember.lastName}</strong><br/>${addressBlock}</div><div class="date">${new Date().toLocaleDateString(letterLang === 'es' ? 'es-US' : 'en-US', {month:'long',day:'numeric',year:'numeric'})}</div></div><div class="greeting">Welcome to ${centerName}!</div><div class="intro">We are excited to have you as a member! This letter contains your personal check-in credentials. Please keep this information for your records.</div><div class="box"><div class="box-hdr">YOUR MEMBERSHIP DETAILS</div><div class="box-body"><div class="row"><span class="lbl">Member ID</span><span class="val">${selectedMember.id}</span></div><div class="row"><span class="lbl">Membership Type</span><span class="val">${selectedMember.type}</span></div><div class="row"><span class="lbl">Home Center</span><span class="val">${selectedMember.center}</span></div><div class="row"><span class="lbl">Hours</span><span class="val">${centerHours.replace(/&amp;/g, '&')}</span></div></div></div><div class="qr-section"><div class="qr-left"><p style="font-size:9px;font-weight:700;color:#64748b;text-transform:uppercase;letter-spacing:2px;margin-bottom:8px">Your QR Badge</p><img src="${qrUrl}" width="130" height="130" /><p style="font-size:10px;color:#64748b;margin-top:6px">Scan at the kiosk</p></div><div class="qr-right"><p class="pin-label">Your 4-Digit PIN</p><p class="pin-display">${selectedMember.password}</p><p style="font-size:10px;color:#94a3b8">Enter at the kiosk when<br/>checking in by name</p></div></div><div class="how-to"><strong>How to Check In at the Front Desk Kiosk:</strong><br/>1. <strong>Option A:</strong> Hold this QR code up to the kiosk camera<br/>2. <strong>Option B:</strong> Type your name, select yourself, and enter your 4-digit PIN<br/>3. A green "Welcome" message confirms you're checked in!</div><div class="sign">Welcome aboard!</div><div class="sign-name">${directorName}</div><div class="sign-title">Director, ${centerName}</div></div><div class="ftr"><span class="ftr-l">${centerName} | Harper County, KS</span><span class="ftr-r">pattersonhc.org/wellness-centers</span></div></div></body></html>`; if (letterLang === 'es') { [["Welcome - ","Bienvenido/a - "],["Welcome to ","¡Bienvenido/a a "],["We are excited to have you as a member! This letter contains your personal check-in credentials. Please keep this information for your records.","¡Nos alegra tenerle como miembro! Esta carta contiene sus credenciales personales de registro. Conserve esta información para sus registros."],["YOUR MEMBERSHIP DETAILS","DETALLES DE SU MEMBRESÍA"],["Your QR Badge","Su Código QR"],["Scan at the kiosk","Escanéelo en el quiosco"],["Your 4-Digit PIN","Su PIN de 4 Dígitos"],["Enter at the kiosk when<br/>checking in by name","Introdúzcalo en el quiosco al<br/>registrarse por nombre"],["How to Check In at the Front Desk Kiosk:","Cómo Registrarse en el Quiosco de Recepción:"],["<strong>Option A:</strong> Hold this QR code up to the kiosk camera","<strong>Opción A:</strong> Muestre este código QR a la cámara del quiosco"],["<strong>Option B:</strong> Type your name, select yourself, and enter your 4-digit PIN","<strong>Opción B:</strong> Escriba su nombre, selecciónese y escriba su PIN de 4 dígitos"],["A green \"Welcome\" message confirms you're checked in!","¡Un mensaje verde de &laquo;Bienvenido&raquo; confirma que se ha registrado!"],["Welcome aboard!","¡Bienvenido/a a bordo!"],[">Member ID<",">Número de Miembro<"],[">Membership Type<",">Tipo de Membresía<"],[">Home Center<",">Centro Principal<"],[">Hours<",">Horario<"],["Director, ","Director/a, "],["Harper County, KS","Condado de Harper, KS"]].forEach(function(p){ letterHtml = letterHtml.split(p[0]).join(p[1]); }); } w.document.write(letterHtml); w.document.close(); setTimeout(() => w.print(), 500); }} className="bg-[#16a34a]/10 text-[#16a34a] hover:bg-[#16a34a] hover:text-white py-4 rounded-xl font-bold shadow-sm transition-all flex flex-col items-center justify-center gap-2 text-xs"><Mail size={20} /> Welcome Letter</button>
                   ...
                   <button onClick={() => {
                      ...
                      const qrUrl = 'https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=' + encodeURIComponent(m.id) + '&color=' + qrColor + '&bgcolor=ffffff';
                      const printChoice = window.confirm('OK = Front only (for printing on pre-printed back)\nCancel = Both sides');
                      const frontPage = '<div class="card-page front"><div class="qr-section"><div class="qr-box"><img class="qr-img" src="' + qrUrl + '" /></div><div class="qr-label">Scan to Enter</div></div><div class="info-section"><div class="center-name">' + (isHarper ? 'Harper' : 'Anthony') + '</div><div class="center-sub">Wellness Center</div><div class="member-id">ID: ' + m.id + '</div></div></div>';
                      const backPage = '<div class="card-page back"><div class="back-header">If Found, Please Return</div><div class="addresses"><div class="back-address"><strong>Anthony Wellness</strong>309 W Main St<br/>Anthony, KS 67003<br/>(620) 842-5190</div><div class="back-address"><strong>Harper Wellness</strong>615 W 12th St<br/>Harper, KS 67058<br/>(620) 896-1202</div></div><div class="back-footer">This card is the property of Patterson Health Center. It is non-transferable and intended for the exclusive use of the member named on the reverse side. If found, please return to one of the addresses above. Thank you!</div></div>';
                      const pages = printChoice ? frontPage : frontPage + backPage;
                      const html = '<!DOCTYPE html><html><head><title>VIP Access Card - ' + m.firstName + ' ' + m.lastName + '</title><link href="https://fonts.googleapis.com/css2?family=Montserrat:wght@300;500;700;900&display=swap" rel="stylesheet"><style>@page{size:3.375in 2.125in;margin:0}@media print{*{-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}.card-page{page-break-after:always}}body{font-family:Montserrat,Arial,sans-serif;margin:0;padding:0;background:#fff}.card-page{width:3.375in;height:2.125in;display:flex;flex-direction:row;overflow:hidden;box-sizing:border-box;position:relative}.card-page.front{background:' + bgGradient + ';color:#fff}.qr-section{width:1.4in;height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:0.15in;box-sizing:border-box}.qr-box{background:#fff;padding:0.08in;border-radius:0.08in;display:flex;align-items:center;justify-content:center;box-shadow:0 2px 8px rgba(0,0,0,0.3)}.qr-img{width:1in;height:1in;display:block}.qr-label{font-size:7px;font-weight:700;color:rgba(255,255,255,0.85);text-transform:uppercase;letter-spacing:1.5px;margin-top:0.08in}.info-section{flex:1;padding:0 0.15in 0 0.1in;display:flex;flex-direction:column;justify-content:center;align-items:flex-start;z-index:10}.center-name{font-size:22px;font-weight:900;text-transform:uppercase;margin-bottom:0;line-height:1;text-shadow:0 2px 4px rgba(0,0,0,0.5);letter-spacing:1px}.center-sub{font-size:11px;font-weight:300;text-transform:uppercase;margin-top:4px;letter-spacing:3px;color:rgba(255,255,255,0.95);margin-bottom:0.12in}.member-id{font-size:9px;font-family:monospace;font-weight:900;background:rgba(0,0,0,0.3);padding:0.04in 0.08in;border-radius:0.04in;border:1px solid rgba(255,255,255,0.15);letter-spacing:1px}.card-page.back{background:#fff;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:0.2in;text-align:center}.back-header{font-size:11px;font-weight:900;color:#003d6b;text-transform:uppercase;letter-spacing:2px;margin-bottom:0.15in}.addresses{display:flex;gap:0.25in;justify-content:center;width:100%}.back-address{font-size:9px;color:#475569;line-height:1.5;text-align:center}.back-address strong{display:block;color:#003d6b;font-weight:900;font-size:10px;margin-bottom:3px;text-transform:uppercase;letter-spacing:0.5px}.back-footer{font-size:7px;color:#94a3b8;text-align:center;line-height:1.4;margin-top:0.18in;padding:0 0.1in}</style></head><body>' + pages + '</body></html>';
                      const w = window.open('', '_blank');
                      w.document.write(html);
                      w.document.close();
...
{/* PAYMENT MODAL */}
      {paymentModal && (() => {
        ...
        const daysRemaining = daysInMonth - today.getDate() + 1;
        const proratedAmount = proratePayment === 16 ? 16 : proratePayment === 5 ? 5 : 0;
        const displayAmount = customPayAmount ? parseFloat(customPayAmount) : (proratePayment ? proratedAmount : fullRate);
        return (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white rounded-2xl w-full max-w-sm p-8 relative shadow-2xl">
            ...
            <div className="grid grid-cols-2 gap-4 mb-5">
              <div>
                <label className="text-xs font-bold text-[#dd6d22] uppercase tracking-widest mb-2 block">Renewal Date <span className="normal-case text-slate-300">(optional)</span></label>
                <input type="date" id="renewalDate" className="w-full p-3 bg-orange-50 border border-orange-200 rounded-xl outline-none focus:border-[#dd6d22] text-sm font-bold text-[#dd6d22] placeholder-orange-300" />
                <p className="text-[9px] text-slate-400 mt-1">Leave blank for auto (1 month). Set for prepays.</p>
              </div>
              <div>
                <label className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2 block">Assign to Center</label>
                <select id="paymentCenter" defaultValue={paymentModal.center || 'Anthony'} className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-[#16a34a] text-sm font-bold text-[#001f3f]">
                  <option value="Anthony">Anthony</option>
                  <option value="Harper">Harper</option>
                </select>
              </div>
         ...
         <div className="mb-5">
              ...
              <div className="grid grid-cols-3 gap-2 mb-3">
                <button type="button" onClick={() => { setProratePayment(false); setCustomPayAmount(''); }} className={"p-3 rounded-xl border-2 text-center transition-all " + (!proratePayment && !customPayAmount ? "border-[#16a34a] bg-green-50" : "border-slate-200 bg-white hover:bg-slate-50")}><p className="text-lg font-black text-[#16a34a]">${fullRate.toFixed(2)}</p><p className="text-[9px] font-bold text-slate-400 uppercase">Full Month</p></button>
                <button type="button" onClick={() => { setProratePayment(16); setCustomPayAmount(''); }} className={"p-3 rounded-xl border-2 text-center transition-all " + (proratePayment === 16 && !customPayAmount ? "border-[#f59e0b] bg-amber-50" : "border-slate-200 bg-white hover:bg-slate-50")}><p className="text-lg font-black text-[#f59e0b]">$16.00</p><p className="text-[9px] font-bold text-slate-400 uppercase">Rest of Month</p></button>
                <button type="button" onClick={() => { setProratePayment(5); setCustomPayAmount(''); }} className={"p-3 rounded-xl border-2 text-center transition-all " + (proratePayment === 5 && !customPayAmount ? "border-[#1080ad] bg-blue-50" : "border-slate-200 bg-white hover:bg-slate-50")}><p className="text-lg font-black text-[#1080ad]">$5.00</p><p className="text-[9px] font-bold text-slate-400 uppercase">Day Pass</p></button>
              </div>
              <div className={"rounded-xl border-2 p-3 transition-all " + (customPayAmount ? "border-[#8b5cf6] bg-purple-50" : "border-slate-200 bg-white")}>
                <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-1">Custom Amount (for prepays, partial payments, etc.)</p>
                <div className="flex items-center gap-2">
                  <span className="text-lg font-black text-slate-400">$</span>
                  <input type="number" step="0.01" min="0" placeholder={fullRate.toFixed(2)} value={customPayAmount} onChange={function(e) { setCustomPayAmount(e.target.value); if (e.target.value) { setProratePayment(false); } }} className="flex-1 p-2 bg-white border border-slate-200 rounded-lg text-lg font-black text-[#001f3f] outline-none focus:border-[#8b5cf6] placeholder-slate-300" />
                </div>
              </div>
            </div>
            <div className="mb-5">
              <label className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2 block">Note (Optional)</label>
              <input type="text" value={paymentNote} onChange={(e) => setPaymentNote(e.target.value)} placeholder="e.g. 24/7 card replacement, June prepay, lost card fee" className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-[#16a34a] text-sm text-[#001f3f] placeholder-slate-300" />
            </div>
            <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3">Payment Method</p>
            {checkNumber !== '' && (
              <div className="mb-3">
                <div className="flex gap-2">
                  <input value={checkNumber === 'showInput' ? '' : checkNumber} onChange={(e) => setCheckNumber(e.target.value || 'showInput')} onKeyDown={function(e) { if (e.key === 'Enter') { var cn = checkNumber && checkNumber !== 'showInput' ? checkNumber.trim() : ''; if (!cn) return; var payMethod = 'Check #' + cn; var amtLabel = fullRate > 0 ? ' ($' + displayAmount.toFixed(2) + ')' : ''; if (!window.confirm('Log Check payment' + amtLabel + ' for ' + paymentModal.firstName + ' ' + paymentModal.lastName + '?')) return; fetch('/api/log-payment', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ airtableId: paymentModal.airtableId, memberName: paymentModal.firstName + ' ' + paymentModal.lastName, method: payMethod, amount: displayAmount, currentDueDate: paymentModal.nextPayment, prorated: proratePayment, proratedAmount: proratePayment ? proratedAmount : null, paymentDate: document.getElementById('paymentDate') ? document.getElementById('paymentDate').value : null, renewalDateOverride: document.getElementById('renewalDate') ? document.getElementById('renewalDate').value : null, paymentCenter: document.getElementById('paymentCenter') ? document.getElementById('paymentCenter').value : null, note: paymentNote }) }).then(function(r) { return r.json(); }).then(function(result) { if (result.success) { setMembers(function(prev) { return prev.map(function(mem) { return mem.airtableId === paymentModal.airtableId ? Object.assign({}, mem, { status: 'ACTIVE', nextPayment: result.nextPaymentDue, paymentMethod: payMethod }) : mem; }); }); if (selectedMember && selectedMember.airtableId === paymentModal.airtableId) { setSelectedMember(Object.assign({}, selectedMember, { status: 'ACTIVE', nextPayment: result.nextPaymentDue, paymentMethod: payMethod })); } setPaymentModal(null); setCheckNumber(''); setCustomPayAmount(''); setPaymentNote(''); showToast('Payment logged! Next due: ' + result.nextPaymentDue, 'success', 5000); } else { alert('Error: ' + result.error); } }).catch(function() { alert('Network error.'); }); } }} placeholder="Enter check number..." className="flex-1 p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-[#16a34a] text-sm font-bold text-[#001f3f] placeholder-slate-300" autoFocus />
                  <button onClick={function() { var cn = checkNumber && checkNumber !== 'showInput' ? checkNumber.trim() : ''; if (!cn) { alert('Please enter a check number.'); return; } var payMethod = 'Check #' + cn; var amtLabel = fullRate > 0 ? ' ($' + displayAmount.toFixed(2) + ')' : ''; if (!window.confirm('Log Check payment' + amtLabel + ' for ' + paymentModal.firstName + ' ' + paymentModal.lastName + '?')) return; fetch('/api/log-payment', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ airtableId: paymentModal.airtableId, memberName: paymentModal.firstName + ' ' + paymentModal.lastName, method: payMethod, amount: displayAmount, currentDueDate: paymentModal.nextPayment, prorated: proratePayment, proratedAmount: proratePayment ? proratedAmount : null, paymentDate: document.getElementById('paymentDate') ? document.getElementById('paymentDate').value : null, renewalDateOverride: document.getElementById('renewalDate') ? document.getElementById('renewalDate').value : null, paymentCenter: document.getElementById('paymentCenter') ? document.getElementById('paymentCenter').value : null, note: paymentNote }) }).then(function(r) { return r.json(); }).then(function(result) { if (result.success) { setMembers(function(prev) { return prev.map(function(mem) { return mem.airtableId === paymentModal.airtableId ? Object.assign({}, mem, { status: 'ACTIVE', nextPayment: result.nextPaymentDue, paymentMethod: payMethod }) : mem; }); }); if (selectedMember && selectedMember.airtableId === paymentModal.airtableId) { setSelectedMember(Object.assign({}, selectedMember, { status: 'ACTIVE', nextPayment: result.nextPaymentDue, paymentMethod: payMethod })); } setPaymentModal(null); setCheckNumber(''); setCustomPayAmount(''); setPaymentNote(''); showToast('Payment logged! Next due: ' + result.nextPaymentDue, 'success', 5000); } else { alert('Error: ' + result.error); } }).catch(function() { alert('Network error.'); }); }} className="bg-[#16a34a] text-white px-5 rounded-xl font-bold text-sm hover:bg-green-700 transition-colors shadow-sm">Submit</button>
                </div>
              </div>
            )}
            <div className="grid grid-cols-2 gap-3 mb-6">
              {['Cash', 'Check', 'Card', 'ACH', 'Other'].map(m => (
                <button key={m} onClick={async () => {
                  ...
                  if (m === 'Other' && (!paymentNote || paymentNote.trim().length < 3)) { alert('Please add a note explaining why this payment is being logged as "Other" (e.g., free week promo, complimentary visit, etc.) before selecting Other.'); return; }
                  ...
                  try {
                    const res = await fetch('/api/log-payment', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ airtableId: paymentModal.airtableId, memberName: `${paymentModal.firstName} ${paymentModal.lastName}`, method: payMethod, amount: displayAmount, currentDueDate: paymentModal.nextPayment, prorated: proratePayment, proratedAmount: proratePayment ? proratedAmount : null, paymentDate: document.getElementById('paymentDate') ? document.getElementById('paymentDate').value : null, renewalDateOverride: document.getElementById('renewalDate') ? document.getElementById('renewalDate').value : null, paymentCenter: document.getElementById('paymentCenter') ? document.getElementById('paymentCenter').value : null, note: paymentNote }) });
                    const result = await res.json();
                    if (result.success) {
                      setMembers(prev => prev.map(mem => { if (mem.airtableId === paymentModal.airtableId) return { ...mem, status: 'ACTIVE', nextPayment: result.nextPaymentDue }; if (result.familyMembersUpdated > 0 && paymentModal.familyName && mem.familyName && mem.familyName === paymentModal.familyName) return { ...mem, status: 'ACTIVE', nextPayment: result.nextPaymentDue }; return mem; }));
              ...
              const receiptAmt = customPayAmount ? parseFloat(customPayAmount).toFixed(2) : (proratePayment ? proratedAmount.toFixed(2) : fullRate.toFixed(2));
          ...
          <button onClick={() => { setPaymentModal(null); setCheckNumber(''); setCustomPayAmount(''); setPaymentNote(''); }} className="w-full text-slate-400 text-sm font-bold hover:text-slate-600">Cancel</button>
          </div>
        </div>
        );
      })()}
...
{/* ISSUE REFUND MODAL */}
      {refundModal && (
        <div className="fixed inset-0 z-[80] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white rounded-2xl w-full max-w-md p-8 relative shadow-2xl">
            <button onClick={() => { setRefundModal(null); }} className="absolute top-4 right-4 text-slate-300 hover:text-red-500"><X size={20}/></button>
            <div className="text-center mb-6">
              <RotateCcw size={40} className="text-orange-500 mx-auto mb-3" />
              <h3 className="text-xl font-black text-[#001f3f]">Issue Refund</h3>
              <p className="text-sm text-slate-400 mt-1">{refundModal.member.firstName} {refundModal.member.lastName}</p>
            </div>
            {refundModal.originalPayment && (
              <div className="bg-blue-50 border border-blue-100 rounded-xl p-3 mb-4">
                <p className="text-[10px] font-black text-blue-600 uppercase tracking-widest mb-1">Refunding Payment</p>
                <p className="text-sm font-bold text-[#001f3f]">${parseFloat(refundModal.originalPayment.amount).toFixed(2)} · {refundModal.originalPayment.method} · {refundModal.originalPayment.date ? new Date(refundModal.originalPayment.date + 'T00:00:00').toLocaleDateString('en-US', {month: 'short', day: 'numeric', year: 'numeric'}) : 'Unknown date'}</p>
              </div>
            )}
            {!refundModal.originalPayment && (
              <div className="bg-amber-50 border border-amber-100 rounded-xl p-3 mb-4 flex items-start gap-2">
                <AlertCircle size={14} className="text-amber-500 shrink-0 mt-0.5" />
                <p className="text-[11px] text-amber-700 font-medium">Standalone refund (not linked to a specific payment). For a refund tied to a specific past payment, go to the Payments tab and click the refund icon on that payment row.</p>
              </div>
            )}
            <div className="space-y-4 mb-6">
              <div>
                <label className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2 block">Refund Date</label>
                <input type="date" value={refundDate} onChange={(e) => setRefundDate(e.target.value)} className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-orange-500 text-sm font-bold text-[#001f3f]" />
              </div>
              <div>
                <label className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2 block">Refund Amount</label>
                <div className="flex items-center gap-2">
                  <span className="text-lg font-black text-slate-400">$</span>
                  <input type="number" step="0.01" min="0" value={refundAmount} onChange={(e) => setRefundAmount(e.target.value)} placeholder="0.00" className="flex-1 p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-orange-500 text-sm font-bold text-[#001f3f]" />
                </div>
              </div>
              <div>
                <label className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2 block">Refund Method</label>
                <select value={refundMethod} onChange={(e) => setRefundMethod(e.target.value)} className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-orange-500 text-sm font-bold text-[#001f3f]">
                  <option value="Cash">Cash</option>
                  <option value="Check">Check</option>
                  <option value="Card">Card</option>
                  <option value="ACH">ACH</option>
              ...
              {refundMethod === 'Check' && (
                <div>
                  <label className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2 block">Check Number</label>
                  <input type="text" value={refundCheckNumber} onChange={(e) => setRefundCheckNumber(e.target.value)} placeholder="e.g. 1234" className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-orange-500 text-sm font-bold text-[#001f3f]" />
                </div>
              )}
              <div>
                <label className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2 block">Reason (Optional)</label>
                <textarea value={refundReason} onChange={(e) => setRefundReason(e.target.value)} placeholder="e.g. Overcharged, Membership canceled, Wrong amount" className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-orange-500 text-sm min-h-[60px]" />
              </div>
            </div>
            <div className="flex gap-3">
              <button onClick={async function() {
                var amt = parseFloat(refundAmount) || 0;
                if (amt <= 0) { alert('Refund amount must be greater than zero.'); return; }
                if (!refundDate) { alert('Refund date is required.'); return; }
                var methodLabel = refundMethod === 'Check' && refundCheckNumber.trim() ? 'Check #' + refundCheckNumber.trim() : refundMethod;
                try {
                  var res = await fetch('/api/log-refund', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                      airtableId: refundModal.member.airtableId,
                      memberName: refundModal.member.firstName + ' ' + refundModal.member.lastName,
                      method: methodLabel,
                      amount: amt,
                      refundDate: refundDate,
                      originalPaymentId: refundModal.originalPayment ? refundModal.originalPayment.airtableId : null,
                      reason: refundReason
                    })
                  });
                  var result = await res.json();
                  if (result.success) {
                    setPayments(function(prev) { return prev.concat([{ airtableId: result.recordId, memberRecId: refundModal.member.airtableId, memberId: refundModal.member.id, amount: amt, method: refundMethod, date: refundDate, notes: (refundReason ? refundReason + ' | ' : '') + 'REFUND - Issued by staff via Wellness Hub', checkNumber: refundMethod === 'Check' ? refundCheckNumber : '', isRefund: true, refundOf: refundModal.originalPayment ? refundModal.originalPayment.airtableId : '' }]); });
                    setRefundModal(null);
                    showToast('Refund of $' + amt.toFixed(2) + ' logged.', 'success', 4000);
                  } else {
                    alert('Error: ' + (result.error || 'Could not log refund.'));
                  }
                } catch (err) {
                  alert('Network error.');
                }
              }} className="flex-1 bg-orange-500 text-white py-3 rounded-xl font-bold text-sm hover:bg-orange-600 transition-colors">Log Refund</button>
              <button onClick={() => { setRefundModal(null); }} className="flex-1 bg-slate-100 text-slate-600 py-3 rounded-xl font-bold text-sm hover:bg-slate-200 transition-colors">Cancel</button>
            </div>
          </div>
        </div>
      )}

      {/* EDIT PAYMENT MODAL */}       {editPaymentModal && (() => {         var mem = members.find(function(m) { return m.airtableId === editPaymentModal.memberRecId; });         var memName = mem ? (mem.firstName + ' ' + mem.lastName) : 'Unknown';         return (           <div className="fixed inset-0 z-[80] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">             <div className="bg-white rounded-2xl w-full max-w-md p-8 relative shadow-2xl">               <button onClick={function() { setEditPaymentModal(null); }} className="absolute top-4 right-4 text-slate-300 hover:text-red-500"><X size={20}/></button>               <div className="text-center mb-6">                 <CreditCard size={40} className="text-[#1080ad] mx-auto mb-3" />                 <h3 className="text-xl font-black text-[#001f3f]">Edit Payment</h3>                 <p className="text-sm text-slate-400 mt-1">{editPaymentModal._isVisitor ? (() => { var vis = visitors.find(function(v) { return v.airtableId === editPaymentModal._visitorAirtableId; }); return vis ? vis.firstName + ' ' + vis.lastName + ' (Visitor)' : 'Visitor'; })() : memName}</p>               </div>               <div className="bg-amber-50 border border-amber-100 rounded-xl p-3 mb-5 flex items-start gap-2">                 <AlertCircle size={14} className="text-amber-500 shrink-0 mt-0.5" />                 <p className="text-[11px] text-amber-700 font-medium">Use this to correct payment dates, amounts, or methods. Changes update Airtable immediately and there is no undo history.</p>               </div>               <div className="space-y-4 mb-6">                 <div>                   <label className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2 block">Payment Date</label>                   <input type="date" id="ep_date" defaultValue={editPaymentModal.date || ''} className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-[#1080ad] text-sm font-bold text-[#001f3f]" />                 </div>                 <div>                   <label className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2 block">Amount</label>                   <div className="flex items-center gap-2">                     <span className="text-lg font-black text-slate-400">$</span>                     <input type="number" step="0.01" min="0" id="ep_amount" defaultValue={parseFloat(editPaymentModal.amount) || 0} className="flex-1 p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-[#1080ad] text-sm font-bold text-[#001f3f]" />                   </div>                 </div>                 <div>                   <label className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2 block">Payment Method</label>                   <select id="ep_method" defaultValue={editPaymentModal.method || 'Cash'} onChange={function(e) { var row = document.getElementById('ep_checkrow'); if (row) row.style.display = e.target.value === 'Check' ? 'block' : 'none'; }} className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-[#1080ad] text-sm font-bold text-[#001f3f]">                     <option value="Cash">Cash</option>                     <option value="Check">Check</option>                     <option value="Card">Card</option>                     <option value="ACH">ACH</option>                     <option value="Other">Other</option>                   </select>                 </div>                 <div id="ep_checkrow" style={{display: editPaymentModal.method === 'Check' ? 'block' : 'none'}}>                   <label className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2 block">Check Number</label>                   <input type="text" id="ep_checknum" defaultValue={editPaymentModal.checkNumber || ''} placeholder="e.g. 1234" className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-[#1080ad] text-sm font-bold text-[#001f3f]" />                 </div>                 <div>                   <label className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2 block">Notes (Optional)</label>                   <textarea id="ep_notes" defaultValue={editPaymentModal.notes || ''} placeholder="Optional notes about this payment" className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-[#1080ad] text-sm min-h-[60px]" />                 </div>               </div>               <div className="flex gap-3">                 <button onClick={async function() {                   var newDate = document.getElementById('ep_date').value;                   var newAmount = parseFloat(document.getElementById('ep_amount').value) || 0;                   var newMethod = document.getElementById('ep_method').value;                   var newCheckNum = document.getElementById('ep_checknum') ? document.getElementById('ep_checknum').value.trim() : '';                   var newNotes = document.getElementById('ep_notes').value;                   if (!newDate || newAmount <= 0) { alert('Date and amount are required.'); return; }                   try {                     if (editPaymentModal._isVisitor) {                       var res = await fetch('/api/update-visitor', {                         method: 'POST',                         headers: { 'Content-Type': 'application/json' },                         body: JSON.stringify({                           visitorAirtableId: editPaymentModal._visitorAirtableId,                           amountPaid: newAmount,                           paymentMethod: newMethod,                           purchaseDate: newDate                         })                       });                       var result = await res.json();                       if (result.success) {                         setVisitors(function(prev) { return prev.map(function(v) { return v.airtableId === editPaymentModal._visitorAirtableId ? Object.assign({}, v, { amountPaid: newAmount, paymentMethod: newMethod, purchaseDate: newDate }) : v; }); });                         setEditPaymentModal(null);                         showToast('Visitor payment updated.', 'success', 3000);                       } else {                         alert('Error: ' + (result.error || 'Could not update visitor payment.'));                       }                     } else {                       var res = await fetch('/api/update-payment', {                         method: 'POST',                         headers: { 'Content-Type': 'application/json' },                         body: JSON.stringify({                           paymentId: editPaymentModal.airtableId,                           date: newDate,                           amount: newAmount,                           method: newMethod,                           checkNumber: newMethod === 'Check' ? newCheckNum : '',                           notes: newNotes                         })                       });                       var result = await res.json();                       if (result.success) {                         setPayments(function(prev) { return prev.map(function(p) { return p.airtableId === editPaymentModal.airtableId ? Object.assign({}, p, { date: newDate, amount: newAmount, method: newMethod, checkNumber: newMethod === 'Check' ? newCheckNum : '', notes: newNotes }) : p; }); });                         setEditPaymentModal(null);                         showToast('Payment updated.', 'success', 3000);                       } else {                         alert('Error: ' + (result.error || 'Could not update payment.'));                       }                     }                   } catch (err) {                     alert('Network error.');                   }                 }} className="flex-1 bg-[#1080ad] text-white py-3 rounded-xl font-bold text-sm hover:bg-blue-700 transition-colors">Save Changes</button>                 <button onClick={function() { setEditPaymentModal(null); }} className="flex-1 bg-slate-100 text-slate-600 py-3 rounded-xl font-bold text-sm hover:bg-slate-200 transition-colors">Cancel</button>               </div>             </div>           </div>         );       })()}       {/* DUPLICATES MODAL */}
      {showDuplicatesModal && (() => {
        const pairKey = (a, b) => [a.airtableId, b.airtableId].sort().join('::');
        const candidates = [];
        const seen = new Set();
        for (let i = 0; i < members.length; i++) {
          for (let j = i + 1; j < members.length; j++) {
            ...
            if (seen.has(key) || dismissedDuplicatePairs.includes(key)) continue;
            const nameMatch = (a.firstName + a.lastName).toLowerCase().replace(/\s+/g, '') === (b.firstName + b.lastName).toLowerCase().replace(/\s+/g, '');
            const emailMatch = a.email && b.email && a.email.toLowerCase().trim() === b.email.toLowerCase().trim();
            const phoneMatch = a.phone && b.phone && a.phone.replace(/\D/g, '') === b.phone.replace(/\D/g, '') && a.phone.replace(/\D/g, '').length >= 10;
            if (nameMatch || emailMatch || phoneMatch) {
              const confidence = (nameMatch && (emailMatch || phoneMatch)) ? 'high' : nameMatch ? 'medium' : 'low';
              const aPayments = payments.filter(p => p.memberRecId === a.airtableId).length;
              const bPayments = payments.filter(p => p.memberRecId === b.airtableId).length;
              const aVisits = visits.filter(v => v.name === (a.firstName + ' ' + b.lastName)).length;
              const bVisits = visits.filter(v => v.name === (b.firstName + ' ' + b.lastName)).length;
              candidates.push({ a, b, key, confidence, aPayments, bPayments, aVisits, bVisits, nameMatch, emailMatch, phoneMatch });
              seen.add(key);
            }
          }
        }
        ...
        const handleMerge = async (keeper, duplicate) => {
          if (!window.confirm(`Merge ${duplicate.firstName} ${duplicate.lastName} (${duplicate.id}) INTO ${keeper.firstName} ${keeper.lastName} (${keeper.id})?\n\n• All payments, visits, and card requests will move to ${keeper.id}\n• ${duplicate.id} will be marked inactive and hidden from the system\n• Anyone with ${duplicate.id}'s old QR badge will still be able to check in (it'll log them in as ${keeper.id})\n\nThis action cannot be easily undone. Continue?`)) return;
          setMergingPair({ keeper: keeper.airtableId, duplicate: duplicate.airtableId });
          try {
            const res = await fetch('/api/merge-members', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({
                keeperAirtableId: keeper.airtableId,
                duplicateAirtableId: duplicate.airtableId,
                duplicateMemberId: duplicate.id,
                mergedBy: user ? user.name : 'Unknown'
              })
            });
            const result = await res.json();
            if (result.success) {
              setMembers(prev => prev.filter(m => m.airtableId !== duplicate.airtableId).map(m => m.airtableId === keeper.airtableId ? { ...m, legacyMemberIds: (m.legacyMemberIds ? m.legacyMemberIds + ',' : '') + duplicate.id } : m));
              showToast(`Merged. Moved ${result.paymentsRelinked} payment(s), ${result.visitsRelinked} visit(s), and ${result.cardsRelinked} card request(s) to ${keeper.id}.`, 'success', 6000);
            } else {
        ...
        const handleDismiss = (pair) => {
          setDismissedDuplicatePairs(prev => [...prev, pair.key]);
        };

        return (
          <div className="fixed inset-0 z-[80] flex items-center justify-center p-4 bg-[#001f3f]/90 backdrop-blur-md">
            <div className="bg-white rounded-3xl w-full max-w-5xl p-8 relative shadow-2xl max-h-[90vh] overflow-y-auto">
              <button onClick={() => setShowDuplicatesModal(false)} className="absolute top-6 right-6 text-slate-300 hover:text-red-500 transition-all z-10"><X size={24}/></button>
              <div className="mb-6">
                <h3 className="text-3xl font-black text-[#001f3f] mb-2 tracking-tight">Find Duplicates</h3>
                <p className="text-slate-500 font-medium">Review candidate duplicate pairs. Merging moves all payments, visits, and card requests to the keeper, and the duplicate's old badge keeps working.</p>
              </div>
              {candidates.length === 0 ? (
                <div className="bg-green-50 border border-green-200 rounded-2xl p-12 text-center">
                  <CheckCircle size={48} className="text-[#16a34a] mx-auto mb-4" />
                  <p className="text-lg font-black text-[#16a34a] mb-1">No duplicates found.</p>
                  <p className="text-sm text-slate-500">Every active member has a unique name, email, and phone combination.{dismissedDuplicatePairs.length > 0 && ` (${dismissedDuplicatePairs.length} pair${dismissedDuplicatePairs.length !== 1 ? 's' : ''} previously dismissed as not duplicates.)`}</p>
                  {dismissedDuplicatePairs.length > 0 && (
                    <button onClick={() => { if (window.confirm('Reset all dismissed pairs? They will reappear for review.')) setDismissedDuplicatePairs([]); }} className="mt-4 text-xs font-bold text-slate-400 hover:text-slate-600 underline">Reset dismissed pairs</button>
                  )}
                </div>
              ) : (
                <div className="space-y-4">
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">{candidates.length} candidate pair{candidates.length !== 1 ? 's' : ''} found</p>
                  {candidates.map(pair => {
                    const isMerging = mergingPair && (mergingPair.keeper === pair.a.airtableId || mergingPair.keeper === pair.b.airtableId);
                    const confidenceColor = pair.confidence === 'high' ? 'red' : pair.confidence === 'medium' ? 'amber' : 'slate';
                    const confidenceLabel = pair.confidence === 'high' ? 'Very Likely Duplicate' : pair.confidence === 'medium' ? 'Possible Duplicate' : 'Low Confidence';
                    const matchReasons = [];
                    if (pair.nameMatch) matchReasons.push('Same name');
                    if (pair.emailMatch) matchReasons.push('Same email');
                    if (pair.phoneMatch) matchReasons.push('Same phone');
                    return (
                      <div key={pair.key} className={`bg-white border-2 rounded-2xl p-5 ${pair.confidence === 'high' ? 'border-red-200' : pair.confidence === 'medium' ? 'border-amber-200' : 'border-slate-200'}`}>
                        <div className="flex items-center justify-between mb-4">
                          <div className="flex items-center gap-3">
                            <span className={`px-3 py-1 rounded-full text-[10px] font-black tracking-widest uppercase bg-${confidenceColor}-100 text-${confidenceColor}-700`}>{confidenceLabel}</span>
                            <span className="text-xs font-bold text-slate-400">{matchReasons.join(' · ')}</span>
                          </div>
                          <button onClick={() => handleDismiss(pair)} className="text-xs font-bold text-slate-400 hover:text-slate-600 underline">Not a Duplicate</button>
                        </div>
                        <div className="grid grid-cols-2 gap-4">
                          {[pair.a, pair.b].map((mem, idx) => {
                            const otherMem = idx === 0 ? pair.b : pair.a;
                            const memPayments = idx === 0 ? pair.aPayments : pair.bPayments;
                            const memVisits = idx === 0 ? pair.aVisits : pair.bVisits;
                            return (
                              <div key={mem.airtableId} className="bg-slate-50 rounded-xl p-4 border border-slate-100">
                                <div className="flex items-start gap-3 mb-3">
                                  <MemberPhoto src={mem.photoUrl} name={mem.firstName} size={40} />
                                  <div className="flex-1 min-w-0">
                                    <p className="font-black text-[#001f3f] truncate">{mem.firstName} {mem.lastName}</p>
                                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{mem.id} · {mem.type}</p>
                                  </div>
                                </div>
                                <div className="space-y-1 text-xs text-slate-600 mb-3">
                                  <p><strong className="text-slate-400 uppercase text-[9px] tracking-widest block">Center</strong>{mem.center || '—'}</p>
                                  <p><strong className="text-slate-400 uppercase text-[9px] tracking-widest block mt-2">Email</strong>{mem.email || '—'}</p>
                                  <p><strong className="text-slate-400 uppercase text-[9px] tracking-widest block mt-2">Phone</strong>{mem.phone || '—'}</p>
                                  <p><strong className="text-slate-400 uppercase text-[9px] tracking-widest block mt-2">Visits / Payments</strong>{mem.visits} visits · {memPayments} payment{memPayments !== 1 ? 's' : ''}</p>
                                  <p><strong className="text-slate-400 uppercase text-[9px] tracking-widest block mt-2">Started</strong>{mem.startDate ? new Date(mem.startDate + 'T00:00:00').toLocaleDateString('en-US', {month: 'short', day: 'numeric', year: 'numeric'}) : '—'}</p>
                                  {mem.notes && <p className="mt-2 text-[10px] text-slate-500 italic bg-amber-50 border border-amber-100 px-2 py-1 rounded">{mem.notes.length > 80 ? mem.notes.slice(0, 80) + '…' : mem.notes}</p>}
                                </div>
                                <button onClick={() => handleMerge(mem, otherMem)} disabled={isMerging} className={`w-full py-2 rounded-lg text-xs font-bold transition-colors ${isMerging ? 'bg-slate-200 text-slate-400 cursor-not-allowed' : 'bg-[#1080ad] text-white hover:bg-blue-700'}`}>{isMerging ? 'Merging…' : `Keep ${mem.id}, Merge ${otherMem.id} In`}</button>
                              </div>
                            );
                          })}
                        </div>
      ...
      {/* BRANDED TOAST NOTIFICATION */}      {toast && (
        <div className="fixed top-6 right-6 z-[300] animate-in fade-in slide-in-from-top-4 duration-300">
          <div className={"px-6 py-4 rounded-2xl shadow-2xl border-2 flex items-center gap-4 max-w-md " + (toast.type === 'success' ? "bg-green-50 border-green-300 text-green-800" : toast.type === 'warning' ? "bg-amber-50 border-amber-300 text-amber-800" : toast.type === 'error' ? "bg-red-50 border-red-300 text-red-800" : "bg-blue-50 border-blue-300 text-blue-800")}>
            ...
            <button onClick={() => setToast(null)} className="text-slate-300 hover:text-slate-500 shrink-0"><X size={16}/></button>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[80] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white rounded-2xl w-full max-w-sm p-8 relative shadow-2xl">
            ...
            {checkNumber !== '' && (
              <div className="mb-3">
                <div className="flex gap-2">
                  <input value={checkNumber === 'showInput' ? '' : checkNumber} onChange={(e) => setCheckNumber(e.target.value || 'showInput')} onKeyDown={function(e) { if (e.key === 'Enter') { var cn = checkNumber && checkNumber !== 'showInput' ? checkNumber.trim() : ''; if (!cn) return; var methodLabel = 'Check #' + cn; var entry = corpPaymentModal.reportMonth + ':' + methodLabel; if (corpPaymentModal.splitCenter) { var fieldKey = corpPaymentModal.splitCenter === 'harper' ? 'paidMonthsHarper' : 'paidMonthsAnthony'; var airtableField = corpPaymentModal.splitCenter === 'harper' ? 'Paid Months Harper' : 'Paid Months Anthony'; var currentValue = corpPaymentModal[fieldKey] || ''; var newValue = currentValue ? currentValue + ',' + entry : entry; setCorporatePartners(function(prev) { return prev.map(function(c) { return c.id === corpPaymentModal.id ? Object.assign({}, c, { [fieldKey]: newValue }) : c; }); }); setCorpPaymentModal(null); setCheckNumber(''); fetch('/api/update-corporate', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ recordId: corpPaymentModal.id, [airtableField]: newValue }) }).catch(function() { alert('Failed to sync.'); }); } else { var newPaidMonths = corpPaymentModal.paidMonths ? corpPaymentModal.paidMonths + ',' + entry : entry; setCorporatePartners(function(prev) { return prev.map(function(c) { return c.id === corpPaymentModal.id ? Object.assign({}, c, { paidMonths: newPaidMonths }) : c; }); }); setCorpPaymentModal(null); setCheckNumber(''); fetch('/api/update-corporate', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ recordId: corpPaymentModal.id, paidMonths: newPaidMonths }) }).catch(function() { alert('Failed to sync.'); }); } } }} placeholder="Enter check number..." className="flex-1 p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-[#8b5cf6] text-sm font-bold text-[#001f3f] placeholder-slate-300" autoFocus />
                  <button onClick={function() { var cn = checkNumber && checkNumber !== 'showInput' ? checkNumber.trim() : ''; if (!cn) { alert('Please enter a check number.'); return; } var methodLabel = 'Check #' + cn; var entry = corpPaymentModal.reportMonth + ':' + methodLabel; if (corpPaymentModal.splitCenter) { var fieldKey = corpPaymentModal.splitCenter === 'harper' ? 'paidMonthsHarper' : 'paidMonthsAnthony'; var airtableField = corpPaymentModal.splitCenter === 'harper' ? 'Paid Months Harper' : 'Paid Months Anthony'; var currentValue = corpPaymentModal[fieldKey] || ''; var newValue = currentValue ? currentValue + ',' + entry : entry; setCorporatePartners(function(prev) { return prev.map(function(c) { return c.id === corpPaymentModal.id ? Object.assign({}, c, { [fieldKey]: newValue }) : c; }); }); setCorpPaymentModal(null); setCheckNumber(''); fetch('/api/update-corporate', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ recordId: corpPaymentModal.id, [airtableField]: newValue }) }).catch(function() { alert('Failed to sync.'); }); } else { var newPaidMonths = corpPaymentModal.paidMonths ? corpPaymentModal.paidMonths + ',' + entry : entry; setCorporatePartners(function(prev) { return prev.map(function(c) { return c.id === corpPaymentModal.id ? Object.assign({}, c, { paidMonths: newPaidMonths }) : c; }); }); setCorpPaymentModal(null); setCheckNumber(''); fetch('/api/update-corporate', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ recordId: corpPaymentModal.id, paidMonths: newPaidMonths }) }).catch(function() { alert('Failed to sync.'); }); } }} className="bg-[#8b5cf6] text-white px-5 rounded-xl font-bold text-sm hover:bg-purple-700 transition-colors shadow-sm">Submit</button>
                </div>
              </div>
            )}
        {corpPaymentModal.splitCenter && (<div className={"mb-4 px-4 py-2 rounded-lg text-center text-sm font-black tracking-widest uppercase " + (corpPaymentModal.splitCenter === 'harper' ? 'bg-orange-100 text-orange-700' : 'bg-blue-100 text-blue-700')}>{corpPaymentModal.splitCenter === 'harper' ? 'Harper Center Payment' : 'Anthony Center Payment'}</div>)}
            <div className="grid grid-cols-2 gap-3 mb-6">
              {['Check', 'ACH', 'Card', 'Cash'].map(m => (
                <button key={m} onClick={async () => {
                  ...
                  if (corpPaymentModal.splitCenter) {
                    const fieldKey = corpPaymentModal.splitCenter === 'harper' ? 'paidMonthsHarper' : 'paidMonthsAnthony';
                    const airtableField = corpPaymentModal.splitCenter === 'harper' ? 'Paid Months Harper' : 'Paid Months Anthony';
                    const currentValue = corpPaymentModal[fieldKey] || '';
                    const newValue = currentValue ? `${currentValue},${entry}` : entry;
                    setCorporatePartners(prev => prev.map(c => c.id === corpPaymentModal.id ? { ...c, [fieldKey]: newValue } : c));
                    setCorpPaymentModal(null);
                    setCheckNumber('');
                    try {
                      await fetch('/api/update-corporate', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ recordId: corpPaymentModal.id, [airtableField]: newValue }) });
            ...
            <button onClick={() => { setCorpPaymentModal(null); setCheckNumber(''); }} className="w-full text-slate-400 text-sm font-bold hover:text-slate-600">Cancel</button>
          </div>
        </div>
      )}
    </div>
