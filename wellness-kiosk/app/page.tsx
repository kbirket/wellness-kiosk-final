...
import { 
  ...
  Filter, Download, Bell, FileText, Plus, Smartphone, Clock, Camera, UserCircle, Lock, Printer, Trash2, Briefcase, KeyRound
} from 'lucide-react';
...
export default function WellnessHub() {
  ...
  useEffect(() => {
    ...
    if (savedToken) {
      ...
      .then(data => {
        if (data.valid) {
          setSessionToken(savedToken); setLastActivity(Date.now());
          if (data.type === 'director' && data.user) { setUser(data.user); setViewingCenter(data.user.center); setView('dashboard'); }
          else if (data.type === 'corporate' && data.corp) { setActiveCorp(data.corp); setView('corp_portal'); }
  ...
  const handleCorpLogin = async () => {
    const u = document.getElementById('c_in').value.toLowerCase().trim();
    const p = document.getElementById('c_pin').value.trim();
    try {
      const res = await fetch('/api/login', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ username: u, password: p, loginType: 'corporate' }) });
      const data = await res.json();
      if (data.success) { setActiveCorp(data.corp); setSessionToken(data.token); setLastActivity(Date.now()); localStorage.setItem('wellnessToken', data.token); setView('corp_portal'); }
  ...
  useEffect(() => {
    ...
    window.addEventListener('click', updateActivity); window.addEventListener('keydown', updateActivity);
    window.addEventListener('scroll', updateActivity); window.addEventListener('touchstart', updateActivity);
    return () => { window.removeEventListener('click', updateActivity); window.removeEventListener('keydown', updateActivity); window.removeEventListener('scroll', updateActivity); window.removeEventListener('touchstart', updateActivity); };
  }, []);
  useEffect(() => {
    if (!user && !activeCorp) return;
    const interval = setInterval(() => { if (Date.now() - lastActivity > SESSION_TIMEOUT) { alert('Your session has expired due to inactivity. Please log in again.'); handleLogout(); } }, 60 * 1000);
  ...
  filteredVisits.forEach(v => { const hour = new Date(v.time).getHours(); if (hour >= 6 && hour <= 20) heatmapData[hour - 6]++; });
...
const [quickSearch, setQuickSearch] = useState('');
        ...
        {activeTab === 'reports' && (<div className="space-y-6 print:space-y-3 print:m-0 print:p-0"><div className="flex justify-between items-center mb-8 print:hidden"><div><h2 className="text-3xl font-bold text-[#001f3f] tracking-tight">Monthly Summary Report</h2></div><div className="flex gap-3"><button onClick={() => window.print()} className="bg-white border border-slate-200 text-[#001f3f] px-6 py-3 rounded-xl font-bold text-sm shadow-sm flex items-center gap-2 hover:bg-slate-50 transition-all"><Printer size={16} /> Print Visual Report</button><button onClick={handleMonthlySummary} className="bg-[#1080ad] text-white px-6 py-3 rounded-xl font-bold text-sm shadow-xl flex items-center gap-2 hover:bg-blue-600 transition-all"><FileText size={16} /> Download Raw CSV</button></div></div><div className="hidden print:block mb-8 print:mb-4 text-center border-b-4 border-[#001f3f] pb-8 print:pb-4"><img src={LOGO_URL} alt="Logo" className="h-16 print:h-12 mx-auto mb-4 print:mb-2 invert grayscale" /><h1 className="text-4xl print:text-2xl font-black text-[#001f3f] tracking-tighter">{viewingCenter === 'both' ? 'System-Wide' : viewingCenter.charAt(0).toUpperCase() + viewingCenter.slice(1)} Wellness Center</h1><p className="text-lg print:text-sm font-bold text-slate-500 uppercase tracking-widest mt-2 print:mt-1">Executive Summary • {currentDateString}</p></div><div className="mb-8 print:mb-4"><ProListCard title="Current Membership Breakdown"><div className="py-6 print:py-2"><DonutChart data={planChartData} totalLabel="Members" /></div></ProListCard></div><div className="grid grid-cols-1 md:grid-cols-2 gap-8 print:gap-4 print:grid-cols-2"><ProListCard title="Standard Memberships"><table className="w-full text-sm print:text-xs"><tbody><tr className="border-b print:border-slate-200"><td className="py-3 print:py-1.5 font-medium text-slate-600 print:text-black">Single:</td><td className="py-3 print:py-1.5 font-bold text-right print:text-[#001f3f]">{reportStats.single}</td></tr><tr className="border-b print:border-slate-200"><td className="py-3 print:py-1.5 font-medium text-slate-600 print:text-black">Family:</td><td className="py-3 print:py-1.5 font-bold text-right print:text-[#001f3f]">{reportStats.family}</td></tr><tr className="border-b print:border-slate-200"><td className="py-3 print:py-1.5 font-medium text-slate-600 print:text-black">Senior Citizen:</td><td className="py-3 print:py-1.5 font-bold text-right print:text-[#001f3f]">{reportStats.seniorCitizen}</td></tr><tr className="border-b print:border-slate-200"><td className="py-3 print:py-1.5 font-medium text-slate-600 print:text-black">Senior Family:</td><td className="py-3 print:py-1.5 font-bold text-right print:text-[#001f3f]">{reportStats.seniorFamily}</td></tr><tr><td className="py-3 print:py-1.5 font-medium text-slate-600 print:text-black">Student (14-22):</td><td className="py-3 print:py-1.5 font-bold text-right print:text-[#001f3f]">{reportStats.student}</td></tr></tbody></table></ProListCard><div className="space-y-8 print:space-y-4"><ProListCard title="Corporate, Staff & Military"><table className="w-full text-sm print:text-xs"><tbody><tr className="border-b print:border-slate-200"><td className="py-2 print:py-1.5 font-medium text-slate-600 print:text-black">Corporate:</td><td className="py-2 print:py-1.5 font-bold text-right print:text-[#001f3f]">{reportStats.corporate}</td></tr><tr className="border-b print:border-slate-200"><td className="py-2 print:py-1.5 font-medium text-slate-600 print:text-black">Corporate Family:</td><td className="py-2 print:py-1.5 font-bold text-right print:text-[#001f3f]">{reportStats.corporateFamily}</td></tr><tr className="border-b print:border-slate-200"><td className="py-2 print:py-1.5 font-medium text-slate-600 print:text-black">HD6/HCHF (Staff):</td><td className="py-2 print:py-1.5 font-bold text-right print:text-[#001f3f]">{reportStats.staff}</td></tr><tr><td className="py-2 print:py-1.5 font-medium text-slate-600 print:text-black">Active Military (Free):</td><td className="py-2 print:py-1.5 font-bold text-right print:text-[#001f3f]">{reportStats.military}</td></tr></tbody></table></ProListCard><ProListCard title="Other & Totals"><table className="w-full text-sm print:text-xs"><tbody><tr className="border-b print:border-slate-200"><td className="py-2 print:py-1 font-medium text-slate-600 print:text-black">Day Passes:</td><td className="py-2 print:py-1 font-bold text-right print:text-[#001f3f]">{reportStats.dayPass}</td></tr><tr className="bg-slate-50 print:bg-transparent print:border-t-2 print:border-[#001f3f]"><td className="py-3 print:py-2 px-2 print:px-0 font-bold text-[#001f3f] text-lg print:text-base">Total Members:</td><td className="py-3 print:py-2 px-2 print:px-0 font-black text-right text-[#001f3f] text-lg print:text-base">{stats.total}</td></tr></tbody></table></ProListCard></div></div></div>)}

        {activeTab === 'notif' && (<div className="space-y-6"><div className="flex justify-between items-center mb-8"><div><h2 className="text-3xl font-bold text-[#001f3f] tracking-tight">Notifications</h2><p className="text-slate-400 font-medium">Payment reminders via email & SMS</p></div><button className="bg-[#dd6d22] text-white px-8 py-3 rounded-xl font-bold shadow-lg shadow-orange-900/20 flex items-center gap-2"><Bell size={20} /> Send All Due</button></div><ProListCard title="Due for Reminder"><div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden mt-4"><table className="w-full text-left border-collapse"><thead className="bg-slate-50 text-[11px] font-black text-slate-400 uppercase tracking-widest border-b"><tr><th className="px-8 py-4 w-64">Member</th><th className="px-8 py-4 w-40">Type</th><th className="px-8 py-4 w-32">Status</th><th className="px-8 py-4 w-32">Due</th><th className="px-8 py-4 w-24">Actions</th></tr></thead><tbody className="text-sm">{scopedMembers.filter(m => m.status !== 'ACTIVE').map(m => (<tr key={m.id} className="border-b"><td className="px-8 py-5"><p className="font-bold text-slate-800">{m.firstName} {m.lastName}</p><p className="text-[11px] text-slate-400">{m.email}</p></td><td className="px-8 py-5"><span className="px-3 py-1 rounded-full bg-blue-50 text-blue-600 text-[10px] font-black">{m.type}</span></td><td className="px-8 py-5"><span className={`px-3 py-1 rounded-full text-[10px] font-black ${m.status === 'OVERDUE' ? 'bg-red-100 text-red-600' : 'bg-amber-100 text-amber-600'}`}>{m.status}</span></td><td className="px-8 py-5 text-slate-600 font-medium">{m.nextPayment}</td><td className="px-8 py-5 flex gap-2"><button className="p-2 bg-[#1080ad] text-white rounded-lg shadow-md"><Mail size={16}/></button><button className="p-2 bg-[#dd6d22] text-white rounded-lg shadow-md"><Phone size={16}/></button></td></tr>))}</tbody></table></div></ProListCard></div>)}
