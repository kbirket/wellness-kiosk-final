                            ...
                            displayedVisits.map((v, i) => (
                              <div key={i} className="flex items-center justify-between p-4 bg-slate-50 rounded-xl border border-slate-100">
                                ...
                                <div className="flex items-center gap-3 text-slate-400 text-xs font-medium">
                                  ...
                                  <button onClick={function(e) { e.stopPropagation(); if (!window.confirm('Remove check-in for ' + v.name + ' at ' + new Date(v.time).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}) + '?')) return; var matchMember = members.find(function(m) { return (m.firstName + ' ' + m.lastName) === v.name; }); if (matchMember) { fetch('/api/delete-visit', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ memberAirtableId: matchMember.airtableId, visitTime: v.time, visitCenter: v.center }) }).catch(function(err) { console.error('Could not delete from Airtable:', err); }); setMembers(function(prev) { return prev.map(function(m) { return m.id === matchMember.id ? Object.assign({}, m, { visits: Math.max(0, m.visits - 1) }) : m; }); }); } setVisits(function(prev) { return prev.filter(function(visit) { return !(visit.name === v.name && visit.time === v.time && visit.center === v.center); }); }); }} className="text-slate-300 hover:text-red-500 transition-colors p-1" title="Remove this check-in"><X size={14}/></button>
                                </div>
                              </div>
                              </div>
                            ))
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-3">
              ...
              {corporatePartners.filter(c => c.name.toLowerCase().includes(corpSearch.toLowerCase())).map(corp => {
                 ...
                 const primaryIds = {}; corpMembers.forEach(function(cm) { if (cm.familyName && !primaryIds[cm.familyName]) primaryIds[cm.familyName] = cm.airtableId; }); const enrichedMembers = corpMembers.map(mem => { const memVisits = currentPeriodVisits.filter(v => v.name.toLowerCase() === `${mem.firstName} ${mem.lastName}`.toLowerCase()); totalPeriodVisits += memVisits.length; var rawRate = parseFloat(String(mem.monthlyRate).replace(/[^0-9.]/g, '')) || 0; var isDependent = mem.familyName && primaryIds[mem.familyName] && primaryIds[mem.familyName] !== mem.airtableId; var rate = isDependent ? 0 : rawRate; let memberOwed = 0; let activeMonthsCount = 0; if (isUsageBased) { targetMonths.forEach(mIdx => { const visitedInMonth = memVisits.some(v => new Date(v.time).getMonth() === mIdx); if (visitedInMonth) { memberOwed += rate; activeMonthsCount++; } }); } else { if (mem.status === 'ACTIVE') { memberOwed = rate * targetMonths.length; activeMonthsCount = targetMonths.length; } } totalOwed += memberOwed; return { ...mem, periodVisits: memVisits.length, memberOwed, activeMonthsCount }; });
...
{(selectedMember.needsOrientation || !selectedMember.orientationAnthony || !selectedMember.orientationHarper) && (<div className="mb-6 bg-blue-50 border-l-4 border-blue-500 p-4 rounded-r-lg">
              ...
              <p className="text-[10px] font-black text-purple-600 uppercase tracking-widest mb-3">Family Members — {selectedMember.familyName}</p><div className="space-y-2">{famMembers.map(fm => (<div key={fm.airtableId} className="flex justify-between items-center bg-white p-3 rounded-lg border border-purple-100 hover:border-purple-300 hover:bg-purple-50 transition-all group"><button onClick={() => setSelectedMember(fm)} className="flex-1 text-left"><p className="font-bold text-[#001f3f] group-hover:text-[#8b5cf6] transition-colors">{fm.firstName} {fm.lastName}</p><p className="text-[9px] text-slate-400 uppercase tracking-widest">{fm.id} • {fm.type}</p></button><div className="flex items-center gap-2"><ChevronRight size={14} className="text-slate-300 group-hover:text-[#8b5cf6] transition-colors" /><button onClick={async (e) => { e.stopPropagation(); if (!window.confirm("Remove " + fm.firstName + " " + fm.lastName + " from this family group? They will keep their individual membership.")) return; try { var res = await fetch('/api/update-member', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ airtableId: fm.airtableId, familyGroup: null }) }); if (res.ok) { setMembers(function(prev) { return prev.map(function(m) { return m.airtableId === fm.airtableId ? Object.assign({}, m, { familyName: '' }) : m; }); }); alert(fm.firstName + " has been removed from the family group."); } else { alert("Could not remove from family."); } } catch (err) { alert("Network error."); } }} className="text-slate-300 hover:text-red-500 transition-colors p-1 shrink-0" title="Remove from family"><X size={14}/></button></div></div>))}</div>
                 ...
                 {user?.role !== 'business' && (<div className="mt-14 grid grid-cols-2 md:grid-cols-3 gap-3">
                    ...
                    <button onClick={() => { setProratePayment(false); setCheckNumber(''); setPaymentModal(selectedMember); }} className="bg-green-50 text-green-600 hover:bg-green-600 hover:text-white py-4 rounded-xl font-bold shadow-sm transition-all flex flex-col items-center justify-center gap-2 text-xs"><CreditCard size={20} /> Log Payment</button>
      ...
      {paymentModal && (() => {
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
         ...
         <div className="mb-5">
            ...
            <div className="grid grid-cols-2 gap-3 mb-6">
              ...
              <div className="mb-3">
                <input value={checkNumber === 'showInput' ? '' : checkNumber} onChange={(e) => setCheckNumber(e.target.value || 'showInput')} placeholder="Enter check number..." className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-[#8b5cf6] text-sm font-bold text-[#001f3f] placeholder-slate-300" autoFocus />
                  ...
                  try {
                    const res = await fetch('/api/log-payment', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ airtableId: paymentModal.airtableId, memberName: `${paymentModal.firstName} ${paymentModal.lastName}`, method: m === 'Check' && checkNumber && checkNumber !== 'showInput' ? 'Check #' + checkNumber.trim() : m, currentDueDate: paymentModal.nextPayment, prorated: proratePayment, proratedAmount: proratePayment ? proratedAmount : null }) });
                    const result = await res.json();
                    if (result.success) {
              ...
              const receiptAmt = proratePayment ? proratedAmount.toFixed(2) : fullRate.toFixed(2);
            ...
            <button onClick={() => { setPaymentModal(null); setCheckNumber(''); }} className="w-full text-slate-400 text-sm font-bold hover:text-slate-600">Cancel</button>
          </div>
        </div>
        );
      })()}
            ...
            <div className="grid grid-cols-2 gap-3 mb-6">
              {['Check', 'ACH', 'Card', 'Cash'].map(m => (
                <button key={m} onClick={async () => {
                  ...
                  var cleanCheck = checkNumber && checkNumber !== 'showInput' ? checkNumber.trim() : ''; var methodLabel = m === 'Check' && cleanCheck ? 'Check #' + cleanCheck : m;
