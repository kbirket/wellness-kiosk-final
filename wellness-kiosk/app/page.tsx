...
const heatmapData = Array(15).fill(0); visits.forEach(v => { if (viewingCenter !== 'both' && !(v.center && v.center.toLowerCase().includes(viewingCenter))) return; const hour = new Date(v.time).getHours(); if (hour >= 6 && hour <= 20) heatmapData[hour - 6]++; }); const maxVisits = Math.max(...heatmapData, 1);
