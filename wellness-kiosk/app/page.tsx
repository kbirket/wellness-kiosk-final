              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <ProListCard title="Peak Hours Heatmap">
                  <div className="flex items-end justify-between h-48 mt-8 gap-2">
                    {heatmapData.map((count,i) => { const hp = count===0?5:(count/maxVisits)*100; const hl = (i+6)>12?`${(i+6)-12}P`:`${i+6}A`; return (<div key={i} className="flex-1 flex flex-col items-center gap-2 group"><div className="w-full rounded-t-md relative flex items-end justify-center transition-colors" style={{height:'100%', backgroundColor:'#dbeafe'}}><div className="w-full rounded-t-md transition-all duration-500 relative" style={{height:`${hp}%`, backgroundColor:'#1080ad'}}><span className="absolute -top-6 left-1/2 -translate-x-1/2 text-xs font-bold text-[#001f3f] opacity-0 group-hover:opacity-100 transition-opacity">{count}</span></div></div><span className="text-[10px] font-bold text-slate-400">{hl}</span></div>); })}
                  </div>
                </ProListCard>
              </div>
            </div>
