...
export default function WellnessHub() {
  ...
  const handleMonthlySummary = () => {
    ...
    // For the placeholder, we'll export standard metrics. We can update this when you provide the exact columns!
    const headers = ["Member Name", "Member ID", "Center", "Status", "Total Check-ins"];
    const csvRows = [
      headers.join(','),
      ...scopedMembers.map(m => `"${m.firstName} ${m.lastName}","${m.id}","${m.center}","${m.status}","${m.visits}"`)
    ].join('\n');
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `${centerName}_Monthly_Summary_${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
    window.URL.revokeObjectURL(url);
  };
    ...
    <div className="flex min-h-screen bg-[#f0f2f5] font-sans text-slate-800">
      ...
      <main className="flex-1 p-10 h-screen overflow-y-auto">
        ...
        {activeTab === 'reports' && (
          <div className="space-y-6">
             <div className="mb-8">
                <h2 className="text-3xl font-bold text-[#001f3f] tracking-tight">Reports & Exports</h2>
                <p className="text-slate-400 font-medium">Generate data exports for {viewingCenter === 'both' ? 'both locations' : viewingCenter.charAt(0).toUpperCase() + viewingCenter.slice(1) + ' Center'}</p>
             </div>
             
             <div className="grid grid-cols-1 gap-6">
                <ProListCard 
                  title="Monthly Center Summary"
                  actions={
                    <button onClick={handleMonthlySummary} className="bg-[#1080ad] text-white px-6 py-2 rounded-xl font-bold text-sm shadow-xl flex items-center gap-2 hover:bg-blue-600 transition-all">
                      <Download size={16} /> Download Summary
                    </button>
                  }
                >
                  <p className="text-slate-500 mb-4">Export a complete monthly breakdown of your members, their current statuses, and total visits for easy reporting.</p>
                  <div className="flex gap-4 items-center p-4 bg-slate-50 rounded-xl border border-slate-100">
                    <FileText className="text-slate-400" />
                    <div>
                      <p className="text-sm font-bold text-[#001f3f]">{viewingCenter === 'both' ? 'System-Wide' : viewingCenter.charAt(0).toUpperCase() + viewingCenter.slice(1)} Data</p>
                      <p className="text-xs text-slate-500">Contains {scopedMembers.length} records</p>
                    </div>
                  </div>
                </ProListCard>
             </div>
...
export default function WellnessHub() {
  ...
  // THE FIX: Uses .includes() instead of strict equals
  const scopedMembers = members.filter(m => viewingCenter === 'both' || (m.center && m.center.toLowerCase().includes(viewingCenter)));
  const filteredMembers = scopedMembers.filter(m => `${m.firstName} ${m.lastName} ${m.id}`.toLowerCase().includes(searchQuery.toLowerCase()));
    ...
    <ProListCard title="By Location">
      <div className="flex gap-4">
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 flex-1 border-l-6" style={{ borderLeftColor: centerColors.Harper }}>
          <p className="text-5xl font-extrabold mb-1" style={{ color: centerColors.Harper }}>{members.filter(m => m.center && m.center.toLowerCase().includes('harper')).length}</p>
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 flex-1 border-l-6" style={{ borderLeftColor: centerColors.Anthony }}>
          <p className="text-5xl font-extrabold mb-1" style={{ color: centerColors.Anthony }}>{members.filter(m => m.center && m.center.toLowerCase().includes('anthony')).length}</p>
  ...
  const MixChart = () => (
    <ProListCard title="Membership Mix">
      <div className="space-y-3">
        {[
          { label: 'Monthly', count: scopedMembers.filter(m => m.type === 'MONTHLY').length, color: '#001f3f' },
          { label: 'Annual', count: scopedMembers.filter(m => m.type === 'ANNUAL').length, color: '#1080ad' },
          { label: 'Day Pass / Punch Card', count: scopedMembers.filter(m => m.type === 'DAY PASS / PUNCH CARD').length, color: '#dd6d22' },
          { label: 'Family', count: scopedMembers.filter(m => m.type === 'FAMILY').length, color: '#f59e0b' },
        ].map(item => {
          const p = scopedMembers.length > 0 ? Math.round((item.count / scopedMembers.length) * 100) : 0;
          return (
