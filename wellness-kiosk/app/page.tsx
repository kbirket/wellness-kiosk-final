...
var showToast = function(message, type, duration) { setToast({ message: message, type: type || 'success' }); setTimeout(function() { setToast(null); }, duration || 4000); };
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {displayedClasses.map((c, i) => {
                  var classRosterKey = c.name + '_' + c.center + '_' + selectedClassDate.toISOString().split('T')[0]; var classRoster = savedClassRosters[classRosterKey]; var classVisits = classRoster ? classRoster.attendees.map(function(a) { return { name: a.name, time: a.time, type: a.type, center: c.center === 'anthony' ? 'Anthony' : 'Harper', method: 'Class: ' + c.name }; }) : [];
                  return (
                    <div key={i} className={`bg-white p-6 rounded-2xl shadow-sm border border-slate-100 border-t-4 ${c.color} flex flex-col justify-between`}>
                      ...
                      <div className="flex justify-between items-end mt-6"><div><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Check-ins</p><p className="text-3xl font-black text-[#1080ad] leading-none">{classVisits.length}</p></div><button onClick={() => setActiveClass(c)} className="bg-blue-50 text-blue-600 hover:bg-blue-600 hover:text-white px-5 py-3 rounded-xl text-sm font-bold transition-colors shadow-sm flex items-center justify-center gap-2">
  <Users size={16} /> Manage Roster
              ...
              {activeClass && (() => {
                var activeRosterKey = activeClass.name + '_' + activeClass.center + '_' + checkinDate; var activeRoster = savedClassRosters[activeRosterKey]; var classVisits = activeRoster ? activeRoster.attendees.map(function(a) { return { name: a.name, time: a.time, type: a.type, center: activeClass.center === 'anthony' ? 'Anthony' : 'Harper', method: 'Class: ' + activeClass.name }; }) : [];
                return (
                    ...
                    <div className="bg-white rounded-3xl w-full max-w-4xl p-10 relative shadow-2xl flex flex-col md:flex-row gap-8">
                      ...
                      <div className="flex-1 relative">
                        ...
                        <div className="flex gap-3">
                          <input autoFocus className="flex-1 p-5 border-2 border-slate-200 rounded-2xl outline-none focus:border-[#1080ad] text-2xl bg-slate-50" placeholder="e.g. Smith" value={kioskInput} onChange={(e) => setKioskInput(e.target.value)}onKeyDown={(e) => { if (e.key === 'Enter') { var foundMember = members.find(function(mem) { return mem.id === kioskInput.toUpperCase().trim() || (mem.firstName + ' ' + mem.lastName).toLowerCase().includes(kioskInput.toLowerCase()); }); if (foundMember) { var rosterTime2 = new Date().toISOString(); var rosterKey2 = activeClass.name + '_' + activeClass.center + '_' + checkinDate; var alreadyVisited2 = visits.some(function(v) { return v.name === (foundMember.firstName + ' ' + foundMember.lastName) && new Date(v.time).toDateString() === new Date().toDateString(); }); if (!alreadyVisited2) { var classCenter2 = activeClass.center === 'anthony' ? 'Anthony' : 'Harper'; fetch('/api/visits', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ airtableId: foundMember.airtableId, center: classCenter2, time: rosterTime2, method: 'Class: ' + activeClass.name }) }).then(function(r) { return r.json(); }).then(function(result) { if (result.success) { setVisits(function(prev) { return [{ name: foundMember.firstName + ' ' + foundMember.lastName, center: classCenter2, time: rosterTime2, type: foundMember.type, method: 'Class: ' + activeClass.name }, ...prev]; }); setMembers(function(prev) { return prev.map(function(mem) { return mem.id === foundMember.id ? Object.assign({}, mem, { visits: mem.visits + 1 }) : mem; }); }); } }); } setSavedClassRosters(function(prev) { var existing = prev[rosterKey2] || { className: activeClass.name, center: activeClass.center, date: checkinDate, attendees: [] }; existing.attendees.push({ name: foundMember.firstName + ' ' + foundMember.lastName, time: rosterTime2, type: foundMember.type }); var updated = Object.assign({}, prev); updated[rosterKey2] = existing; return updated; }); setKioskMessage({ text: foundMember.firstName + ' added to class!', type: 'success', subtext: '' }); setTimeout(function() { setKioskMessage({ text: '', type: '', subtext: '' }); }, 3000); } else { setKioskMessage({ text: 'Member not found', type: 'error', subtext: 'Try searching by name.' }); setTimeout(function() { setKioskMessage({ text: '', type: '', subtext: '' }); }, 3000); } setKioskInput(''); } }} />
                          <button onClick={() => { var foundMem = members.find(function(mem) { return mem.id === kioskInput.toUpperCase().trim() || (mem.firstName + ' ' + mem.lastName).toLowerCase().includes(kioskInput.toLowerCase()); }); if (foundMem) { var rosterTime3 = new Date().toISOString(); var rosterKey3 = activeClass.name + '_' + activeClass.center + '_' + checkinDate; var alreadyVisited3 = visits.some(function(v) { return v.name === (foundMem.firstName + ' ' + foundMem.lastName) && new Date(v.time).toDateString() === new Date().toDateString(); }); if (!alreadyVisited3) { var classCenter3 = activeClass.center === 'anthony' ? 'Anthony' : 'Harper'; fetch('/api/visits', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ airtableId: foundMem.airtableId, center: classCenter3, time: rosterTime3, method: 'Class: ' + activeClass.name }) }).then(function(r) { return r.json(); }).then(function(result) { if (result.success) { setVisits(function(prev) { return [{ name: foundMem.firstName + ' ' + foundMem.lastName, center: classCenter3, time: rosterTime3, type: foundMem.type, method: 'Class: ' + activeClass.name }, ...prev]; }); setMembers(function(prev) { return prev.map(function(mem) { return mem.id === foundMem.id ? Object.assign({}, mem, { visits: mem.visits + 1 }) : mem; }); }); } }); } setSavedClassRosters(function(prev) { var existing = prev[rosterKey3] || { className: activeClass.name, center: activeClass.center, date: checkinDate, attendees: [] }; existing.attendees.push({ name: foundMem.firstName + ' ' + foundMem.lastName, time: rosterTime3, type: foundMem.type }); var updated = Object.assign({}, prev); updated[rosterKey3] = existing; return updated; }); setKioskMessage({ text: foundMem.firstName + ' added to class!', type: 'success', subtext: '' }); setTimeout(function() { setKioskMessage({ text: '', type: '', subtext: '' }); }, 3000); } else { setKioskMessage({ text: 'Member not found', type: 'error', subtext: 'Try searching by name.' }); setTimeout(function() { setKioskMessage({ text: '', type: '', subtext: '' }); }, 3000); } setKioskInput(''); }}className="bg-[#001f3f] text-white px-8 rounded-2xl font-bold text-xl hover:bg-blue-900 transition-colors shadow-lg">Check In</button>
                        </div>
                        {kioskMatches.length > 0 && (<div className="absolute top-[80%] left-0 right-0 mt-2 bg-white border-2 border-[#1080ad] rounded-2xl shadow-2xl z-50 overflow-hidden text-left">{kioskMatches.map(m => (<button key={m._type + (m.airtableId || m.id)} onClick={() => { var rosterTime = new Date().toISOString(); var rosterKey = activeClass.name + '_' + activeClass.center + '_' + new Date().toISOString().split('T')[0]; var alreadyVisited = visits.some(function(v) { return v.name === (m.firstName + ' ' + m.lastName) && new Date(v.time).toDateString() === new Date().toDateString(); }); if (!alreadyVisited) { var classCenter = activeClass.center === 'anthony' ? 'Anthony' : 'Harper'; fetch('/api/visits', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ airtableId: m.airtableId, center: classCenter, time: rosterTime, method: 'Class: ' + activeClass.name }) }).then(function(r) { return r.json(); }).then(function(result) { if (result.success) { setVisits(function(prev) { return [{ name: m.firstName + ' ' + m.lastName, center: classCenter, time: rosterTime, type: m.type, method: 'Class: ' + activeClass.name }, ...prev]; }); setMembers(function(prev) { return prev.map(function(mem) { return mem.id === m.id ? Object.assign({}, mem, { visits: mem.visits + 1 }) : mem; }); }); } }); } setSavedClassRosters(function(prev) { var existing = prev[rosterKey] || { className: activeClass.name, center: activeClass.center, date: new Date().toISOString().split('T')[0], attendees: [] }; existing.attendees.push({ name: m.firstName + ' ' + m.lastName, time: rosterTime, type: m.type || 'VISITOR' }); var updated = Object.assign({}, prev); updated[rosterKey] = existing; return updated; }); setKioskMessage({ text: m.firstName + ' added to class!', type: 'success', subtext: '' }); setTimeout(function() { setKioskMessage({ text: '', type: '', subtext: '' }); }, 3000); setKioskInput(''); }}className="w-full p-4 border-b border-slate-100 hover:bg-blue-50 transition-colors flex justify-between items-center group"><div><p className="font-bold text-[#001f3f] text-lg">{m.firstName} {m.lastName}</p></div><div className="bg-[#1080ad] text-white px-4 py-2 rounded-xl text-sm font-bold shadow-md group-hover:scale-105 transition-transform">Check In</div></button>))}</div>)}
