      ...
      {selectedMember && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#001f3f]/90 backdrop-blur-md">
           <div className="bg-white rounded-[2rem] w-full max-w-4xl flex overflow-hidden shadow-2xl relative max-h-[90vh]">
              ...
              <div className="w-1/3 bg-slate-50 p-12 flex flex-col items-center justify-center border-r border-slate-100"><div className="mb-6 relative group"><MemberPhoto src={selectedMember.photoUrl} name={selectedMember.firstName} size={140} className="shadow-xl border-4 border-white" /><label className="absolute inset-0 rounded-full bg-black/0 group-hover:bg-black/40 flex items-center justify-center cursor-pointer transition-all"><input type="file" accept="image/*" className="hidden" onChange={async (e) => { const file = e.target.files[0]; if (!file) return; if (file.size > 5 * 1024 * 1024) { alert('Photo must be under 5MB.'); return; } const reader = new FileReader(); reader.onload = async () => { try { const res = await fetch('/api/upload-photo', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ airtableId: selectedMember.airtableId, fileName: file.name, fileType: file.type, fileData: reader.result }) }); const result = await res.json(); if (result.success && result.photoUrl) { setMembers(prev => prev.map(m => m.airtableId === selectedMember.airtableId ? { ...m, photoUrl: result.photoUrl } : m)); setSelectedMember({ ...selectedMember, photoUrl: result.photoUrl }); } else { alert('Upload failed: ' + (result.error || 'Unknown error')); } } catch (err) { alert('Network error uploading photo.'); } }; reader.readAsDataURL(file); }} /><Camera size={28} className="text-white opacity-0 group-hover:opacity-100 transition-opacity" /></label></div><div className="bg-white p-6 rounded-2xl shadow-xl mb-8 border border-slate-100"><QRCode data={selectedMember.id} size={180} /></div><p className="text-[10px] font-black text-slate-300 uppercase tracking-widest mb-1">Member Identity</p><p className="text-xl font-bold text-[#001f3f] mb-6">#{selectedMember.id}</p><p className="text-[10px] font-black text-slate-300 uppercase tracking-widest mb-1">Kiosk PIN</p><p className="text-3xl font-black tracking-[0.2em] text-[#1080ad]">{selectedMember.password}</p></div>
