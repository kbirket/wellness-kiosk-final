...
{(selectedMember.needsOrientation || !selectedMember.orientationAnthony || !selectedMember.orientationHarper) && (<div className="mb-6 bg-blue-50 border-l-4 border-blue-500 p-4 rounded-r-lg">
              ...
              <p className="text-[10px] font-black text-purple-600 uppercase tracking-widest mb-3">Family Members — {selectedMember.familyName}</p><div className="space-y-2">{famMembers.map(fm => (<div key={fm.airtableId} className="flex justify-between items-center bg-white p-3 rounded-lg border border-purple-100 hover:border-purple-300 hover:bg-purple-50 transition-all group"><button onClick={() => setSelectedMember(fm)} className="flex-1 text-left"><p className="font-bold text-[#001f3f] group-hover:text-[#8b5cf6] transition-colors">{fm.firstName} {fm.lastName}</p><p className="text-[9px] text-slate-400 uppercase tracking-widest">{fm.id} • {fm.type}</p></button><div className="flex items-center gap-2"><ChevronRight size={14} className="text-slate-300 group-hover:text-[#8b5cf6] transition-colors" /><button onClick={async (e) => { e.stopPropagation(); if (!window.confirm("Remove " + fm.firstName + " " + fm.lastName + " from this family group? They will keep their individual membership.")) return; try { var res = await fetch('/api/update-member', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ airtableId: fm.airtableId, familyGroup: null }) }); if (res.ok) { setMembers(function(prev) { return prev.map(function(m) { return m.airtableId === fm.airtableId ? Object.assign({}, m, { familyName: '' }) : m; }); }); alert(fm.firstName + " has been removed from the family group."); } else { alert("Could not remove from family."); } } catch (err) { alert("Network error."); } }} className="text-slate-300 hover:text-red-500 transition-colors p-1 shrink-0" title="Remove from family"><X size={14}/></button></div></div>))}</div>
                 ...
                 {user?.role !== 'business' && (<div className="mt-14 grid grid-cols-2 md:grid-cols-3 gap-3">
                    ...
                    <button onClick={() => { const isHarper = selectedMember.center && selectedMember.center.toLowerCase().includes('harper'); const centerName = isHarper ? 'Harper Wellness Center' : 'Anthony Wellness Center'; const centerAddr = isHarper ? '615 W 12th St, Harper, KS 67058' : '309 W Main St, Anthony, KS 67003'; const centerPhone = isHarper ? '(620) 896-1202' : '(620) 842-5190'; const centerHours = isHarper ? 'M-F 8am-12pm &amp; 5pm-8pm, Sat 9am-noon' : 'M-F 7am-8pm, Sat 8am-1pm'; const directorName = isHarper ? 'Patrick Johnson' : 'Deanna Smithhisler'; const addressBlock = selectedMember.address ? `${selectedMember.address}<br/>${selectedMember.city}, ${selectedMember.state} ${selectedMember.zip}` : ''; const qrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=160x160&data=${encodeURIComponent(selectedMember.id)}&color=003d6b&bgcolor=ffffff`; const w = window.open('', '_blank'); w.document.write(`<!DOCTYPE html><html><head><title>New Check-In System - ${selectedMember.firstName} ${selectedMember.lastName}</title><style>@page{margin:0.4in 0.5in}@media print{body{margin:0}*{-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}}body{font-family:Arial,sans-serif;color:#1e293b;margin:0}.page{max-width:680px;margin:0 auto}.hdr{background:#003d6b;padding:14px 32px;display:flex;justify-content:space-between;align-items:center}.hdr-left{display:flex;align-items:center;gap:14px}.hdr-logo{height:30px;opacity:.95}.hdr-name{font-size:16px;font-weight:700;color:#fff}.hdr-sub{font-size:9px;color:#8bb8d9;letter-spacing:1px;margin-top:2px}.accent{height:3px;background:linear-gradient(to right,#dba51f,#dd6d22)}.body{padding:24px 32px}.top-row{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:16px}.addr{font-size:13px;line-height:1.5}.date{font-size:11px;color:#94a3b8}.greeting{font-size:14px;margin-bottom:10px;font-weight:700;color:#003d6b}.intro{font-size:12px;color:#475569;line-height:1.8;margin-bottom:16px}.section-title{font-size:11px;font-weight:900;color:#003d6b;text-transform:uppercase;letter-spacing:1.5px;margin-bottom:8px;padding-bottom:4px;border-bottom:2px solid #dba51f}.box{border:1.5px solid #003d6b;border-radius:6px;overflow:hidden;margin-bottom:14px}.box-hdr{background:#003d6b;padding:6px 14px;font-size:9px;font-weight:700;color:#fff;letter-spacing:1.5px}.box-body{padding:0 14px}.row{display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid #e2e8f0}.row:last-child{border-bottom:none}.lbl{font-size:11px;color:#64748b}.val{font-size:11px;font-weight:700;color:#003d6b}.cred-section{display:flex;align-items:center;gap:24px;justify-content:center;margin:16px 0;padding:16px;background:#f8fafc;border-radius:8px;border:1px solid #e2e8f0}.cred-left{text-align:center}.cred-right{text-align:center;padding-left:24px;border-left:1px solid #e2e8f0}.pin-display{font-size:28px;font-weight:900;letter-spacing:0.3em;color:#003d6b;margin:4px 0}.pin-label{font-size:9px;color:#94a3b8;text-transform:uppercase;letter-spacing:2px}.how-to{font-size:11px;color:#475569;line-height:1.8;margin:12px 0;padding:10px 14px;background:#eff6ff;border-radius:6px;border:1px solid #bfdbfe}.how-to strong{color:#003d6b}.portal-box{margin:14px 0;padding:14px;background:#f0fdf4;border-radius:8px;border:1.5px solid #bbf7d0;text-align:center}.portal-url{font-size:14px;font-weight:900;color:#16a34a;word-break:break-all}.portal-note{font-size:10px;color:#64748b;margin-top:4px}.sign{font-size:12px;margin-bottom:2px;margin-top:16px}.sign-name{font-size:12px;font-weight:700;color:#003d6b}.sign-title{font-size:10px;color:#94a3b8}.ftr{border-top:2px solid #003d6b;padding:8px 32px;display:flex;justify-content:space-between;align-items:center;margin-top:14px}.ftr-l{font-size:9px;color:#94a3b8}.ftr-r{font-size:9px;color:#1080ad}</style></head><body><div class="page"><div class="hdr"><div class="hdr-left"><img src="https://pattersonhc.org/sites/default/files/wellness_white.png" class="hdr-logo" /><div><div class="hdr-name">${centerName}</div><div class="hdr-sub">${centerAddr} | ${centerPhone}</div></div></div></div><div class="accent"></div><div class="body"><div class="top-row"><div class="addr"><strong>${selectedMember.firstName} ${selectedMember.lastName}</strong>${addressBlock ? '<br/>' + addressBlock : ''}</div><div class="date">${new Date().toLocaleDateString('en-US', {month:'long',day:'numeric',year:'numeric'})}</div></div><div class="greeting">Dear ${selectedMember.firstName},</div><div class="intro">We are excited to announce a new digital check-in system at ${centerName}! Starting soon, you will be able to check in quickly at the front desk kiosk using either your personal QR code or your name and PIN. You can also view your membership details online anytime through our new Member Portal.</div><div class="section-title">Your Check-In Credentials</div><div class="cred-section"><div class="cred-left"><p style="font-size:9px;font-weight:700;color:#64748b;text-transform:uppercase;letter-spacing:2px;margin-bottom:6px">Your QR Badge</p><img src="${qrUrl}" width="120" height="120" /><p style="font-size:10px;color:#64748b;margin-top:4px">Hold up to the kiosk camera</p></div><div class="cred-right"><p class="pin-label">Your 4-Digit PIN</p><p class="pin-display">${selectedMember.password}</p><p style="font-size:10px;color:#94a3b8">Use when checking in by name</p><p class="pin-label" style="margin-top:12px">Member ID</p><p style="font-size:16px;font-weight:900;color:#003d6b;font-family:monospace">${selectedMember.id}</p></div></div><div class="how-to"><strong>How to Check In at the Kiosk:</strong><br/>1. <strong>Scan</strong> &mdash; Hold this letter (or your phone) up to the kiosk camera to scan the QR code above<br/>2. <strong>Search</strong> &mdash; Or type your name on the kiosk screen, select yourself, and enter your 4-digit PIN<br/>3. A green &ldquo;Welcome&rdquo; message confirms you are checked in!</div><div class="section-title">Your Online Member Portal</div><div class="intro">You can now view your membership status, visit history, and QR code online anytime. No app download required &mdash; just visit the link below from any phone, tablet, or computer.</div><div class="portal-box"><p style="font-size:9px;font-weight:700;color:#64748b;text-transform:uppercase;letter-spacing:2px;margin-bottom:6px">Member Portal</p><p class="portal-url">wellness-kiosk-7k6p.vercel.app</p><p class="portal-note">Tap &ldquo;Member Portal&rdquo; &bull; Search your name &bull; Enter your 4-digit PIN</p></div><div class="box"><div class="box-hdr">YOUR MEMBERSHIP SUMMARY</div><div class="box-body"><div class="row"><span class="lbl">Member ID</span><span class="val">${selectedMember.id}</span></div><div class="row"><span class="lbl">Membership Type</span><span class="val">${selectedMember.type}</span></div><div class="row"><span class="lbl">Home Center</span><span class="val">${selectedMember.center}</span></div><div class="row"><span class="lbl">Hours</span><span class="val">${centerHours.replace(/&amp;/g, '&')}</span></div></div></div><div class="intro" style="font-size:11px;color:#64748b;margin-top:12px">Please keep this letter for your records. Your PIN is private &mdash; do not share it with others. If you have any questions about the new system, please ask the front desk staff and we will be happy to help!</div><div class="sign">We look forward to seeing you!</div><div class="sign-name">${directorName}</div><div class="sign-title">Director, ${centerName}</div></div><div class="ftr"><span class="ftr-l">${centerName} | Harper County, KS</span><span class="ftr-r">pattersonhc.org/wellness-centers</span></div></div></body></html>`); w.document.close(); setTimeout(() => w.print(), 500); }} className="bg-[#003d6b]/10 text-[#003d6b] hover:bg-[#003d6b] hover:text-white py-4 rounded-xl font-bold shadow-sm transition-all flex flex-col items-center justify-center gap-2 text-xs"><FileText size={20} /> Launch Letter</button>
